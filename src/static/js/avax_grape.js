$(function () {		
  ...
  async function main() {		
      ...
      const StakingPool = {
        ...
        abi : GRAPE_STAKING_ABI,
        stakeTokenFunction : "share",
        rewardTokenAddresses : "grape"
      }
      
      const WINE_CHEF_ADDR = "0x28c65dcb3a5f0d456624aff91ca03e4e315bee49";
      const rewardTokenTicker = "WINE";
      const WINE_CHEF = new ethers.Contract(WINE_CHEF_ADDR, WINE_CHEF_ABI, App.provider);

      const rewardsPerWeek = await WINE_CHEF.winePerSecond() / 1e18 * 604800;
          
      const tokens = {};
      const prices = await getAvaxPrices();

      await loadAvaxWINEContract(App, tokens, prices, WINE_CHEF, WINE_CHEF_ADDR, WINE_CHEF_ABI, rewardTokenTicker,		
          "wine", null, rewardsPerWeek, "pendingShare");
      _print(`\nThe winery does not print Grape when below 1.01 TWAP, staking here below 1.01 TWAP will not generate rewards.`);
      _print(`Staked WINE can only be withdrawn every 4 epochs (24hrs) & rewards claimed every 2 epochs (12hrs). Staking or claiming resets this timer.\n`)
      const p0 = await loadAvaxGRAPESynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                      StakingPool.address, 
...
async function loadAvaxGRAPESynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadAvaxGRAPESynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printGRAPESynthetixPool(App, info, "avax");
}

async function loadAvaxGRAPESynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printGRAPESynthetixPool(App, info, chain="eth", customURLs) {
...
async function loadAvaxWINEContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
