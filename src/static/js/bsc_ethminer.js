...
async function main() {
  ...
  const addr = "0x212A3A41a0e58CCdc86F013b003d4afF805a958c";
  const contract = new ethers.Contract(addr, abi, app.provider);
  const contractSend = new ethers.Contract(addr, abi, signer);
  const prices = await getBscPrices();
  const ethUSD = prices["0x2170ed0880ac9a755fd29b2688956bd959f933f8"]["usd"];
  ...
  const barrel = Number(ethers.utils.formatEther(sell) - ethers.utils.formatEther(devFee)).toFixed(8);
  ...
  const balanceUsd = numberWithCommas(Number(ethUSD * balance).toFixed(2));
  ...
  _print("Barrel: " + barrel + " ETH");
  _print("");
  _print_link("Hire more miners", compound);
  _print_link("Pocket your ETH", sellEggs);
  _print("");
  _print("Contract balance: " + balanceStr + " ETH ($" + balanceUsd + ")");
