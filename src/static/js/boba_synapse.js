...
async function main() {
    ...
    const SYN_CHEF_ADDR = "0xd5609cD0e1675331E4Fb1d43207C8d9D83AAb17C";
    const rewardTokenTicker = "SYN";
    const SYN_CHEF = new ethers.Contract(SYN_CHEF_ADDR, SYN_CHEF_ABI, App.provider);

    const rewardsPerWeek = await SYN_CHEF.synapsePerSecond() /1e18 * 604800;

    const tokens = {};
    const prices = await getBobaPrices();

    prices["0x9D7283A6AeeD9BCd4Ac70876fEA2b69a63DD8cb9"] = { usd : 1 };
    prices["0x498657f2AF18D525049dE520dD86ee376Db9c67c"] = prices["0xDeadDeAddeAddEAddeadDEaDDEAdDeaDDeAD0000"];
    
    await loadSynChefContract(App, tokens, prices, SYN_CHEF, SYN_CHEF_ADDR, SYN_CHEF_ABI, rewardTokenTicker,
        "SYNAPSE", null, rewardsPerWeek, "pendingSynapse");

    hideLoading();
}

async function getSynPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  ...
  const poolToken = await getBobaToken(app, lpToken, chefAddress);
...
async function loadSynChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    ...
    await getSynPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printChefPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
        ...
        pendingRewardsFunction, null, null, "boba", poolInfos[i].depositFee, poolInfos[i].withdrawFee)
      aprs.push(apr);
    }
  }
