...
const NERVE_CHEF_ABI = [
  {
    "inputs": [
      {
        "internalType": "contract BrainCell",
        "name": "_govToken",
        "type": "address"
      },
      {
...
async function main() {
   ...
   const NERVE_CHEF_ADDR = "0xe653f898b1F216884bC1Fdd5B47b9933B5F4e5FB";
   const rewardTokenTicker = "CELL";
   const NERVE_CHEF = new ethers.Contract(NERVE_CHEF_ADDR, NERVE_CHEF_ABI, App.provider);
   
   const rewardsPerWeek = await NERVE_CHEF.REWARD_PER_BLOCK() /1e18
        * 604800 / 1.1;
    const tokens = {};
    const prices = await getAuroraPrices();
 
    await loadGeneralEthcallChefContract(App, tokens, prices, NERVE_CHEF, NERVE_CHEF_ADDR, NERVE_CHEF_ABI, rewardTokenTicker,
        "govToken", null, rewardsPerWeek, "pendingReward", [], "aurora");
    hideLoading();
  }