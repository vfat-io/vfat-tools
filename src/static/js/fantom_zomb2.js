...
const ZOMBI_REWARD_GENERIS_POOL_ABI = [
  {
    "inputs": [
      {
...
const TOMB_REWARD_POOL_ADDR = "0xa7b9123f4b15fE0fF01F469ff5Eab2b41296dC0E";// TombFtmLpTombRewardPool
const TOMB_REWARD_POOL_OLD_ADDR = "0xA7B16703470055881e7EE093e9b0bF537f29CD4d";// old
const GENESIS_TOMB_REWARD_POOL_ADDR = "0x9D6f186223F6dbD3eBd265b98D82B0c6A5A3e41F";
const TSHARE_REWARD_POOL_ADDR = "0x7b4C5f9Ed34519cBB99C833389582Dc3D4Fe1f1e";
const TOMB_ADDR = "0xA87bcE04d4a396DF40cC1be3a21CF04592C08290";
...
async function main() {
  ...
  const zombiRewardPoolGenerisContract = new ethers.Contract(GENESIS_TOMB_REWARD_POOL_ADDR, ZOMBI_REWARD_GENERIS_POOL_ABI, App.provider);
  const tokens = {};
  ...
  const startTime2 = await zombiRewardPoolGenerisContract.poolStartTime();
  const currentTime = Date.now() / 1000

  const MASONRY_ADDR = "0xBEe596786Db8C602661284e89D493B05b60E12Fc";
  const ORACLE_ADDR = "0x655e7223198D81421282766a9067997783b86D68";
  const rewardTokenAddress = "0xA87bcE04d4a396DF40cC1be3a21CF04592C08290"; //TOMB
  const lptAddress = "0xEe17f36CdcA299eE7705413AB6111a8fe1B2dF17"; //TSHARE-FTM-LP
  const stakeTicker = "SZOMB2";
  const rewardTicker = "ZOMB2";
  ...
  let zombiRewardPool = await loadRewardPoolContract(App, tokens, prices, zombiRewardPoolGenerisContract, GENESIS_TOMB_REWARD_POOL_ADDR, ZOMBI_REWARD_GENERIS_POOL_ABI, "ZOMB2",
    "tomb", "pendingTOMB", 4, startTime2, currentTime);

  _print_bold(`Total Staked: $${formatMoney(zombiRewardPool.totalStaked)}`);
  ...
  let tShareRewardPool = await loadRewardPoolContract(App, tokens, prices, tShareRewardPoolContract, TSHARE_REWARD_POOL_ADDR, TSHARE_REWARD_POOL_ABI, "SZOMB2",
  ...
  // _print("")
  // let tombRewardPool = await loadRewardPoolContract(App, tokens, prices, tombRewardPoolContract, TOMB_REWARD_POOL_ADDR, TOMB_REWARD_POOL_ABI, "ZOMB2",
...
async function getTokenRewardPerSecond(poolContract) {
  if (poolContract.address == TOMB_REWARD_POOL_ADDR) {
    ...
    if (poolContract.address == GENESIS_TOMB_REWARD_POOL_ADDR)
      return await poolContract.tombPerSecond()
