...
if(fireAirdropAmountUi > 0){
  ...
  _print(`Currently staking ${usersStakedAmount.toFixed(4)} FIRE $${((usersStakedAmount) * firePrice).toFixed(4)} (to receive multiplied Multiplied 3x = ${(usersStakedAmount * multiplier).toFixed(4)})\n`);
  ...
  if(Date.now() / 1000 > endTime){
    _print_link(`Unstake ${(usersStakedAmount * multiplier).toFixed(2)} and claim your FIRE (Multiplied 3x = ${(usersStakedAmount * multiplier)})`, unstake);
  }else{
    _print(`Withdraw your FIRE after ${endDate}`);
  }
}else{
  _print("My brother in flames, you did not receive Airdrop 0 and are ineligible for this contract. Stay tuned for information on depositing for LP rewards. Hold onto your flame imperishable and follow in the telegram for updates. https://t.me/promethios_main ");
}
}
