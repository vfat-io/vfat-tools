...
async function main() {
  ...
  let p0 = await loadPopSynthetixPool(App, tokens, prices, Pool0.abi, Pool0.address,
    Pool0.rewardTokenFunction, Pool0.stakeTokenFunction)
  let p1 = await loadPopSynthetixPool(App, tokens, prices, Pool1.abi, Pool1.address,
  ...
  if (p0.totalUserStaked > 0 || p1.totalUserStaked > 0) {
    _print(`You are staking a total of $${formatMoney(p0.totalUserStaked + p1.totalUserStaked)} at an APR of ${(p0.totalAPR + p1.totalAPR * 100).toFixed(2)}%\n`);
  }
  
  hideLoading();
}

async function loadPopSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadPopSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  return await printSynthetixPool(App, info);
}

async function loadPopSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
  rewardTokenFunction, stakeTokenFunction) {
    ...
    var newPrices = rewardToken.tokens.filter(x =>
      !getParameterCaseInsensitive(prices, x));
    var newPrice = await lookUpTokenPrices(newPrices);
    for (const key in newPrice) {
      if (newPrice[key]?.usd)
          prices[key] = newPrice[key];
    }
