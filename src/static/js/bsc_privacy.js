...
async function main() {
   ...
   const PRV_CHEF_ADDR = "0xD8151Da76095264929ab7cB680457c18014D2f9d";
   const rewardTokenTicker = "PRV";
   const PRV_CHEF = new ethers.Contract(PRV_CHEF_ADDR, PRV_CHEF_ABI, App.provider);

   const rewardsPerWeek = await PRV_CHEF.prvPerBlock() /1e18
        * 604800 / 3;
...
async function loadPrvContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getPrvInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printPrvPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
async function getPrvInfo(App, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
function printPrvPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printPrvContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fee, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printPrvContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
