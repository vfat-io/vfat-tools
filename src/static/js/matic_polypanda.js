...
async function main() {
   ...
   if(currentBlock < startBlock){
    ...
    await loadPolypandaContract(App, tokens, prices, BAMBOO_CHEF, BAMBOO_CHEF_ADDR, BAMBOO_CHEF_ABI, rewardTokenTicker,
        "bamboo", null, rewardsPerWeek, "pendingBamboo", [10]);

    hideLoading();
  }

  async function loadPolypandaContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getPolypandaPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printPolypandaPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
async function getPolypandaPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
function printPolypandaPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printPolypandaContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fee, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printPolypandaContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
