...
async function main() {
   ...
   const STG_CHEF_ADDR = "0x8731d54E9D02c286767d56ac03e8037C07e01e98";
   const rewardTokenTicker = "STG";
   const STG_CHEF = new ethers.Contract(STG_CHEF_ADDR, STG_CHEF_ABI, App.provider);
   const currentBlock = await App.provider.getBlockNumber();
   const multiplier = await STG_CHEF.getMultiplier(currentBlock, currentBlock + 1);
   const rewardsPerWeek = await STG_CHEF.stargatePerBlock() /1e18
      * 604800 * multiplier / 2.1;
    const tokens = {};
    const prices = await getMaticPrices();
    prices["0x29e38769f23701A2e4A8Ef0492e19dA4604Be62c"] = { usd : 1 };
    prices["0x1205f31718499dBf1fCa446663B532Ef87481fe1"] = { usd : 1 };

    await loadStargateChefContract(App, tokens, prices, STG_CHEF, STG_CHEF_ADDR, STG_CHEF_ABI, rewardTokenTicker,
        "stargate", null, rewardsPerWeek, "pendingStargate");

    hideLoading();
  }

async function getStargatePoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
async function loadStargateChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getStargatePoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
