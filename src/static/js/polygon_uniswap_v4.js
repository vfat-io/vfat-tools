...
// Uniswap v4 Polygon deployments (137): https://docs.uniswap.org/contracts/v4/deployments
const nft_manager_address_v4 = '0x1ec2ebf4f37e7363fdfe3551602425af0b3ceef9'
const pool_manager_address_v4 = '0x67366782805870060151383f4bbff9dab53e5cd6'
// Reading current tick/sqrtPrice is done via StateView, not PoolManager.
const state_view_address_v4 = '0x5ea1bd7974c8a611cbab0bdcafcb1d9cc9b3ba5a'
const sickle_factory_address = '0xAc371D6E651b6450ea8c4cE346Ddd44B62d851B5'
const sweep_address = '0xa7C8eCC21BcD023EB1dB5Bb35113b7123cb1F33b'
...
async function getErc20MetaCached(App, cache, tokenAddress, chainId) {
  return uniswapV4Helpers().getErc20MetaCached(App, cache, tokenAddress, chainId, { nativeSymbol: 'MATIC', nativeDecimals: 18 })
}
...
const withdraw_nfts = async function(App, nft_manager_v4, nft_manager_address_v4, sickleAddress) {
  ...
  for (const p of positions) {
    ...
    const token0Meta = await getErc20MetaCached(App, tokenMetaCache, p.poolKey.currency0, POLYGON_CHAIN_ID)
    const token1Meta = await getErc20MetaCached(App, tokenMetaCache, p.poolKey.currency1, POLYGON_CHAIN_ID)
  ...
  for (const [poolId32, infos] of groups.entries()) {
    ...
    for (const [idx, info] of infos.entries()) {
      ...
      const exitToMatic = async function() {
      ...
      if(window.Sickle){
        ...
        idx < infos.length - 1 ? _print_inline(`|    `) : _print_inline(`     `)
        _print_link(`Exit to MATIC`, exitToMatic)
...
const fallback_withdraw = async function(App, nft_manager_address_v4, sickleAddress) {
  ...
  _print(
    `<a target="_blank" href="https://polygonscan.com/token/${nft_manager_address_v4}?a=${sickleAddress}#inventory">Nft Position Manager</a>`
  )
  _print('')
