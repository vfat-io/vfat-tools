...
async function main() {
   ...
   const BENIS_ADDR = "0xefa4aED9Cf41A8A0FcdA4e88EfA2F60675bAeC9F";
   const rewardTokenTicker = "wBAN";
   const BENIS = new ethers.Contract(BENIS_ADDR, BENIS_ABI, App.provider);

   const startBlock = 18820405;//await BENIS.startTime();
   ...
   if(currentBlock < startBlock){
    ...
    await loadMaticBenisContract(App, tokens, prices, BENIS, BENIS_ADDR, BENIS_ABI, rewardTokenTicker,"wban", null, rewardsPerWeek, "pendingWBAN");

    hideLoading();
  }

  
async function loadMaticBenisContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getMaticBenisPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
...
async function getMaticBenisPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {  
