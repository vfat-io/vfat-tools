$(function () {		
  ...
  async function main() {		
      ...
      const StakingPool = {
        address : "0xe8ea0828ff7bf03c868a2370b83bc06f50d4eed9",
        abi : _2DOGE_STAKING_ABI,
        stakeTokenFunction : "share",
        rewardTokenAddresses : "tomb"
      }
      
      const _2SDOGE_CHEF_ADDR = "0x60a92645fe34ce7c16f72986e0f980297152535a";
      const rewardTokenTicker = "2SDOGE";
      const _2SDOGE_CHEF = new ethers.Contract(_2SDOGE_CHEF_ADDR, _2SDOGE_CHEF_ABI, App.provider);

      const rewardsPerWeek = await _2SDOGE_CHEF.tSharePerSecond() / 1e18 * 604800;
          
      const tokens = {};
      const prices = await getFantomPrices();
      
      await loadFantom2SDogeContract(App, tokens, prices, _2SDOGE_CHEF, _2SDOGE_CHEF_ADDR, _2SDOGE_CHEF_ABI, rewardTokenTicker,		
          "tshare", null, rewardsPerWeek, "pendingShare");
      _print(`\nStaked 2SDOGEs can only be withdrawn after 6 epochs since deposit.\n`);
      const p0 = await loadFantom2DOGESynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                            StakingPool.address, 
...
async function loadFantom2DOGESynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadFantom2DOGESynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await print2DOGESynthetixPool(App, info, "fantom");
}

async function loadFantom2DOGESynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function print2DOGESynthetixPool(App, info, chain="eth", customURLs) {
...
async function loadFantom2SDogeContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
