...
async function main() {
    ...
    let p = await loadMultipleMeritPools(App, tokens, prices, Pools)
...
async function loadMultipleMeritPools(App, tokens, prices, pools) {
  let totalStaked  = 0, totalUserStaked = 0, individualAPRs = [];
  const infos = await Promise.all(pools.map(p =>
    loadMeritPoolInfo(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction)));
...
async function loadMeritPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
  rewardTokenFunction, stakeTokenFunction) {
    ...
    const calls = [STAKING_MULTI.balanceOf(App.YOUR_ADDRESS), STAKING_MULTI.withdrawableRewardsOf(App.YOUR_ADDRESS)]
