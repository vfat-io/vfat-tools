$(function () {		
    ...
    async function main() {		
        ...
        const StakingPool = {
          address : "0xcd7af5947037336a20fe5877899e13a60a0a01b5",
          abi : SERENE_STAKING_ABI,
          stakeTokenFunction : "share",
          rewardTokenAddresses : "peg"
        }
        
        const PBL_CHEF_ADDR = "0x99121ce549c631cbf81e4769fcc737fc8c053e5e";
        ...
        await loadFantomPBLContract(App, tokens, prices, PBL_CHEF, PBL_CHEF_ADDR, PBL_CHEF_ABI, rewardTokenTicker,		
            "gov", null, rewardsPerWeek, "pendingShare");
        _print("");
        _print("Staked PBLs can only be withdrawn 48 hours after deposit.\n")
        const p0 = await loadFantomSERENESynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                              StakingPool.address, 
  ...
  async function loadFantomSERENESynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
    const info = await loadFantomSERENESynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
    if (!info) return null;
    return await printSERENESynthetixPool(App, info, "fantom");
  }
  
  async function loadFantomSERENESynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
  ...
  async function printSERENESynthetixPool(App, info, chain="eth", customURLs) {
  ...
  async function loadFantomPBLContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
    ...
    const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
      await getPblPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
