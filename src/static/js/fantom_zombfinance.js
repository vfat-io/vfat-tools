$(function () {		
  ...
  async function main() {		
      ...
      const StakingPool = {
        address : "0x7f805209F8FaF499a0623C0Bf25f582188cA73E0",
        abi : ZOMB_STAKING_ABI,
        stakeTokenFunction : "share",
        rewardTokenAddresses : "zomb"
      }
      
      const ZSHARE_CHEF_ADDR = "0x724783fde5c8d0caf60f780f4a45430da4d72544";
      const rewardTokenTicker = "ZSHARE";
      const ZSHARE_CHEF = new ethers.Contract(ZSHARE_CHEF_ADDR, ZSHARE_CHEF_ABI, App.provider);

      const rewardsPerWeek = await ZSHARE_CHEF.zSharePerSecond() / 1e18 * 604800;
          
      const tokens = {};
      const prices = await getFantomPrices();
      
      await loadFantomZshareContract(App, tokens, prices, ZSHARE_CHEF, ZSHARE_CHEF_ADDR, ZSHARE_CHEF_ABI, rewardTokenTicker,		
          "zShare", null, rewardsPerWeek, "pendingShare");
      _print("");
      _print("Staked zSHAREs can only be withdrawn after 6 epochs since deposit.\n")
      const p0 = await loadFantomZOMBSynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                            StakingPool.address, 
...
async function loadFantomZOMBSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadFantomZOMBSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printZOMBSynthetixPool(App, info, "fantom");
}

async function loadFantomZOMBSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printZOMBSynthetixPool(App, info, chain="eth", customURLs) {
...
async function loadFantomZshareContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
