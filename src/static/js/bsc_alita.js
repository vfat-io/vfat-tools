...
async function main() {
   ...
   const ALI_CHEF_ADDR = "0x4f7b2Be2bc3C61009e9aE520CCfc830612A10694";
   const rewardTokenTicker = "ALI";
   const ALI_CHEF = new ethers.Contract(ALI_CHEF_ADDR, ALI_CHEF_ABI, App.provider);

   const startBlock = await ALI_CHEF.startBlock();
   ...
   if(currentBlock < startBlock){
    ...
    periodIndex = await ALI_CHEF.getPeriodIndexByBlockNumber(currentBlock);
    rewardsPerWeek = await ALI_CHEF.getRewardPerBlock(periodIndex) /1e18
        * 604800 / 3;
   }
    ...
    await loadAliChefContract(App, tokens, prices, ALI_CHEF, ALI_CHEF_ADDR, ALI_CHEF_ABI, rewardTokenTicker,
        "ali", null, rewardsPerWeek, "pendingALI", [1]);

    hideLoading();
  }

  async function loadAliChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getAliPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
...
async function getAliPoolInfo(App, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
