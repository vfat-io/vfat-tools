...
async function main() {  
 ...
 const rewardTokenTicker = "SOUPS";
 const SOUPS_CHEF = new ethers.Contract(SOUPS_CHEF_ADDR, SOUPS_CHEF_ABI, App.provider);

 const rewardsPerWeek = await SOUPS_CHEF.soupsPerBlock() /1e18
      * 604800 / 3;

  const tokens = {};
  const prices = await getBscPrices();

  await loadSoupChefContract(App, tokens, prices, SOUPS_CHEF, SOUPS_CHEF_ADDR, SOUPS_CHEF_ABI, rewardTokenTicker,
      "soups", null, rewardsPerWeek, "pendingRewards");
