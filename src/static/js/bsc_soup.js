...
async function main() {  
 ...
 const rewardTokenTicker = "SOUP";
 const SOUP_CHEF = new ethers.Contract(SOUP_CHEF_ADDR, SOUP_CHEF_ABI, App.provider);

 const rewardsPerWeek = await SOUP_CHEF.epochSoupPerBlock(0) /1e18
      * 604800 / 3;

  const tokens = {};
  const prices = await getBscPrices();

  await loadSoupChefContract(App, tokens, prices, SOUP_CHEF, SOUP_CHEF_ADDR, SOUP_CHEF_ABI, rewardTokenTicker,
      "soup", null, rewardsPerWeek, "pendingRewards");
