...
// Uniswap v4 Ethereum deployments (1): https://docs.uniswap.org/contracts/v4/deployments
const pool_manager_address_v4 = '0x000000000004444c5dc75cB358380D2e3dE08A90'
// Reading current tick/sqrtPrice is done via StateView, not PoolManager.
const state_view_address_v4 = '0x7ffe42c4a5deea5b0fec41c94c136cf115597227'
...
const withdraw_nfts = async function(App, nft_manager_v4, nft_manager_address_v4, sickleAddress) {
  ...
  for (const p of positions) {
    ...
    const token0Meta = await getErc20MetaCached(App, tokenMetaCache, p.poolKey.currency0, MAINNET_CHAIN_ID)
    const token1Meta = await getErc20MetaCached(App, tokenMetaCache, p.poolKey.currency1, MAINNET_CHAIN_ID)
...
const fallback_withdraw = async function(App, nft_manager_address_v4, sickleAddress) {
  ...
  _print(
    `<a target="_blank" href="https://etherscan.io/token/${nft_manager_address_v4}?a=${sickleAddress}#inventory">Nft Position Manager</a>`
  )
  _print('')
