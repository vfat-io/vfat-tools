...
async function main() {
   ...
   if(currentBlock < startBlock){
    ...
    await loadRaptorContract(App, tokens, prices, RAPTOR_CHEF, RAPTOR_CHEF_ADDR, RAPTOR_CHEF_ABI, rewardTokenTicker,
        "raptor", null, rewardsPerWeek, "pendingRaptor", [1]);

    hideLoading();
  }

async function loadRaptorContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getRaptorPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printRaptorPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
async function getRaptorPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
function printRaptorPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printRaptorContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fee, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printRaptorContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
