...
async function main() {
  ...
  const MAIA_STAKING_ADDR = "0x879828da3a678D349A3C8d6B3D9C78e9Ee31137F"
  const MAIA_STAKING_CONTRACT = new ethers.Contract(MAIA_STAKING_ADDR, MAIA_STAKING_ABI, App.provider);
  //const MAIA_TOKEN_ADDR = "0x72c232D56542Ba082592DEE7C77b1C6CFA758BCD"  //decimals 9
  const HERMES_TOKEN_ADDR = "0xb27BbeaACA2C00d6258C3118BAB6b5B6975161c8"  //hermes token for rewards???
  ...
  for(const lpToken of lpTokens){
    _gauges.push(await MAIA_STAKING_CONTRACT.gauges(lpToken))
  }
  ...
  const claimAll = async function() {
    ...
    return maiaContract_claimAll(p.gaugeAddresses, rewardTokenAddresses, MAIA_STAKING_ABI, App)
  }
...
const maiaContract_claimAll = async function(gaugeAddresses, rewardTokenAddresses, rewardPoolAddr, App) {
  const signer = App.provider.getSigner()

  const REWARD_POOL = new ethers.Contract(rewardPoolAddr, MAIA_STAKING_ABI, signer)
