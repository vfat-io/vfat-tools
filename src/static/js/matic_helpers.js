...
async function getCMaticToken(App, cToken, address, stakingAddress) {
  const calls = [
    cToken.decimals(),
    cToken.underlying(),
    cToken.totalSupply(),
  ...
  const token = await getMaticToken(App, underlying, address)
  return {
    address,
    name,
    symbol,
...
async function getMaticBhToken(App, vault, address, stakingAddress) {
  const decimals = await vault.decimals();
...
async function getMaticStoredToken(App, tokenAddress, stakingAddress, type) {
  switch (type) {
    ...
    case "bhToken":
      const bhToken = new ethers.Contract(tokenAddress, MATIC_BH_TOKEN_ABI, App.provider);
      return await getMaticBhToken(App, bhToken, tokenAddress, stakingAddress);
    ...
    case 'cToken':
      const cMaticToken = new ethcall.Contract(tokenAddress, CTOKEN_ABI)
      return await getCMaticToken(App, cMaticToken, tokenAddress, stakingAddress)
    case "erc20":
...
async function getMaticToken(App, tokenAddress, stakingAddress) {
    ...
    try {
      const BH = new ethers.Contract(tokenAddress, MATIC_BH_TOKEN_ABI, App.provider);
      const _token = await BH.LUSD();
      const bhToken = await getMaticBhToken(App, BH, tokenAddress, stakingAddress);
    ...
    try {
      ...
      const res = await getCMaticToken(App, cToken, tokenAddress, stakingAddress)
...
async function getMaticPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  ...
  const poolToken = await getMaticToken(app, poolInfo.lpToken ?? poolInfo.stakingToken ?? poolInfo.token, chefAddress);
