...
$(function() {
  ...
  const ZKSMASTER_ABI = [
    ...
    },
    {
      ...
      "name": "getDaysSinceDeposit",
      "outputs": [
        {
          "internalType": "uint256",
          "name": "daysSinceDeposit",
          "type": "uint256"
        }
      ],
      "stateMutability": "view",
      ...
      "inputs": [
        {
          ...
          "internalType": "struct zksMaster.nftDataIn[]",
          "name": "nftData",
          "type": "tuple[]"
        }
      ],
  ...
  async function main() {
     ...
     const ZKSMASTER_ADDR = "0x7bA76d4e4cBD4A9B7E3fd9a3B7Db067a51ca9682";
  
     const rewardTokenTicker = "ZKS";
     const ZKSMASTER = new ethers.Contract(ZKSMASTER_ADDR, ZKSMASTER_ABI, App.provider);
  
     let rewardsPerWeek = 0
     const farmStartTime = await ZKSMASTER.startTimestamp() * 1000;
    const startTime = new Date(farmStartTime);
     const currentTime = Date.now();
  
     if(currentTime < farmStartTime){
      _print_bold(`Rewards will be started at ${startTime} \n`);
     }else{
      rewardsPerWeek = await ZKSMASTER.rewardPerSecond() /1e18  * 604800;
     }
      const tokens = {};
