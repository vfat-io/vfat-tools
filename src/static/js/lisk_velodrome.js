...
const NFT_FARM_STRATEGY_ADDRESS = "0xde181C7412162eeF86174D9F0dCB299B4C2284f4";

const FARM_STRATEGY_ADDRESS = "0xc3fbe11b1b10E2b770358d0078E318807fd5C069";
...
async function main() {
  ...
  const tokens = {};
  try {
    prices = await getLiskPrices();
  } catch (e) {
    prices = await getLiskPrices2();
  }
...
async function printVelodromeClPool(App, info, chain = "eth", customURLs) {
  ...
  _print(`<a target="_blank" href="https://blockscout.lisk.com/address/${info.stakingAddress}">Lisk Scan</a>`);
...
async function loadFlowSynthetixPools(App, tokens, prices, pools, has_sickle_account, customURLs) {
  ...
  for (const i of infos.filter(i => i?.poolPrices)) {
    let p = await printVelodromePool(App, i, "lisk", customURLs);
...
async function loadFlowSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
  ...
  const poolPrices = getPoolPrices(tokens, prices, stakeToken, "lisk");
...
async function printVelodromePool(App, info, chain = "eth", customURLs) {
  ...
  _print(`<a target="_blank" href="https://blockscout.lisk.com/address/${info.stakingAddress}#code">Lisk Scan</a>`);
  if (info.has_sickle_account) {
...
async function getLiskPrices2() {
  const OptTokenContracts = LiskTokens.map(x => x.contract.toLowerCase())
...
const lookUpPrices2 = async function (id_array) {
  ...
  let res = await $.ajax({
    url: 'https://api.vfat.io/v1/token?chainId=1135&address=' + ids,
    type: 'GET',
  })
