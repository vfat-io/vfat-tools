...
const CUTEY_CHEF_ABI = [
  {
    inputs: [
      {internalType: 'contract CuteyToken', name: '_cute', type: 'address'},
      {internalType: 'address', name: '_devaddr', type: 'address'},
      {internalType: 'address', name: '_feeAddress', type: 'address'},
      {internalType: 'uint256', name: '_cutePerBlock', type: 'uint256'},
  ...
  },
  {
    inputs: [],
    name: 'cute',
    outputs: [{internalType: 'contract CuteyToken', name: '', type: 'address'}],
    stateMutability: 'view',
    type: 'function',
  },
  {
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accCutePerShare', type: 'uint256'},
  ...
  {
    inputs: [{internalType: 'uint256', name: '_cutePerBlock', type: 'uint256'}],
    name: 'updateEmissionRate',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
...
async function main() {
  ...
  const CUTEY_CHEF_ADDR = '0x742e95124e65b4115a44ABbEE64B1Ee0e082A785'
  const rewardTokenTicker = 'CUTE'
  const CUTEY_CHEF = new ethers.Contract(CUTEY_CHEF_ADDR, CUTEY_CHEF_ABI, App.provider)

  const rewardsPerWeek = (((await CUTEY_CHEF.cutePerBlock()) / 1e18) * 604800) / 2
  const tokens = {}
