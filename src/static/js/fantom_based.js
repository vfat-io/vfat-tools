...
const CRV3_ADDR = "0x58e57cA18B7A47112b877E31929798Cd3D703b0f";
...
async function main() {
  ...
  let bShareRewardPool = await loadRewardPoolContract(App, tokens, prices, bShareRewardPoolContract, BSHARE_REWARD_POOL_ADDR, BSHARE_REWARD_POOL_ABI, "BSHARE",
    "bshare", "pendingShare", 11, startTime, currentTime);
...
async function getCrv3CryptoPrice(App) {
  ...
  const btcPrice = await CRV3_SWAP_CONTRACT.price_oracle(0) / 1e18;
  const ethPrice = await CRV3_SWAP_CONTRACT.price_oracle(1) / 1e18;
  const virtualPrice = await CRV3_SWAP_CONTRACT.get_virtual_price() / 1e18;

  return 3 * virtualPrice * ((btcPrice * ethPrice) ** (1 / 3))
}
