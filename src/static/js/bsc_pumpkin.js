...
async function main() {
  ...
  let p = await loadPumpBscSynthetixPool(App, tokens, prices, Pools[0].abi, Pools[0].address,
  ...
  let p1 = await loadPumpBscSynthetixPool(App, tokens, prices, Pools[1].abi, Pools[1].address,
  ...
  let p2 = await loadPumpBscSynthetixPool(App, tokens, prices, Pools[2].abi, Pools[2].address,
    Pools[2].rewardTokenFunction, Pools[2].stakeTokenFunction)
...
async function loadPumpBscSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadPumpBscSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
...
async function loadPumpBscSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
  rewardTokenFunction, stakeTokenFunction) {
    ...
    const periodFinish = await STAKING_POOL.eraPeriod();//thelei allagi
