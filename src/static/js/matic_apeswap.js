...
async function main() {
   ...
   const BANANA_CHEF_ADDR = "0x54aff400858Dcac39797a81894D9920f16972D1D";
   const rewardTokenTicker = "BANANA";
   const BANANA_CHEF = new ethers.Contract(BANANA_CHEF_ADDR, BANANA_CHEF_ABI, App.provider);

   const rewardsPerWeek = await BANANA_CHEF.bananaPerSecond() /1e18
      * 604800;

    const tokens = {};
    const prices = await getMaticPrices();

    await loadMaticBananaContract(App, tokens, prices, BANANA_CHEF, BANANA_CHEF_ADDR, BANANA_CHEF_ABI, rewardTokenTicker,
        "BANANA", null, rewardsPerWeek, "pendingBanana");

    hideLoading();
  }

async function loadMaticBananaContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getMaticBananaPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
...
async function getMaticBananaPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {  
