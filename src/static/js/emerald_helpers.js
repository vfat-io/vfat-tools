...
async function getEmeraldUniPool(App, pool, poolAddress, stakingAddress) {
  const calls = [
...
async function getEmeraldErc20(App, token, address, stakingAddress) {
...
async function getEmeraldBasicVault(App, vault, address, stakingAddress) {
  ...
  const [ decimals, underlying, name, symbol, totalSupply, staked, unstaked, balance] =
    await App.ethcallProvider.all(calls);
  const token = await getEmeraldToken(App, underlying, address);
  return {
    address,
    name,
    symbol,
...
async function getEmeraldCurveToken(App, curve, address, stakingAddress, minterAddress) {
...
async function getEmeraldStableswapToken(App, stable, address, stakingAddress) {
...
async function getCEmeraldToken(App, cToken, address, stakingAddress) {
...
async function getEmeraldStoredToken(App, tokenAddress, stakingAddress, type) {
  switch (type) {
    case "curve":
      const crv = new ethcall.Contract(tokenAddress, CURVE_ABI);
      const [minter] = await App.ethcallProvider.all([crv.minter()]);
      return await getEmeraldCurveToken(App, crv, tokenAddress, stakingAddress, minter);
    case "stableswap":
      const stable = new ethcall.Contract(tokenAddress, STABLESWAP_ABI);
      return await getEmeraldStableswapToken(App, stable, tokenAddress, stakingAddress);
    case "cToken":
      const cEmeraldToken = new ethcall.Contract(tokenAddress, CTOKEN_ABI);
      return await getCEmeraldToken(App, cEmeraldToken, tokenAddress, stakingAddress);
    case "uniswap":
      const pool = new ethcall.Contract(tokenAddress, UNI_ABI);
      return await getEmeraldUniPool(App, pool, tokenAddress, stakingAddress);
    case "basicEmeraldVault":
      const basicEmeraldVault = new ethcall.Contract(tokenAddress, HARVEST_VAULT_ABI);
      return await getEmeraldBasicVault(App, basicEmeraldVault, tokenAddress, stakingAddress);
    case "erc20":
      const erc20 = new ethcall.Contract(tokenAddress, ERC20_ABI);
      return await getEmeraldErc20(App, erc20, tokenAddress, stakingAddress);
  }
}

async function getEmeraldToken(App, tokenAddress, stakingAddress) {
    if (tokenAddress == "0x0000000000000000000000000000000000000000") {
      return getEmeraldErc20(App, null, tokenAddress, "")
    }
    const type = window.localStorage.getItem(tokenAddress);
    if (type) return getEmeraldStoredToken(App, tokenAddress, stakingAddress, type);
    try {
      const crv = new ethcall.Contract(tokenAddress, CURVE_ABI);
      const [minter] = await App.ethcallProvider.all([crv.minter()]);
      const res = await getEmeraldCurveToken(App, crv, tokenAddress, stakingAddress, minter);
    ...
    try {
      ...
      const uniPool = await getEmeraldUniPool(App, pool, tokenAddress, stakingAddress);
    ...
    try {
      ...
      const res = await getCEmeraldToken(App, cToken, tokenAddress, stakingAddress);
    ...
    try {
      ...
      const res = await getEmeraldBasicVault(App, basicVault, tokenAddress, stakingAddress);
      window.localStorage.setItem(tokenAddress, "basicEmeraldVault");
      return res;
    }
    catch(err) {
    }
    try {
      ...
      const erc20tok = await getEmeraldErc20(App, erc20, tokenAddress, stakingAddress);
...
async function loadEmeraldSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
    rewardTokenFunction, stakeTokenFunction) {
      ...
      var stakeToken = await getEmeraldToken(App, stakeTokenAddress, stakingAddress);
      ...
      for (const address of newTokenAddresses) {
          tokens[address] = await getEmeraldToken(App, address, stakingAddress);
      }
      if (!getParameterCaseInsensitive(tokens, rewardTokenAddress)) {
          tokens[rewardTokenAddress] = await getEmeraldToken(App, rewardTokenAddress, stakingAddress);
      }
      const rewardToken = getParameterCaseInsensitive(tokens, rewardTokenAddress);

      const rewardTokenTicker = rewardToken.symbol;

      const poolPrices = getPoolPrices(tokens, prices, stakeToken, "emerald");
...
async function loadEmeraldSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
    const info = await loadEmeraldSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
    if (!info) return null;
    return await printSynthetixPool(App, info, "emerald");
}

async function loadEmeraldBasisFork(data) {
...
async function getEmeraldPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
async function loadMultipleEmeraldSynthetixPools(App, tokens, prices, pools) {
  let totalStaked  = 0, totalUserStaked = 0, individualAPRs = [];
  const infos = await Promise.all(pools.map(p =>
      loadEmeraldSynthetixPoolInfo(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction)));
  for (const i of infos) {
    let p = await printSynthetixPool(App, i, "emerald");
...
async function loadMultipleEmeraldSynthetixPoolsSequential(App, tokens, prices, pools) {
  let totalStaked  = 0, totalUserStaked = 0, individualAPRs = [];
  for (const p of pools) {
    let res = await loadEmeraldSynthetixPool(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction);
    if (!res) continue;
