$(function () {		
  ...
  async function main() {		
      ...
      const StakingPool = {
        address : "0x57d0e9716317cfb169b5820ba694989baa747ef7",
        abi : HAM_STAKING_ABI,
        stakeTokenFunction : "share",
        rewardTokenAddresses : "hamster"
      }
      
      const HSHARE_CHEF_ADDR = "0x9bbdcb4ca405c72e9159f263457676adec408b32";
      const GEM_CHEF_ADDR = "0x2d9c943a13b8e3f4b6c72c8e9ba656a42460cd0f";
      const rewardTokenTicker = "HSHARE";
      const rewardGEMTokenTicker = "GEM";
      const HSHARE_CHEF = new ethers.Contract(HSHARE_CHEF_ADDR, HSHARE_CHEF_ABI, App.provider);
      const GEM_CHEF = new ethers.Contract(GEM_CHEF_ADDR, HSHARE_CHEF_ABI, App.provider);

      const rewardsPerWeek = await HSHARE_CHEF.hSharePerSecond() / 1e18 * 604800;
      const rewardsGEMPerWeek = await GEM_CHEF.hSharePerSecond() / 1e18 * 604800;
          
      const tokens = {};
      const prices = await getFantomPrices();
      
      await loadFantomHshareContract(App, tokens, prices, HSHARE_CHEF, HSHARE_CHEF_ADDR, HSHARE_CHEF_ABI, rewardTokenTicker,		
          "hshare", null, rewardsPerWeek, "pendingShare");
      _print("");
      await loadFantomGEMContract(App, tokens, prices, GEM_CHEF, GEM_CHEF_ADDR, HSHARE_CHEF_ABI, rewardGEMTokenTicker,		
          "hshare", null, rewardsGEMPerWeek, "pendingShare");
      _print("");
      const p0 = await loadFantomHAMSynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                            StakingPool.address, 
...
async function loadFantomHAMSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadFantomHAMSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printHAMSynthetixPool(App, info, "fantom");
}

async function loadFantomHAMSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printHAMSynthetixPool(App, info, chain="eth", customURLs) {
...
async function loadFantomHshareContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
...
async function loadFantomGEMContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  rewardTokenFunction, rewardsPerBlockFunction, rewardsPerWeekFixed, pendingRewardsFunction,
  deathPoolIndices, claimFunction) {
  const chefContract = chef ?? new ethers.Contract(chefAddress, chefAbi, App.provider);
