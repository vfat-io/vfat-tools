...
async function main() {
   ...
   const INK_CHEF_ADDR = "0xf95ab2a261b0920f3d5abc02a41dbe125eba10ae";

   const rewardTokenTicker = "INK";
   const INK_CHEF = new ethers.Contract(INK_CHEF_ADDR, INK_CHEF_ABI, App.provider);

    const rewardsPerWeek = await INK_CHEF.inkPerSecond() /1e18 * 604800;

    const tokens = {};
    const prices = await getFantomPrices();

    await loadFantomPrinterChefContract(App, tokens, prices, INK_CHEF, INK_CHEF_ADDR, INK_CHEF_ABI, rewardTokenTicker,
      "ink", null, rewardsPerWeek, "pendingShare", [2]);

    hideLoading();
  }

async function loadFantomPrinterChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
