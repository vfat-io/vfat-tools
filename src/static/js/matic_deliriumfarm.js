...
async function main() {
  ...
  const startBlock = await DELIRIUM_CHEF.startBlock()
  const currentBlock = await App.provider.getBlockNumber()

  const multiplier = await DELIRIUM_CHEF.getMultiplier(currentBlock, currentBlock + 1)
  ...
  if (currentBlock < startBlock) {
    ...
    rewardsPerWeek = (((await DELIRIUM_CHEF.deliriumPerBlock()) / 1e18) * 604800 * multiplier) / 3
  }
