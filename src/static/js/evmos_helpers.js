...
async function getEvmos20(App, token, address, stakingAddress) {
...
async function getEvmosVault(App, vault, address, stakingAddress) {
  const decimals = await vault.decimals();
  const token_ = await vault.token();
  const token = await getEvmosToken(App, token_, address);
  return {
    address,
...
async function getEvmosSaddleToken(App, saddle, address, stakingAddress, swapAddress) {
  const swap = new ethers.Contract(swapAddress, EVMOS_SADDLE_SWAP_ABI, App.provider)
  const virtualPrice = await swap.getVirtualPrice();
  const coin0 = await swap.getToken(0);
  const token = await getEvmosToken(App, coin0, address);
...
async function getEvmosAdamantVault(App, vault, address, stakingAddress) {
  const token_ = await vault.token();
  const token = await getEvmosToken(App, token_, address);
  return {
    address,
...
async function getEvmosWantVault(App, vault, address, stakingAddress) {
  const decimals = await vault.decimals();
...
async function getCEvmosToken(App, cToken, address, stakingAddress) {
  const calls = [
    cToken.decimals(),
    cToken.underlying(),
    cToken.totalSupply(),
  ...
  const token = await getEvmosToken(App, underlying, address)
  return {
    address,
    name,
    symbol,
...
async function getEvmosBhToken(App, vault, address, stakingAddress) {
  const decimals = await vault.decimals();
...
async function getEvmosStoredToken(App, tokenAddress, stakingAddress, type) {
  switch (type) {
    case "uniswap":
      const pool = new ethers.Contract(tokenAddress, UNI_ABI, App.provider);
      return await getEvmosUniPool(App, pool, tokenAddress, stakingAddress);
    case "evmosVault":
      const vault = new ethers.Contract(tokenAddress, EVMOS_VAULT_TOKEN_ABI, App.provider);
      return await getEvmosVault(App, vault, tokenAddress, stakingAddress);
    case "bhToken":
      const bhToken = new ethers.Contract(tokenAddress, EVMOS_BH_TOKEN_ABI, App.provider);
      return await getEvmosBhToken(App, bhToken, tokenAddress, stakingAddress);
    case "evmosAdamantVault":
      const adamantVault = new ethers.Contract(tokenAddress, EVMOS_ADAMANT_VAULT_TOKEN_ABI, App.provider);
      return await getEvmosAdamantVault(App, adamantVault, tokenAddress, stakingAddress);
    case "evmosWantVault":
      const wantVault = new ethers.Contract(tokenAddress, EVMOS_VAULT_WANT_ABI, App.provider);
      return await getEvmosWantVault(App, wantVault, tokenAddress, stakingAddress);
    case "curve":
      const crv = new ethers.Contract(tokenAddress, EVMOS_CURVE_ABI, App.provider);
      if (tokenAddress.toLowerCase() == "0x88E11412BB21d137C217fd8b73982Dc0ED3665d7".toLowerCase()) {
        ...
        return await getCurveEvmosToken(App, crv, tokenAddress, stakingAddress, minter);
      }
      const minter = await crv.minter();
      return await getCurveEvmosToken(App, crv, tokenAddress, stakingAddress, minter);
    case "evmosSaddle":
      const saddle = new ethers.Contract(tokenAddress, EVMOS_SADDLE_LP_TOKEN_ABI, App.provider);
      const swap = await saddle.swap();
      return await getEvmosSaddleToken(App, saddle, tokenAddress, stakingAddress, swap);
    case 'cToken':
      const cEvmosToken = new ethcall.Contract(tokenAddress, CTOKEN_ABI)
      return await getCEvmosToken(App, cEvmosToken, tokenAddress, stakingAddress)
    case "erc20":
      const erc20 = new ethers.Contract(tokenAddress, ERC20_ABI, App.provider);
      return await getEvmos20(App, erc20, tokenAddress, stakingAddress);
  }
}

async function getEvmosToken(App, tokenAddress, stakingAddress) {
    if (tokenAddress == "0x0000000000000000000000000000000000000000") {
      return getEvmos20(App, null, tokenAddress, "")
    }
    const type = window.localStorage.getItem(tokenAddress);
    if (type) return getEvmosStoredToken(App, tokenAddress, stakingAddress, type);
    try {
      const pool = new ethers.Contract(tokenAddress, UNI_ABI, App.provider);
      const _token0 = await pool.token0();
      const uniPool = await getEvmosUniPool(App, pool, tokenAddress, stakingAddress);
    ...
    try {
      ...
      if (tokenAddress.toLowerCase() == "0x0a2ad1e60bcf5f812a2c74ec519822da36f86617".toLowerCase()) {
        ...
        const res = await getCurveEvmosToken(App, crv, tokenAddress, stakingAddress, minter);
    ...
    try {
      ...
      const vault = await getEvmosVault(App, VAULT, tokenAddress, stakingAddress);
      window.localStorage.setItem(tokenAddress, "evmosVault");
      return vault;
    }
    catch(err) {
    }
    try {
      const BH = new ethers.Contract(tokenAddress, EVMOS_BH_TOKEN_ABI, App.provider);
      const _token = await BH.LUSD();
      const bhToken = await getEvmosBhToken(App, BH, tokenAddress, stakingAddress);
    ...
    try {
      ...
      const res = await getCEvmosToken(App, cToken, tokenAddress, stakingAddress)
      window.localStorage.setItem(tokenAddress, 'cToken')
      return res
    } catch (err) {}
    try {
      const ADAMANT_VAULT = new ethers.Contract(tokenAddress, EVMOS_ADAMANT_VAULT_TOKEN_ABI, App.provider);
      const _totalShares = await ADAMANT_VAULT.totalShares();
      const vault = await getEvmosAdamantVault(App, ADAMANT_VAULT, tokenAddress, stakingAddress);
      window.localStorage.setItem(tokenAddress, "evmosAdamantVault");
      return vault;
    }
    catch(err) {
    }
    try {
      const WANT_VAULT = new ethers.Contract(tokenAddress, EVMOS_VAULT_WANT_ABI, App.provider);
      const _want = await await WANT_VAULT.want();
      const wantVault = await getEvmosWantVault(App, WANT_VAULT, tokenAddress, stakingAddress);
      window.localStorage.setItem(tokenAddress, "evmosWantVault");
      return wantVault;
    }
    catch(err) {
    }
    try {
      const saddle = new ethers.Contract(tokenAddress, EVMOS_SADDLE_LP_TOKEN_ABI, App.provider);
      const swap = await saddle.swap();
      const res = await getEvmosSaddleToken(App, saddle, tokenAddress, stakingAddress, swap);
      window.localStorage.setItem(tokenAddress, "evmosSaddle");
      return res;
    }
    catch(err) {
    }
    try {
      ...
      const erc20tok = await getEvmos20(App, erc20, tokenAddress, stakingAddress);
...
async function getCurveEvmosToken(app, curve, address, stakingAddress, minterAddress) {
...
async function loadEvmosSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
    rewardTokenFunction, stakeTokenFunction) {
      ...
      var stakeToken = await getEvmosToken(App, stakeTokenAddress, stakingAddress);
      ...
      for (const address of newTokenAddresses) {
          tokens[address] = await getEvmosToken(App, address, stakingAddress);
      }
      if (!getParameterCaseInsensitive(tokens, rewardTokenAddress)) {
          tokens[rewardTokenAddress] = await getEvmosToken(App, rewardTokenAddress, stakingAddress);
      }
      const rewardToken = getParameterCaseInsensitive(tokens, rewardTokenAddress);

      const rewardTokenTicker = rewardToken.symbol;

      const poolPrices = getPoolPrices(tokens, prices, stakeToken, "evmos");
...
async function loadEvmosSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
    const info = await loadEvmosSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
    if (!info) return null;
    return await printSynthetixPool(App, info, "evmos");
}

async function loadEvmosBasisFork(data) {
...
async function getEvmosPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  ...
  const poolToken = await getEvmosToken(app, poolInfo.lpToken ?? poolInfo.stakingToken ?? poolInfo.token, chefAddress);
...
async function loadEvmosChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getEvmosPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
...
async function loadMultipleEvmosSynthetixPools(App, tokens, prices, pools, customURLs) {
  ...
  const infos = await Promise.all(pools.map(p =>
      loadEvmosSynthetixPoolInfo(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction)));
  for (const i of infos.filter(i => i?.poolPrices)) {
    let p = await printSynthetixPool(App, i, "evmos", customURLs);
...
async function loadMultipleEvmosSynthetixPoolsSequential(App, tokens, prices, pools) {
  let totalStaked  = 0, totalUserStaked = 0, individualAPRs = [];
  for (const p of pools) {
    let res = await loadEvmosSynthetixPool(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction);
    if (!res) continue;
