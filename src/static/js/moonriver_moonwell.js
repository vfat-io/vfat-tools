...
const contracts = {
    ...
    gas: {
        p_address: '0x6a1A771C7826596652daDC9145fEAaE62b1cd07f',
        symbol: 'MOVR',
        decimals: 18,
    },
};
...
async function main() {
    ...
    const COMPTROLLER = new ethers.Contract(contracts.comptroller, MOONWELL_ABI.comptroller, App.provider);
...
async function loadData(App, token) {
    ...
    const PTOKEN_CONTRACT = new ethers.Contract(token, MOONWELL_ABI.pToken, App.provider);
    ...
    if (token.toLowerCase() === contracts.gas.p_address.toLowerCase()) {
        ...
        const UNDERLYING_CONTRACT = new ethers.Contract(underlying, MOONWELL_ABI.erc20, App.provider);
        underlyingSymbol = await UNDERLYING_CONTRACT.symbol();
        underlyingDecimals = await UNDERLYING_CONTRACT.decimals();
    }

    const ORACLE_CONTRACT = new ethers.Contract(contracts.oracle, MOONWELL_ABI.oracle, App.provider);
    ...
    //const RATE_CONTRACT = new ethers.Contract(interestRateModel, MOONWELL_ABI.rate, App.provider);
