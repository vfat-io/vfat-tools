$(function () {		
  ...
  async function main() {		
      ...
      const StakingPool = {
        address : "0x6b66a6ef7b52d45e82af66e252758163f062efa7",
        abi : YGG_STAKING_ABI,
        stakeTokenFunction : "share",
        rewardTokenAddresses : "tomb"
      }
      
      const YSHARE_CHEF_ADDR = "0x2bec9faff656f48897f82f822e1e4e68d8c1c730";
      const rewardTokenTicker = "YSHARE";
      const YSHARE_CHEF = new ethers.Contract(YSHARE_CHEF_ADDR, YSHARE_CHEF_ABI, App.provider);

      const rewardsPerWeek = await YSHARE_CHEF.tSharePerSecond() / 1e18 * 604800;
          
      const tokens = {};
      const prices = await getBscPrices();
      
      await loadBscYshareContract(App, tokens, prices, YSHARE_CHEF, YSHARE_CHEF_ADDR, YSHARE_CHEF_ABI, rewardTokenTicker,		
          "tshare", null, rewardsPerWeek, "pendingShare");
      _print(`\nStaked YSHARES can only be withdrawn after 6 epochs since deposit.\n`);
      const p0 = await loadBscYGGSynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                            StakingPool.address, 
...
async function loadBscYGGSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadBscYGGSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printYGGSynthetixPool(App, info, "bsc");
}

async function loadBscYGGSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printYGGSynthetixPool(App, info, chain="eth", customURLs) {
...
async function loadBscYshareContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
