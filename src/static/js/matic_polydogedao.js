$(function () {		
    ...
    async function main() {		
        ...
        const PDSHARE_CHEF_ADDR = "0x9682D175830643658798ac3367915e57bDdB506A";		
        const rewardTokenTicker = "PDSHARE";		
        const PDSHARE_CHEF = new ethers.Contract(PDSHARE_CHEF_ADDR, PDSHARE_CHEF_ABI, App.provider);		
    		
        const rewardsPerWeek = await PDSHARE_CHEF.tSharePerSecond() / 1e18 * 604800
    		
        const tokens = {};		
        const prices = await getMaticPrices();

        const StakingPool = {
          address : "0x83c3972eefb3326b5098c6dbec01dcc8988b2886",
          abi : PDDOLLAR_STAKING_ABI,
          stakeTokenFunction : "share",
          rewardTokenAddresses : "pddollar"
        }
...
async function loadFantomPDDOLLARSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
    const info = await loadFantomPDDOLLARSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
    if (!info) return null;
    return await printPDDOLLARSynthetixPool(App, info, "fantom");
  }

async function loadFantomPDDOLLARSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
  ...
  async function printPDDOLLARSynthetixPool(App, info, chain="eth", customURLs) {
...
async function getNachoxyzPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {  
...
async function loadNachoxyzContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getNachoxyzPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
