...
async function main() {  
   ...
   const LKM_CHEF_ADDR = "0x8705eaba437A2DEf65b0e455C025EEc05d1ee4aB";
   const rewardTokenTicker = "LKM";
   const LKM_CHEF = new ethers.Contract(LKM_CHEF_ADDR, LKM_CHEF_ABI, App.provider);

   const rewardsPerWeek = await LKM_CHEF.lkmPerBlock() /1e18
        * 604800 / 3;

    const tokens = {};
    const prices = await getBscPrices();

    await loadBscLokumContract(App, tokens, prices, LKM_CHEF, LKM_CHEF_ADDR, LKM_CHEF_ABI, rewardTokenTicker,
        "lokum", null, rewardsPerWeek, "pendingLkm");
