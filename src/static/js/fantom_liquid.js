...
async function main() {
   ...
   const LQDR_CHEF_ADDR = "0x742474dAE70Fa2AB063aB786b1fBe5704e861a0c";

   const rewardTokenTicker = "LQDR";
   const LQDR_CHEF = new ethers.Contract(LQDR_CHEF_ADDR, LQDR_CHEF_ABI, App.provider);

   let rewardsPerWeek = 0
   const startBlock = await LQDR_CHEF.startBlock();
   const currentBlock = await App.provider.getBlockNumber();

   if(currentBlock < startBlock){
     _print(`Rewards start at block ${startBlock}\n`);
   }else{
    rewardsPerWeek = await LQDR_CHEF.lqdrPerBlock() /1e18 * 604800;
   }

    const tokens = {};
    const prices = await getFantomPrices();

    await loadLiquidContract(App, tokens, prices, LQDR_CHEF, LQDR_CHEF_ADDR, LQDR_CHEF_ABI, rewardTokenTicker,
      "lqdr", null, rewardsPerWeek, "pendingLqdr");

    hideLoading();
  }

async function loadLiquidContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printLiquidPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
function printLiquidPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printLiquidContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fee, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printLiquidContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
