...
async function main() {
  ...
  const JEWEL_CHEF = new ethers.Contract(JEWEL_CHEF_ADDR, JEWEL_CHEF_ABI, App.provider);

  const blockNumber = await App.provider.getBlockNumber();
  const multiplier = await JEWEL_CHEF.getMultiplier(blockNumber, blockNumber+1);
  const rewardPerBlock = await JEWEL_CHEF.REWARD_PER_BLOCK();
  const rewardsPerWeek = rewardPerBlock / 1e18 * multiplier * 604800 / 2

  const tokens = {};
  const basePrices = await getHarmonyPrices();

  const { prices, totalUserStaked, totalStaked, averageApr } = await loadJewelChefContract(
    App,
    tokens,
    basePrices,
    JEWEL_CHEF,
  ...
  if (jewelPit.userBalanceUSD > 0) {
    _print(`You are staking a total of $${formatMoney(jewelPit.userBalanceUSD)} in the ${PIT_NAME} single staking pool.`);
  }

  hideLoading();
}

async function loadJewelChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
...
async function jewelPitData(JEWEL, JEWEL_PIT, App, prices) {
  const jewelPitTotalBalance = await JEWEL.balanceOf(JEWEL_PIT_ADDR) / 1e18;
  const jewelPitTotalSupply = await JEWEL_PIT.totalSupply() / 1e18;
  const xJewelRatio = jewelPitTotalBalance / jewelPitTotalSupply;

  const jewelPitUserBalanceBigNum = await JEWEL_PIT.balanceOf(App.YOUR_ADDRESS);
  const jewelPitUserBalance = (jewelPitUserBalanceBigNum / 1e18) * xJewelRatio;
  const jewelPitShare = jewelPitUserBalance / jewelPitTotalBalance;
  ...
  let jewelPitTotalBalanceUSD;
  let jewelPitUserBalanceUSD;

  if (jewelPrice && jewelPrice.usd) {
    jewelPitTotalBalanceUSD = jewelPitTotalBalance * jewelPrice.usd;
    jewelPitUserBalanceUSD = jewelPitUserBalance * jewelPrice.usd;
    ...
    const leave = async function() {
      return jewelPitLeave(JEWEL_PIT, jewelPitUserBalanceBigNum, App);
    }

    _print_link(`Stake ${(jewelBalance).toFixed(2)} ${REWARD_TOKEN_TICKER}`, approveAndEnter);

    if (jewelPitUserBalance > 0) {
      _print_link(`Unstake ${jewelPitUserBalance.toFixed(2)} ${REWARD_TOKEN_TICKER}`, leave);
    }
  ...
  return {
    totalBalance: jewelPitTotalBalance,
    totalBalanceUSD: jewelPitTotalBalanceUSD,
    userBalance: jewelPitUserBalance,
    userBalanceUSD: jewelPitUserBalanceUSD
  }
}

async function jewelPitApproveAndEnter(JEWEL, JEWEL_PIT, currentTokens, App) {
  const allowedTokens = await JEWEL.allowance(App.YOUR_ADDRESS, JEWEL_PIT_ADDR)
  let allow = Promise.resolve()

  if (allowedTokens / 1e18 < currentTokens / 1e18) {
    showLoading()
    allow = JEWEL.approve(JEWEL_PIT_ADDR, ethers.constants.MaxUint256)
    .then(function(t) {
      return App.provider.waitForTransaction(t.hash)
    })
    .catch(function() {
  ...
  if (currentTokens / 1e18 > 0) {
    ...
    allow
    .then(async function() {
      JEWEL_PIT.enter(currentTokens, {gasLimit: 500000})
      .then(function(t) {
        App.provider.waitForTransaction(t.hash).then(function() {
          hideLoading()
        })
...
async function jewelPitLeave(JEWEL_PIT, currentStakedAmount, App) {
  showLoading()
  JEWEL_PIT.leave(currentStakedAmount, {gasLimit: 500000})
  .then(function(t) {
    App.provider.waitForTransaction(t.hash).then(function() {
      hideLoading()
    })
