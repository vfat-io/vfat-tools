...
async function main() {
   ...
   const SADDLE_CHEF_ADDR = "0x2069043d7556B1207a505eb459D18d908DF29b55";
   const rewardTokenTicker = "SDL";
   const SADDLE_CHEF = new ethers.Contract(SADDLE_CHEF_ADDR, SADDLE_CHEF_ABI, App.provider);

   const rewardsPerWeek = await SADDLE_CHEF.saddlePerSecond() /1e18 * 604800;

    const tokens = {};
    const prices = await getArbitrumPrices();

    prices["0xc969dD0A7AB0F8a0C5A69C0839dB39b6C928bC08"] = { usd : 1 };

    await loadSaddleChefContract(App, tokens, prices, SADDLE_CHEF, SADDLE_CHEF_ADDR, SADDLE_CHEF_ABI, rewardTokenTicker,
        "SADDLE", null, rewardsPerWeek, "pendingSaddle");

    hideLoading();
  }

async function getSaddlePoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
async function loadSaddleChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getSaddlePoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
