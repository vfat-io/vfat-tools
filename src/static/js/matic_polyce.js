...
async function main() {
   ...
   const CSI_CHEF_ADDR = "0x1c0833f879d8d75E73CBfD8d0F992112006794a5";
   const rewardTokenTicker = "CSI";
   const CSI_CHEF = new ethers.Contract(CSI_CHEF_ADDR, CSI_CHEF_ABI, App.provider);

   const startBlock = await CSI_CHEF.startBlock();
   ...
   if(currentBlock < startBlock){
    ...
    rewardsPerWeek = await CSI_CHEF.csiPerBlock() /1e18
      * 604800 / 2.1;
   }
    ...
    await loadPolyceContract(App, tokens, prices, CSI_CHEF, CSI_CHEF_ADDR, CSI_CHEF_ABI, rewardTokenTicker,
        "csi", null, rewardsPerWeek, "pendingCsi");

    hideLoading();
  }

async function loadPolyceContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getPolycePoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printPolycePool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
async function getPolycePoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
function printPolycePool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printPolyceContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fee, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printPolyceContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
