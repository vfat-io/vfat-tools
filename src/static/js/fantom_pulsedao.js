$(function () {		
  ...
  async function main() {		
      ...
      const StakingPool = {
        address : "0x88a0EC0c1664cd1205D0a7870302FE184b9a24fA",
        abi : PLD_STAKING_ABI,
        stakeTokenFunction : "share",
        rewardTokenAddresses : "pld"
      }
      
      const pSHARE_CHEF_ADDR = "0xBD7f881bC9b35ff38bd5A99eA0A34d559aF950A4";
      const rewardTokenTicker = "pShare";
      const pSHARE_CHEF = new ethers.Contract(pSHARE_CHEF_ADDR, pSHARE_CHEF_ABI, App.provider);

      const rewardsPerWeek = await pSHARE_CHEF.pSharePerSecond() / 1e18 * 604800;
          
      const tokens = {};
      const prices = await getFantomPrices();
      
      await loadFantompSharesContract(App, tokens, prices, pSHARE_CHEF, pSHARE_CHEF_ADDR, pSHARE_CHEF_ABI, rewardTokenTicker,		
          "pShare", null, rewardsPerWeek, "pendingShare");
      _print(`\nStaked pSHAREs can only be withdrawn after 3 epochs since deposit.`);
      ...
      const p0 = await loadFantomPLDSynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                            StakingPool.address, 
...
async function loadFantomPLDSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadFantomPLDSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printPLDSynthetixPool(App, info, "fantom");
}

async function loadFantomPLDSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printPLDSynthetixPool(App, info, chain="eth", customURLs) {
...
async function loadFantompSharesContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
