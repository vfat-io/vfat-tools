...
async function main() {
   ...
   const SPEAR_CHEF_ADDR = "0xD994a824509Bcc46F6F468eeA62baF6A96FF141A";
   const rewardTokenTicker = "SPR";
   const SPEAR_CHEF = new ethers.Contract(SPEAR_CHEF_ADDR, SPEAR_CHEF_ABI, App.provider);

   const rewardsPerWeek = await SPEAR_CHEF.eggPerBlock() /1e18
        * 604800 / 2.1;

    const tokens = {};
    const prices = await getMaticPrices();

    await loadVolveChefContract(App, tokens, prices, SPEAR_CHEF, SPEAR_CHEF_ADDR, SPEAR_CHEF_ABI, rewardTokenTicker,
        "egg", null, rewardsPerWeek, "pendingEgg");

    hideLoading();
  }

async function loadVolveChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getVolvePoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printVolvePool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
async function getVolvePoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
function printVolvePool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printVolveContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fee, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printVolveContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
