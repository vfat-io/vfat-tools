...
async function main() {
   ...
   const MONO_CHEF_ADDR = "0xb27B20CE2343Bf60C0CAC6de4F577F4E71b987EC";
   const rewardTokenTicker = "MONO";
   const MONO_CHEF = new ethers.Contract(MONO_CHEF_ADDR, MONO_CHEF_ABI, App.provider);

   const startBlock = await MONO_CHEF.startBlock();
   ...
   if(currentBlock < startBlock){
    ...
    rewardsPerWeek = await MONO_CHEF.eggPerBlock() /1e18
      * 604800 / 2.1;
   }
    ...
    await loadMonopolyContract(App, tokens, prices, MONO_CHEF, MONO_CHEF_ADDR, MONO_CHEF_ABI, rewardTokenTicker,
        "egg", null, rewardsPerWeek, "pendingEgg");

    hideLoading();
  }

  async function loadMonopolyContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getMonopolyPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printMonopolyPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
async function getMonopolyPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
function printMonopolyPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printMonopolyContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fee, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printMonopolyContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
