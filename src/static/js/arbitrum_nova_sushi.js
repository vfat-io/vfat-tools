...
$(function() {
  ...
  async function main() {
     ...
     const SUSHI_CHEF_ADDR = "0xC09756432dAD2FF50B2D40618f7B04546DD20043";
     ...
     const rewardsPerWeek = await SUSHI_CHEF.sushiPerSecond() /1e18 * 604800;
      ...
      const tokens = {};
      const prices = await getArbitrumNovaPrices();
  
      await loadSushiChefContract(App, tokens, prices, SUSHI_CHEF, SUSHI_CHEF_ADDR, SUSHI_CHEF_ABI, rewardTokenTicker,
          "SUSHI", null, rewardsPerWeek, "pendingSushi", [1], "arbitrum_nova");
  
      hideLoading();
    }
  
  async function loadSushiChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  _print(`<a href='https://nova.arbiscan.io/address/${chefAddress}' target='_blank'>Staking Contract</a>`);
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getSushiPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
