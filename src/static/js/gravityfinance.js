...
async function loadFarmDetails(App, farmAddress, poolAddress, aTokenAddress, bTokenAddress, ERC20ABI, FARMABI, Bzereos, prices) {
    ...
    if (bTokenAddress != 0) {
        ...
        _print_link(`Stake ${Number(userLPBalance).toFixed(8)} [${uSymbol}-${otherSymbol}]`, approveAndStake);
        ...
        _print_bold(`<a href='${url}' target='_blank'>${uSymbol}</a>`);
...
async function loadFarmOtherDetails(App, farmAddress, aTokenAddress, bTokenAddress, ERC20ABI, FARMABI, Bzereos, prices, GFIprice) {
    ...
    _print_link(`Stake ${Number(userOtherBalance).toFixed(8)} [${otherSymbol}]`, approveAndStake);
