...
async function main() {
  ...
  const gaugeAddresses = ["0x1dB11Cf7C332E797ac912e11b8762e0A4b24a836", "0xd7f3Bf2085AD32ff95E1bCC408d37F10f6949270", 
                          "0x5734BB74cFac69f1c34bA66eA6608cCdEe6b81F2", "0x73280c390dA5c6fe05ad2d1e6837E8E8c05E4b32"]

  const gaugeContracts = gaugeAddresses.map(a => new ethcall.Contract(a, HND_GAUGE_DATA_ABI));
  const gaugeCalls = gaugeContracts.map(c => [c.lp_token(), c.totalSupply(), c.balanceOf(App.YOUR_ADDRESS), c.reward_tokens(0), c.reward_policy_maker(), c.working_balances(App.YOUR_ADDRESS), c.working_supply()]).flat();
  ...
  for(let i = 0; i < gaugeValues.length; i++){
    const gageInfo = {
      ...
      rewardTokenAddress : gaugeValues[i+3],
      rewardsAddress     : gaugeValues[i+4],
      working_balances   : gaugeValues[i+5],
      working_supply     : gaugeValues[i+6]
    }
  ...
  //HND reward token
  const rewardToken = await getOptimisticToken(App, "0x10010078a54396F62c96dF8532dc2B4847d47ED3", App.YOUR_ADDRESS);
  const rewardToken2 = await getOptimisticToken(App, "0x4200000000000000000000000000000000000042", App.YOUR_ADDRESS); //OP
  const rewardTokenPrice = getParameterCaseInsensitive(prices, "0x10010078a54396F62c96dF8532dc2B4847d47ED3")?.usd;
  const rewardTokenPrice2 = getParameterCaseInsensitive(prices, "0x4200000000000000000000000000000000000042")?.usd;
  ...
  for(let i = 0; i < gageInfos.length; i++){
    ...
    const _claimableRewards = await App.ethcallProvider.all([gaugeContracts[i].claimable_reward(App.YOUR_ADDRESS, "0x10010078a54396F62c96dF8532dc2B4847d47ED3")]);
    const _claimableRewards2 = await App.ethcallProvider.all([gaugeContracts[i].claimable_reward(App.YOUR_ADDRESS, gageInfos[i].rewardTokenAddress)]);
    const claimableTokens = _claimableRewards / 10 ** rewardToken.decimals;
    const claimableTokens2 = _claimableRewards2 / 10 ** rewardToken2.decimals;
    const poolPrice = getPoolPrices(tokens, prices, lpToken, "optimism")
    //OP rewards
    const [reward_data] = await App.ethcallProvider.all([gaugeContracts[i].reward_data(gageInfos[i].rewardTokenAddress)]);
    const periodFinish = reward_data.period_finish;
    const rewardRate = reward_data.rate;
    const weeklyRewards = (Date.now() / 1000 > periodFinish) ? 0 : rewardRate / 10 ** rewardToken2.decimals * 604800;
    const usdPerWeek = weeklyRewards * rewardTokenPrice2;
    //hnd rewards
    ...
    await printHndContract(App, lpToken, poolPrice, gageInfos[i], gaugeAddresses[i], claimableTokens, 
                           claimableTokens2, rewardToken, rewardToken2, rewardTokenPrice, 
                           rewardTokenPrice2, weeklyRewards, usdPerWeek, working_balances, working_supply,
                           rewardsPerWeek, staked_tvlForCurrentPool);
  }
...
async function printHndContract(App, lpToken, poolPrice, gaugeResult, gauge, 
                                claimableTokens, claimableTokens2, rewardToken, 
                                rewardToken2, rewardTokenPrice, rewardTokenPrice2, 
                                weeklyRewards, usdPerWeek, working_balances, 
                                working_supply, rewardsHndPerWeek, staked_tvl) {
  poolPrice.print_price();

  const usdHNDPerWeek = rewardsHndPerWeek * rewardTokenPrice;
  _print(`${rewardToken.symbol} Per Week: ${rewardsHndPerWeek.toFixed(2)} ($${formatMoney(usdHNDPerWeek)})`);
  const weeklyHndAPR = usdHNDPerWeek / staked_tvl * 100;
  const dailyHndAPR = weeklyHndAPR / 7;
  const yearlyHndAPR = weeklyHndAPR * 52;
  _print(`APR ${rewardToken.symbol} : Day ${dailyHndAPR.toFixed(2)}% Week ${weeklyHndAPR.toFixed(2)}% Year ${yearlyHndAPR.toFixed(2)}%`);
  const userStaked = gaugeResult.balance / 10 ** lpToken.decimals;
  const userStakedUsd = userStaked * poolPrice.price;
  const userStakedPct = userStakedUsd / poolPrice.tvl * 100;
  const usersDailyAPR = dailyHndAPR * (working_balances / working_supply) / userStakedPct * 100
  const usersWeeklyAPR = weeklyHndAPR * (working_balances / working_supply) / userStakedPct * 100
  const usersYearlyAPR = yearlyHndAPR * (working_balances / working_supply) / userStakedPct * 100
  if(working_balances <= 0){
    _print(`Your HND APR: Day 0% Week 0% Year 0%`);
  }else{
    _print(`Your HND APR: Day ${usersDailyAPR.toFixed(4)}% Week ${usersWeeklyAPR.toFixed(2)}% Year ${usersYearlyAPR.toFixed(2)}%`);
  }

  _print(`${rewardToken2.symbol} Per Week: ${weeklyRewards.toFixed(2)} ($${formatMoney(usdPerWeek)})`);
    const weeklyAPR = usdPerWeek / staked_tvl * 100;
    const dailyAPR = weeklyAPR / 7;
    const yearlyAPR = weeklyAPR * 52;
    _print(`APR ${rewardToken2.symbol} : Day ${dailyAPR.toFixed(2)}% Week ${weeklyAPR.toFixed(2)}% Year ${yearlyAPR.toFixed(2)}%`);
  ...
  const claim = async function() {
    return hndGaugeClaim(App, gauge, rewardToken2)
  }
  ...
  _print_link(`Claim ${claimableTokens.toFixed(6)} ${rewardToken.symbol} ($${formatMoney(claimableTokens*rewardTokenPrice)}) + ${claimableTokens2.toFixed(6)} ${rewardToken2.symbol} ($${formatMoney(claimableTokens2*rewardTokenPrice2)})`, claim);
  _print("");
}
...
async function hndGaugetDeposit(App, gauge, token){
  ...
  const GAUGE_CONTRACT = new ethers.Contract(gauge, HND_GAUGE_DATA_ABI, signer)
