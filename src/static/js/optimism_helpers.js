...
async function getOptimisticSaddleToken(App, saddle, address, stakingAddress, swapAddress) {
  const swap = new ethers.Contract(swapAddress, OPTIMISTIC_SADDLE_SWAP_ABI, App.provider)
...
async function getOptimisticStoredToken(App, tokenAddress, stakingAddress, type) {
  switch (type) {
    ...
    case "optimisticSaddle":
      const saddle = new ethers.Contract(tokenAddress, OPTIMISTIC_SADDLE_LP_TOKEN_ABI, App.provider);
      const swap = await saddle.swap();
      return await getOptimisticSaddleToken(App, saddle, tokenAddress, stakingAddress, swap);
    case "erc20":
...
async function getOptimisticToken(App, tokenAddress, stakingAddress) {
    ...
    try {
      const saddle = new ethers.Contract(tokenAddress, OPTIMISTIC_SADDLE_LP_TOKEN_ABI, App.provider);
      const swap = await saddle.swap();
      const res = await getOptimisticSaddleToken(App, saddle, tokenAddress, stakingAddress, swap);
