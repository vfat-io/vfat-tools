...
$(function() {
    ...
    async function main() {
       ...
       const SPARK_CHEF_ADDR = "0x2c8BBd2cecC77F2d18A04027Cc03CDB8Ab103214";
       const rewardTokenTicker = "SPARK";
       const SPARK_CHEF = new ethers.Contract(SPARK_CHEF_ADDR, SPARK_CHEF_ABI, App.provider);

       const blocksPerSeconds = await getAverageBlockTime(App);
    
       const rewardsPerWeek = await SPARK_CHEF.sparkPerBlock() /1e18 * 604800 / blocksPerSeconds;
    
        const tokens = {};
        const prices = await getPulsePrices();
    
        await loadSparkChefContract(App, tokens, prices, SPARK_CHEF, SPARK_CHEF_ADDR, SPARK_CHEF_ABI, rewardTokenTicker,
            "spark", null, rewardsPerWeek, "pendingSpark", [], "pulse");
    
        hideLoading();
      }

async function getSparkPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  const poolInfo = await chefContract.poolInfo(poolIndex);
  if (poolInfo.allocPoint == 0 || poolIndex == 1 || poolIndex == 3) {
    return {
...
async function loadSparkChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getSparkPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
