$(function () {		
    ...
    async function main() {		
        ...
        const WIND_CHEF_ADDR = "0x6eC89CCcDb563Ac442d2370F6E47bC1C78e023fC";
        const rewardTokenTicker = "WIND";		
        const WIND_CHEF = new ethers.Contract(WIND_CHEF_ADDR, WIND_CHEF_ABI, App.provider);

        let rewardsPerWeek = 0
        const startBlock = await WIND_CHEF.startBlock();
        const currentBlock = await App.provider.getBlockNumber();

        const multiplier = await WIND_CHEF.getMultiplier(currentBlock, currentBlock + 1);
     
        if(currentBlock < startBlock){
         _print(`Rewards start at block ${startBlock}\n`)
        }else{
            rewardsPerWeek = await WIND_CHEF.StormPerBlock() / 1e18 * multiplier * 604800 / 5.6;
        }
        		
        const tokens = {};		
        const prices = await getCronosPrices();		
    		
        await loadWINDContract(App, tokens, prices, WIND_CHEF, WIND_CHEF_ADDR, WIND_CHEF_ABI, rewardTokenTicker,		
            "Storm", null, rewardsPerWeek, "pendingStorm", [8]);		
    		
        hideLoading();		
    }

async function loadWINDContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printWINDPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
function printWINDPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printWINDContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fixedDecimals, claimFunction, rewardPrice, chain, depositFee, withdrawFee);
  return apr;
}

function printWINDContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
