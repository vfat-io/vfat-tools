...
const nft_manager_address_v4 = '0x3C3Ea4B57a46241e54610e5f022E5c45859A1017'
// Uniswap v4 Optimism deployments (10): https://docs.uniswap.org/contracts/v4/deployments
const pool_manager_address_v4 = '0x9a13f98cb987694c9f086b1f5eb990eea8264ec3'
// Reading current tick/sqrtPrice is done via StateView, not PoolManager.
const state_view_address_v4 = '0xc18a3169788f4f75a170290584eca6395c75ecdb'
const sickle_factory_address = '0xB4C31b0f0B76b351395D4aCC94A54dD4e6fbA1E8'
const sweep_address = '0x9b60492564D1c428630D4914d8f61bF8d0AB4f5C'
...
const withdraw_nfts = async function(App, nft_manager_v4, nft_manager_address_v4, sickleAddress) {
  ...
  for (const p of positions) {
    ...
    const token0Meta = await getErc20MetaCached(App, tokenMetaCache, p.poolKey.currency0, OPTIMISM_CHAIN_ID)
    const token1Meta = await getErc20MetaCached(App, tokenMetaCache, p.poolKey.currency1, OPTIMISM_CHAIN_ID)
...
const fallback_withdraw = async function(App, nft_manager_address_v4, sickleAddress) {
  ...
  _print(
    `<a target="_blank" href="https://optimistic.etherscan.io/token/${nft_manager_address_v4}?a=${sickleAddress}#inventory">Nft Position Manager</a>`
  )
  _print('')
