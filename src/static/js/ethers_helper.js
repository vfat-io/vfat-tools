async function init_ethers() {
  ...
  App.provider = new ethers.providers.JsonRpcProvider("wss://bsc-ws-node.nariox.org:443")
  sleep(10)
...
const _print = function(message) {
  ...
  for (let i = 0; i < arguments.length; i++) {
    if (typeof arguments[i] == 'object') {
      logger.innerHTML += '<p>' +
        (JSON && JSON.stringify ? JSON.stringify(arguments[i], undefined, 2) : arguments[i]) + '</p>'
    } else {
