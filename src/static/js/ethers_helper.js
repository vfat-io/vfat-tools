...
const pageNetwork = function() {
  ...
  if (network.toLowerCase() === 'doge') {
    return window.NETWORKS.DOGE
  }
  if (network.toLowerCase() === 'fx') {
    return window.NETWORKS.FX
  }
  ...
  if (network.toLowerCase() === 'monad') {
    return window.NETWORKS.MONAD
  }
  ...
  if (network.toLowerCase() === 'metal_l2') {
    return window.NETWORKS.METAL_L2
  }
...
async function getBalancerV2Pool(app, pool, poolAddress, stakingAddress, tokens, smartToken) {
  const calls = [pool.decimals(), pool.symbol(), pool.name(), pool.totalSupply(),
...
async function getStoredToken(app, tokenAddress, stakingAddress, type) {
  switch (type) {
    ...
    case "balancerV2":
      const balV2 = new ethcall.Contract(tokenAddress, BALANCER_V2_POOL_ABI);
      const [vaultAddress, poolId] = await app.ethcallProvider.all([balV2.getVault(), balV2.getPoolId()]);
...
async function getToken(app, tokenAddress, stakingAddress) {
  ...
  try {
    const bal2 = new ethcall.Contract(tokenAddress, BALANCER_V2_POOL_ABI);
    const [vaultAddress, poolId] = await app.ethcallProvider.all([bal2.getVault(), bal2.getPoolId()]);
    ...
    const balPool2 = await getBalancerV2Pool(app, bal2, tokenAddress, stakingAddress, tokens);
    window.localStorage.setItem(tokenAddress, "balancerV2");
    return balPool2;
  }
  catch(err) {
  }
  try {
    ...
    const vault = new ethcall.Contract(vaultAddress, BALANCER_VAULT_ABI);
    const [tokens] = await app.ethcallProvider.all([vault.getPoolTokens(poolId)]);
    const balStablePool = await getBalancerStablePool(app, balStable, tokenAddress, stakingAddress, tokens);
    window.localStorage.setItem(tokenAddress, "balancerStable");
...
function getUniPrices(tokens, prices, pool, chain="eth")
{
  ...
  else if (pool.symbol.includes("MSLP")) stakeTokenTicker += " MilkySwap LP Token";
      ...
      print_price(chain="eth", decimals, customURLs) {
        ...
        if (customURLs) {
          ...
          pool.symbol.includes("PepeDex-LP") ? [
            ...
            `https://pepedex.pepepal.com/remove/${t0address}/${t1address}`,
            `https://pepedex.pepepal.com/swap?inputCurrency=${t0address}&outputCurrency=${t1address}`
          ] :
          ...
          pool.symbol.includes("FX-V2") ? [
            `https://fx-swap.io/#/add/${t0address}/${t1address}`,
            `https://fx-swap.io/#/remove/${t0address}/${t1address}`,
            `https://fx-swap.io/#/swap?inputCurrency=${t0address}&outputCurrency=${t1address}`
          ] :
          ...
          pool.symbol.includes("DST-V2") ? [
            `https://dexscreener.com/dogechain/`,
            `https://dexscreener.com/dogechain/`,
            `https://dexscreener.com/dogechain/`
          ] :
...
function getErc20Prices(prices, pool, chain="eth") {
  ...
  switch (chain) {
    ...
    case "kava":
      ...
      break;
    case "doge":
      poolUrl=`https://explorer.dogechain.dog/token/${pool.address}`;
      break;
    case "fx":
      poolUrl=`https://explorer.starscan.io/evm/token/${pool.address}`;
      break;
    case "matic":
    ...
    case "lisk":
      ...
      break;
    case "metal_l2":
      poolUrl=`https://explorer.metall2.com/token/${pool.address}`;
      break;
    case "sonic":
...
async function printSynthetixPool(App, info, chain="eth", customURLs) {
    ...
    switch (chain) {
      ...
      case "kava":
        ...
        break;
      case "doge":
        _print(`<a target="_blank" href="https://explorer.dogechain.dog/address/${info.stakingAddress}#code">Doge Explorer</a>`);
        break;
      case "fx":
        _print(`<a target="_blank" href="https://explorer.starscan.io/evm/address/${info.stakingAddress}">Function X Scan</a>`);
        break;
      case "fantom":
      ...
      case "lisk":
        ...
        break;
      case "metal_l2":
        _print(`<a target="_blank" href="https://explorer.metall2.com/address/${info.stakingAddress}">Metal-L2 Explorer</a>`);
        break;
      case "sonic":
...
function getChainExplorerUrl(chain, address){
  switch(chain){
    ...
    case "metal_l2" :
      return `https://explorer.metall2.com/token/${address}`;
    case "sonic" :
