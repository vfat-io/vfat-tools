...
async function main() {
  ...
  const UFX_CHEF_ADDR = '0x92805BBd01D25fD76bB650a3068ead3694E65364'
  const rewardTokenTicker = 'UFX'
  const UFX_CHEF = new ethers.Contract(UFX_CHEF_ADDR, UFX_CHEF_ABI, App.provider)

  const startBlock = await UFX_CHEF.startBlock()
  const currentBlock = await App.provider.getBlockNumber()

  const multiplier = await UFX_CHEF.getMultiplier(currentBlock, currentBlock + 1)
  ...
  if (currentBlock < startBlock) {
    ...
    rewardsPerWeek = (((await UFX_CHEF.ufxPerBlock()) / 1e18) * 604800 * multiplier) / 3
  }
