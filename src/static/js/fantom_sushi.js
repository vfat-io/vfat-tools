$(function () {
  ...
  async function loadFantomSushiContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
    ...
    const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
      await getSushiPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction, chefFtmRewardsContract,
          chefFtmRewardsAddress)));
    ...
    for (i = 0; i < poolCount; i++) {
      if (poolPrices[i]) {
        const apr = printSushiPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
          ...
          pendingRewardsFunction, null, null, "fantom", chefFtmRewardsAbi, chefFtmRewardsAddress, ftmRewardsPerWeek,
          rewardFtmTicker, ftmTokenAddress, poolInfos[i].pendingFtmTokens)
        aprs.push(apr);
      }
    }
  ...
  async function getSushiPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction, chefFtmRewardsContract,
      chefFtmRewardsAddress) {
      ...
      const pendingFtmTokens = await chefFtmRewardsContract.pendingToken(poolIndex, app.YOUR_ADDRESS);
      ...
      return {
          ...
          pendingFtmTokens : pendingFtmTokens / 10 ** 18,
      };
    }
  ...
  function printSushiPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                         ...
                         chefFtmRewardsAbi, chefFtmRewardsAddress, ftmRewardsPerWeek,
                         rewardFtmTicker, ftmTokenAddress, pendingFtmTokens) {
    fixedDecimals = fixedDecimals ?? 2;
    const sp = (poolInfo.stakedToken == null) ? null : getPoolPrices(tokens, prices, poolInfo.stakedToken);
    var poolRewardsPerWeek = poolInfo.allocPoints / totalAllocPoints * rewardsPerWeek;
    let poolFtmRewardsPerWeek = poolInfo.allocPoints / totalAllocPoints * ftmRewardsPerWeek;
    if (poolRewardsPerWeek == 0 && rewardsPerWeek != 0) return;
    const userStaked = poolInfo.userLPStaked ?? poolInfo.userStaked;
    const rewardPrice = getParameterCaseInsensitive(prices, rewardTokenAddress)?.usd;
    const rewardFtmPrice = getParameterCaseInsensitive(prices, ftmTokenAddress)?.usd;
    const staked_tvl = sp?.staked_tvl ?? poolPrices.staked_tvl;
    poolPrices.print_price(chain);
    sp?.print_price(chain);
    const apr = printSushiAPR(rewardTokenTicker, rewardPrice, poolRewardsPerWeek, poolPrices.stakeTokenTicker,
      staked_tvl, userStaked, poolPrices.price, fixedDecimals, poolFtmRewardsPerWeek, rewardFtmPrice, rewardFtmTicker);
    ...
    printSushiContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
      ...
      poolInfo.userStaked, poolInfo.pendingRewardTokens, fixedDecimals, rewardPrice, pendingFtmTokens, rewardFtmTicker, rewardFtmPrice);
    _print("");
    return apr;
  }
  ...
  function printSushiAPR(rewardTokenTicker, rewardPrice, poolRewardsPerWeek,
                    ...
                    fixedDecimals, poolFtmRewardsPerWeek, rewardFtmPrice, rewardFtmTicker) {
    var usdPerWeek = poolRewardsPerWeek * rewardPrice;
    var usdFtmPerWeek = poolFtmRewardsPerWeek * rewardFtmPrice;
    fixedDecimals = fixedDecimals ?? 2;
    _print(`${rewardTokenTicker} Per Week: ${poolRewardsPerWeek.toFixed(fixedDecimals)} ($${formatMoney(usdPerWeek)})`);
    _print(`${rewardFtmTicker} Per Week: ${poolFtmRewardsPerWeek.toFixed(fixedDecimals)} ($${formatMoney(usdFtmPerWeek)})`);
    var weeklyAPR = usdPerWeek / staked_tvl * 100;
    var dailyAPR = weeklyAPR / 7;
    var yearlySushiAPR = weeklyAPR * 52;
    var weeklyFtmAPR = usdFtmPerWeek / staked_tvl * 100;
    var dailyFtmAPR = weeklyFtmAPR / 7;
    var yearlyFtmAPR = weeklyFtmAPR * 52;
    let totalDailyAPR = dailyAPR + dailyFtmAPR;
    let totalWeeklyAPR = weeklyAPR + weeklyFtmAPR;
    let totalYearlyAPR = yearlySushiAPR + yearlyFtmAPR;
    let totalUSDPerWeek = usdPerWeek + usdFtmPerWeek;
    _print(`APR SUSHI: Day ${dailyAPR.toFixed(2)}% Week ${weeklyAPR.toFixed(2)}% Year ${yearlySushiAPR.toFixed(2)}%`);
    _print(`APR WFTM: Day ${dailyFtmAPR.toFixed(2)}% Week ${weeklyFtmAPR.toFixed(2)}% Year ${yearlyFtmAPR.toFixed(2)}%`);
    ...
    var userFtmWeeklyRewards = userStakedPct * poolFtmRewardsPerWeek / 100;
    var userDailyRewards = userWeeklyRewards / 7;
    var userFtmDailyRewards = userFtmWeeklyRewards / 7;
    var userYearlyRewards = userWeeklyRewards * 52;
    var userFtmYearlyRewards = userFtmWeeklyRewards * 52;
    if (userStaked > 0) {
      _print(`Estimated Total earnings:`
          + ` Day ($${formatMoney(userDailyRewards*rewardPrice+userFtmDailyRewards*rewardFtmPrice)})`
          + ` Week ($${formatMoney(userWeeklyRewards*rewardPrice+userFtmWeeklyRewards*rewardFtmPrice)})`
          + ` Year ($${formatMoney(userYearlyRewards*rewardPrice+userFtmYearlyRewards*rewardFtmPrice)})`);
    }
    return {
      userStakedUsd,
      ...
      userYearlyUsd : userYearlyRewards * rewardPrice + userFtmYearlyRewards * rewardFtmPrice
    }
  }

function printSushiContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
    rewardTokenTicker, stakeTokenTicker, unstaked, userStaked, pendingRewardTokens, fixedDecimals, rewardTokenPrice, pendingFtmTokens,
    rewardFtmTicker, rewardFtmPrice) {
  fixedDecimals = fixedDecimals ?? 2;
  ...
  _print_link(`Claim ${pendingRewardTokens.toFixed(fixedDecimals)} ${rewardTokenTicker} ($${formatMoney(pendingRewardTokens*rewardTokenPrice)}) + ${pendingFtmTokens.toFixed(fixedDecimals)} ${rewardFtmTicker} ($${formatMoney(pendingFtmTokens*rewardFtmPrice)})`, claim)
