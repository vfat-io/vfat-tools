...
async function main() {
   ...
   const LAM_CHEF_ADDR = "0x8650f40741eB1f0F422bA2Ac04668bA15746F5b0";
   const rewardTokenTicker = "LAM";
   const LAM_CHEF = new ethers.Contract(LAM_CHEF_ADDR, LAM_CHEF_ABI, App.provider);

   const rewardsPerWeek = await LAM_CHEF.lamPerBlock() /1e18
        * 604800 / 3;

    const tokens = {};
    const prices = await getBscPrices();

    await loadLamContract(App, tokens, prices, LAM_CHEF, LAM_CHEF_ADDR, LAM_CHEF_ABI, rewardTokenTicker,
        "lam", null, rewardsPerWeek, "pendingLam");

    hideLoading();
  }

async function loadLamContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getLamInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printLamPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
async function getLamInfo(App, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
function printLamPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printLamContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fee, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printLamContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
