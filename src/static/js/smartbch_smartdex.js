$(function() {
  ...
  async function main() {
    ...
    const SMARTSWAP_CHEF = new ethers.Contract(SMARTSWAP_CHEF_ADDR, SMARTSWAP_CHEF_ABI, App.provider);
  
    const blockNumber = await App.provider.getBlockNumber();
    const multiplier = await SMARTSWAP_CHEF.getMultiplier(blockNumber, blockNumber+1);
    const rewardPerBlock = await SMARTSWAP_CHEF.REWARD_PER_BLOCK();
    const rewardsPerWeek = rewardPerBlock / 1e18 * multiplier * 604800 / 6
  
    const tokens = {};
    const basePrices = await getSmartbchPrices();
  
    const { prices, totalUserStaked, totalStaked, averageApr } = await loadSmartswapChefContract(
      App,
      tokens,
      basePrices,
      SMARTSWAP_CHEF,
    ...
    const DEXSMART = new ethers.Contract(DSMART_TOKEN_ADDR, ERC20_ABI, App.provider.getSigner());
    const SMARTSWAP_PIT = new ethers.Contract(SMARTSWAP_PIT_ADDR, SMARTSWAP_PIT_ABI, App.provider.getSigner());
    const smartswapPit = await smartswapPitData(DEXSMART, SMARTSWAP_PIT, App, prices);
  
    const totalStakedInclSmartswapPit = totalStaked + smartswapPit.totalBalanceUSD
  
    _print_bold(`Total Staked: $${formatMoney(totalStakedInclSmartswapPit)}`);
    ...
    if (smartswapPit.userBalanceUSD > 0) {
      _print(`You are staking a total of $${formatMoney(smartswapPit.userBalanceUSD)} in the ${PIT_NAME} single staking pool.`);
    }
  
    hideLoading();
  }
  
  async function loadSmartswapChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
    ...
    for (i = 0; i < poolCount; i++) {
      if (poolPrices[i]) {
        const apr = printSmartswapChefPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
  ...
  function printSmartswapChefPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                              totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
    ...
    printSmartswapChefContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fixedDecimals, claimFunction, rewardPrice, chain, depositFee, withdrawFee);
    return apr;
  }
  
  function printSmartswapChefContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
                                       rewardTokenTicker, stakeTokenTicker, unstaked, userStaked, pendingRewardTokens, fixedDecimals,
                                       claimFunction, rewardTokenPrice, chain, depositFee, withdrawFee) {
    fixedDecimals = fixedDecimals ?? 2;
    const approveAndStake = async function() {
      return smartswapContract_stake(chefAbi, chefAddr, poolIndex, poolAddress, App)
    }
    const unstake = async function() {
      return smartswapContract_unstake(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction)
    }
    const claim = async function() {
      return smartswapContract_claim(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction, claimFunction)
    }
    if(depositFee > 0){
  ...
  const smartswapContract_stake = async function(chefAbi, chefAddress, poolIndex, stakeTokenAddr, App) {
  ...
  const smartswapContract_unstake = async function(chefAbi, chefAddress, poolIndex, App, pendingRewardsFunction) {
    const signer = App.provider.getSigner()
    const CHEF_CONTRACT = new ethers.Contract(chefAddress, chefAbi, signer)
  ...
  const smartswapContract_claim = async function(chefAbi, chefAddress, poolIndex, App,
  ...
  async function smartswapPitData(DEXSMART, SMARTSWAP_PIT, App, prices) {
    const smartswapPitTotalBalance = await DEXSMART.balanceOf(SMARTSWAP_PIT_ADDR) / 1e18;
    const smartswapPitTotalSupply = await SMARTSWAP_PIT.totalSupply() / 1e18;
    const xSmartRatio = smartswapPitTotalBalance / smartswapPitTotalSupply;
  
    const smartswapPitUserBalanceBigNum = await SMARTSWAP_PIT.balanceOf(App.YOUR_ADDRESS);
    const smartswapPitUserBalance = (smartswapPitUserBalanceBigNum / 1e18) * xSmartRatio;
    const smartswapPitShare = smartswapPitUserBalance / smartswapPitTotalBalance;
    const dsmartBalanceBigNum = await DEXSMART.balanceOf(App.YOUR_ADDRESS);
    const dsmartBalance = dsmartBalanceBigNum / 1e18;
  
    const dsmartPrice = prices[DSMART_TOKEN_ADDR];
  
    let smartswapPitTotalBalanceUSD;
    let smartswapPitUserBalanceUSD;
  
    if (dsmartPrice && dsmartPrice.usd) {
      smartswapPitTotalBalanceUSD = smartswapPitTotalBalance * dsmartPrice.usd;
      smartswapPitUserBalanceUSD = smartswapPitUserBalance * dsmartPrice.usd;
  
      _print(`${PIT_NAME} TVL: $${formatMoney(smartswapPitTotalBalanceUSD)}`);
      _print(`${REWARD_TOKEN_TICKER} Price: $${formatMoney(dsmartPrice.usd)}`);
      _print(`Staked: ${(smartswapPitTotalBalance).toFixed(2)} ${REWARD_TOKEN_TICKER} ($${formatMoney(smartswapPitTotalBalanceUSD)})`);
      _print(`You are staking ${(smartswapPitUserBalance).toFixed(2)} ${REWARD_TOKEN_TICKER} ($${formatMoney(smartswapPitUserBalanceUSD)}), ${(smartswapPitShare * 100).toFixed(2)}% of the pool.`);
  
      const approveAndEnter = async function() {
        return smartswapPitApproveAndEnter(DEXSMART, SMARTSWAP_PIT, dsmartBalanceBigNum, App);
      }
  
      const leave = async function() {
        return smartswapPitLeave(SMARTSWAP_PIT, smartswapPitUserBalanceBigNum, App);
      }
  
      _print_link(`Stake ${(dsmartBalance).toFixed(2)} ${REWARD_TOKEN_TICKER}`, approveAndEnter);
  
      if (smartswapPitUserBalance > 0) {
        _print_link(`Unstake ${smartswapPitUserBalance.toFixed(2)} ${REWARD_TOKEN_TICKER}`, leave);
      }
    ...
    return {
      totalBalance: smartswapPitTotalBalance,
      totalBalanceUSD: smartswapPitTotalBalanceUSD,
      userBalance: smartswapPitUserBalance,
      userBalanceUSD: smartswapPitUserBalanceUSD
    }
  }
  
  async function smartswapPitApproveAndEnter(DEXSMART, SMARTSWAP_PIT, currentTokens, App) {
    const allowedTokens = await DEXSMART.allowance(App.YOUR_ADDRESS, SMARTSWAP_PIT_ADDR)
    let allow = Promise.resolve()
  
    if (allowedTokens / 1e18 < currentTokens / 1e18) {
      showLoading()
      allow = DEXSMART.approve(SMARTSWAP_PIT_ADDR, ethers.constants.MaxUint256)
      .then(function(t) {
        return App.provider.waitForTransaction(t.hash)
      })
      .catch(function() {
    ...
    if (currentTokens / 1e18 > 0) {
      ...
      allow
      .then(async function() {
        SMARTSWAP_PIT.enter(currentTokens, {gasLimit: 500000})
        .then(function(t) {
          App.provider.waitForTransaction(t.hash).then(function() {
            hideLoading()
          })
  ...
  async function smartswapPitLeave(SMARTSWAP_PIT, currentStakedAmount, App) {
    showLoading()
    SMARTSWAP_PIT.leave(currentStakedAmount, {gasLimit: 500000})
    .then(function(t) {
      App.provider.waitForTransaction(t.hash).then(function() {
        hideLoading()
      })
