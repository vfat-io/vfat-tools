...
async function main() {
   ...
   const DRINK_CHEF_ADDR = "0x405324eb5b216f121b074cf343775814a21ebf91";
   const rewardTokenTicker = "DRINK";
   const DRINK_CHEF = new ethers.Contract(DRINK_CHEF_ADDR, DRINK_CHEF_ABI, App.provider);
   const rewardsPerWeek = await DRINK_CHEF.sushiPerSecond() /1e18 * 604800;
    const tokens = {};
    const prices = await getMaticPrices();
    await loadBeverageContract(App, tokens, prices, DRINK_CHEF, DRINK_CHEF_ADDR, DRINK_CHEF_ABI, rewardTokenTicker,
        "SUSHI", null, rewardsPerWeek, "pendingSushi");

    hideLoading();
  }

async function getBeveragePoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {  
...
async function loadBeverageContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getBeveragePoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
