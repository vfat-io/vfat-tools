$(function () {		
    ...
    async function main() {		
        ...
        const rewardsPerWeek = await GSHARE_CHEF.tSharePerSecond() / 1e18 * 604800;
  ...
  async function loadFantomGHOSTSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
    const info = await loadFantomGHOSTSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
    if (!info) return null;
    return await printGHOSTSynthetixPool(App, info, "fantom");
  }
  
  async function loadFantomGHOSTSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
  ...
  async function printGHOSTSynthetixPool(App, info, chain="eth", customURLs) {
  ...
  async function loadFantomGshareContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
