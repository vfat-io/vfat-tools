...
async function main() {
    ...
    const SYN_CHEF_ADDR = "0xe8c610fcb63A4974F02Da52f0B4523937012Aaa0";
    ...
    //prices[""] = { usd : 1 };
    prices["0x4619a06DDd3b8f0F951354ec5e75c09cD1CD1AEF"] = prices["0x4200000000000000000000000000000000000006"];
...
async function getSynPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  ...
  const poolToken = await getOptimisticToken(app, lpToken, chefAddress);
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printChefPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
        ...
        pendingRewardsFunction, null, null, "optimistic", poolInfos[i].depositFee, poolInfos[i].withdrawFee)
      aprs.push(apr);
    }
  }
