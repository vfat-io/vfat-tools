$(function () {		
    ...
    async function main() {		
        ...
        const StakingPool = {
          address : "0x78996e5f9f486d44f74a6896fdd8d7e21780d86a",
          abi : PLATINUM_STAKING_ABI,
          stakeTokenFunction : "share",
          rewardTokenAddresses : "plat"
        }
    		
        const SPLAT_CHEF_ADDR = "0xe39735d9741cbf0b8824a196a5bcb3d729153702";
        const PLAT_CHEF_ADDR = "0x19d6CE05c9D7Fe5A837cC27400e51817Fc05B229";
        const rewardTokenTicker = "SPLAT";
        const rewardTokenTicker2 = "PLAT";
        const SPLAT_CHEF = new ethers.Contract(SPLAT_CHEF_ADDR, SPLAT_CHEF_ABI, App.provider);
        const PLAT_CHEF = new ethers.Contract(PLAT_CHEF_ADDR, PLAT_CHEF_ABI, App.provider);

        const rewardsPerWeek = await SPLAT_CHEF.sPlatPerSecond() / 1e18 * 604800;
        const rewardsPerWeek2 = await PLAT_CHEF.platPerSecond() / 1e18 * 604800;
        		
        const tokens = {};		
        const prices = await getFantomPrices();		
    		
        await loadFantomSplatContract(App, tokens, prices, SPLAT_CHEF, SPLAT_CHEF_ADDR, SPLAT_CHEF_ABI, rewardTokenTicker,		
            "splat", null, rewardsPerWeek, "pendingShare", [1]);
        _print(`\nUpon stake, the funds will be locked for 6 epochs. Rewards can be claimed after 3 epochs after deposit.`);
        _print(`Any time the user claims rewards or stakes more funds or unstakes partially, both lock and reward counter will be reset.\n`)
        const p0 = await loadFantomPlatinumSynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                              StakingPool.address, 
        ...
        /*await loadFantomPlatContract(App, tokens, prices, PLAT_CHEF, PLAT_CHEF_ADDR, PLAT_CHEF_ABI, rewardTokenTicker2,		
            "plat", null, rewardsPerWeek2, "pendingPLAT");*/
    		
        hideLoading();		
    }

async function loadFantomPlatinumSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
    const info = await loadFantomPlatinumSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
    if (!info) return null;
    return await printPlatinumSynthetixPool(App, info, "fantom");
}

async function loadFantomPlatinumSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printPlatinumSynthetixPool(App, info, chain="eth", customURLs) {
...
async function loadFantomSplatContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
...
async function loadFantomPlatContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  rewardTokenFunction, rewardsPerBlockFunction, rewardsPerWeekFixed, pendingRewardsFunction,
  deathPoolIndices, claimFunction) {
  const chefContract = chef ?? new ethers.Contract(chefAddress, chefAbi, App.provider);
