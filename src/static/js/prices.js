...
const bscTokens = [
  ...
  { "id": "topaz", "symbol": "TOPAZ", "contract": "0xdf002282C1474C9592780618Adda7EaA99998Abd" }
]
...
const ScrollTokens = [
  ...
  { "id": "nuri-exchange", "symbol": "NURI", "contract": "0xaaae8378809bb8815c08d3c59eb0c7d1529ad769" }
];
...
async function getKatanaPrices() {
...
async function getSoneiumPrices() {
  const idPrices = await lookUpPrices(SoneiumTokens.map(x => x.id));
  const prices = {}
  for (const bt of SoneiumTokens)
    if (idPrices[bt.id])
      prices[bt.contract] = idPrices[bt.id];
  return prices;
}
