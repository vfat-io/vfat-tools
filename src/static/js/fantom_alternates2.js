$(function () {		
  ...
  async function main() {		
      ...
      const StakingPool = {
        address : "0xa63CB2d094a033ca9bb86dE94a8a1DE380C7001d",
        abi : ALT2_STAKING_ABI,
        stakeTokenFunction : "share",
        rewardTokenAddresses : "tomb"
      }
      
      const ASHARE2_CHEF_ADDR = "0xEA0032f5177FB2F4D4F932DBE61377F47E8A7E47";
      const rewardTokenTicker = "ASHARE2";
      const ASHARE2_CHEF = new ethers.Contract(ASHARE2_CHEF_ADDR, ASHARE2_CHEF_ABI, App.provider);

      const rewardsPerWeek = await ASHARE2_CHEF.aSharePerSecond() / 1e18 * 604800;
          
      const tokens = {};
      const prices = await getFantomPrices();
      
      await loadFantomAshare2Contract(App, tokens, prices, ASHARE2_CHEF, ASHARE2_CHEF_ADDR, ASHARE2_CHEF_ABI, rewardTokenTicker,		
          "ashare", null, rewardsPerWeek, "pendingShare");
      _print("");
      _print("Staked ASHARE2s can only be withdrawn after 6 epochs since deposit.\n")
      const p0 = await loadFantomALT2SynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                            StakingPool.address, 
...
async function loadFantomALT2SynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadFantomALT2SynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printALT2SynthetixPool(App, info, "fantom");
}

async function loadFantomALT2SynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printALT2SynthetixPool(App, info, chain="eth", customURLs) {
...
async function loadFantomAshare2Contract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
