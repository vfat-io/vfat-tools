...
async function main() {
  ...
  //HND reward token
  const rewradToken = await getHarmonyToken(App, "0x10010078a54396f62c96df8532dc2b4847d47ed3", App.YOUR_ADDRESS);
  ...
  for(let i = 0; i < gageInfos.length; i++){
    const lpToken = await getHarmonyToken(App, gageInfos[i].lpTokenAddress, gaugeAddresses[i]);
    ...
    const poolPrice = getPoolPrices(tokens, prices, lpToken, "harmony")
