...
async function main() {
   ...
   const BIRD_CHEF_ADDR = "0xC312768E45037f0d67B0E3922394316203959358";
   ...
   const rewardsPerWeek = await BIRD_CHEF.birdPerBlock() /1e18 * 604800;
    ...
    await loadAvaxChefContract(App, tokens, prices, BIRD_CHEF, BIRD_CHEF_ADDR, BIRD_CHEF_ABI, rewardTokenTicker,
        "bird", null, rewardsPerWeek, "pendingBIRD");

    hideLoading();
  }

async function loadBirdContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getBirdInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printBirdPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
        totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
        pendingRewardsFunction, poolInfos[i].fee, null, null, "avax")
      aprs.push(apr);
    }
  }
...
async function getBirdInfo(App, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  ...
  return {
      ...
      fee : poolInfo.withdrawFeeBP / 100
  };
}

function printBirdPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printBirdContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fee, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printBirdContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
  ...
  if(fee == 0){
    ...
    _print_link(`Unstake ${userStaked.toFixed(fixedDecimals)} ${stakeTokenTicker} - Fee ${fee.toFixed(2)}%`, unstake)
  }
