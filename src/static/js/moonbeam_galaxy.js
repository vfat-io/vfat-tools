...
async function main() {
  ...
  const StakingContracts = [
    "0x3320368A197400A6Fb28532D9cc4E1Db2e41e227"
  ]
  const pools = StakingContracts.map(a => { return {
      address: a,
      abi: GLX_STAKING_ABI,
      stakeTokenFunction: "share",
      rewardTokenFunction: "galaxy"
    }})

   const GSHARE_CHEF_ADDR = "0x5419181AaF392DE9f04Ce0e26c435b6Bad3eF4D9";
   const rewardTokenTicker = "GSHARE";
   const GSHARE_CHEF = new ethers.Contract(GSHARE_CHEF_ADDR, GSHARE_CHEF_ABI, App.provider);

   const rewardsPerWeek = await GSHARE_CHEF.gSharePerSecond() / 1e18 * 604800;

    const tokens = {};
    const prices = await getMoonbeamPrices();

    await loadGShareChefContract(App, tokens, prices, GSHARE_CHEF, GSHARE_CHEF_ADDR, GSHARE_CHEF_ABI, rewardTokenTicker,
    ...
    let p = await loadMultipleGlxSynthetixPools(App, tokens, prices, pools)
...
async function loadGShareChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
...
async function loadMultipleGlxSynthetixPools(App, tokens, prices, pools) {
  let totalStaked  = 0, totalUserStaked = 0, individualAPRs = [];
  const infos = await Promise.all(pools.map(p =>
      loadGlxSynthetixPoolInfo(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction)));
  for (const i of infos) {
    let p = await printGlxSynthetixPool(App, i, "moonbeam");
...
async function loadGlxSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printGlxSynthetixPool(App, info, chain="eth", customURLs) {
