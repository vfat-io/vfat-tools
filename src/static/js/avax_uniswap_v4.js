...
const nft_manager_address_v4 = '0xB74b1F14d2754AcfcbBe1a221023a5cf50Ab8ACD'
const pool_manager_address_v4 = '0x06380c0e0912312b5150364b9dc4542ba0dbbc85'
// Uniswap v4 Avalanche deployments (43114): https://docs.uniswap.org/contracts/v4/deployments
// Reading current tick/sqrtPrice is done via StateView, not PoolManager.
const state_view_address_v4 = '0xc3c9e198c735a4b97e3e683f391ccbdd60b69286'
const sickle_factory_address = '0x53d9780DbD3831E3A797Fd215be4131636cD5FDf'
const sweep_address = '0x8a09B9784E19De39Bf40106726541C6C09eec40D'

// Avalanche providers can restrict eth_getLogs range/size.
// Prefer explorer-style NFT tx indexing via our server proxy (Snowtrace Etherscan-like for 43114).
// Keep this page consistent with Base: rely on proxy discovery.
...
async function getErc20MetaCached(App, cache, tokenAddress, chainId) {
  return uniswapV4Helpers().getErc20MetaCached(App, cache, tokenAddress, chainId, { nativeSymbol: 'AVAX', nativeDecimals: 18 })
}
...
const withdraw_nfts = async function(App, nft_manager_v4, nft_manager_address_v4, sickleAddress, retry = 0) {
      ...
      if (active_nfts.length > 0) {
        ...
        for (const p of positions) {
          ...
          const token0Meta = await getErc20MetaCached(App, tokenMetaCache, p.poolKey.currency0, AVAX_CHAIN_ID)
          const token1Meta = await getErc20MetaCached(App, tokenMetaCache, p.poolKey.currency1, AVAX_CHAIN_ID)
        ...
        for (const [poolId32, infos] of groups.entries()) {
          ...
          for (const [idx, info] of infos.entries()) {
            ...
            const exitToAvax = async function() {
            ...
            idx < infos.length -1 ? _print_inline(`|    `) : _print_inline(`     `)
            _print_link(`Exit to AVAX`, exitToAvax)
...
const fallback_withdraw = async function(App, nft_manager_address_v4, sickleAddress) {
  ...
  _print(
    `<a target="_blank" href="https://snowtrace.io/token/${nft_manager_address_v4}?a=${sickleAddress}#inventory">Nft Position Manager</a>`
  )
  _print('')
