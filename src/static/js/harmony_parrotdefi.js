...
async function main() {
 ...
 const PPEGG_CHEF_ADDR = "0xfb15945e38a11450af5e3ff20355d71da72ffe8a";
 const rewardTokenTicker = "1PEGG";
 const PPEGG_CHEF = new ethers.Contract(PPEGG_CHEF_ADDR, PPEGG_CHEF_ABI, App.provider);

 const startBlock = await PPEGG_CHEF.startBlock();
 const currentBlock = await App.provider.getBlockNumber();

 let rewardsPerWeek = 0
 if(currentBlock < startBlock){
  _print(`Launch 30/11, start at block ${startBlock}\n`)
 }else{
  rewardsPerWeek = await PPEGG_CHEF.EggPerBlock() /1e18
    * 604800 / 2.2;
 }
  const tokens = {};
  const prices = await getHarmonyPrices();

  await loadHarmonyParrotChefContract(App, tokens, prices, PPEGG_CHEF, PPEGG_CHEF_ADDR, PPEGG_CHEF_ABI, rewardTokenTicker,
      "egg", null, rewardsPerWeek, "pendingEgg");

  hideLoading();
}

async function loadHarmonyParrotChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  //const magicWethLP = await getHarmonyToken(App, "0x8eDfb54E8c76C512DBdD4FAa252Bf4E163CaAD32", chefAddress);
  //getPoolPrices(tokens, prices, magicWethLP, "harmony")
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printChefPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
        ...
        pendingRewardsFunction, null, claimFunction, "harmony", poolInfos[i].depositFee, poolInfos[i].withdrawFee)
      aprs.push(apr);
    }
  }
