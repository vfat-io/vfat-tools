...
async function main() {
    ...
    const STG_CHEF_ADDR = "0x4a364f8c717cAAD9A442737Eb7b8A55cc6cf18D8";
    const rewardTokenTicker = "STG";
    const STG_CHEF = new ethers.Contract(STG_CHEF_ADDR, STG_CHEF_ABI, App.provider);
    const blocksPerSeconds = await getAverageBlockTime(App);
    const rewardsPerWeek = await STG_CHEF.stargatePerBlock() /1e18 * 604800 / blocksPerSeconds;

    const tokens = {};
    const prices = await getOptimisticPrices();
    prices["0xDecC0c09c3B5f6e92EF4184125D5648a66E35298"] = { usd : 1 };
...
async function loadStgChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    ...
    await getStgPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
