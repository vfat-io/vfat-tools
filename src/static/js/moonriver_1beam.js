...
async function main() {
  ...
  const MINI_CHEF_ADDR_V2 = '0xFfC788A463c3ED321c38A867253782097bC54289'
  const rewardTokenAddress = '0x3516a7588C2E6FFA66C9507eF51853eb85d76e5B'
  const MINI_CHEF = new ethers.Contract(MINI_CHEF_ADDR, MINI_CHEF_ABI, App.provider)
  const MINI_CHEF_V2 = new ethers.Contract(MINI_CHEF_ADDR_V2, MINI_CHEF_ABI_V2, App.provider)

  const rewardsPerWeek = ((await MINI_CHEF.rewardPerSecond()) / 1e18) * 604800
  const rewardsPerWeekV2 = ((await MINI_CHEF_V2.rewardPerSecond()) / 1e18) * 604800
  ...
  async function load1swapChefContract(
    ...
    _print_bold(
      `<a href='https://blockscout.moonriver.moonbeam.network/address/${chefAddress}' target='_blank'>Staking Contract ${chefVersion}</a>`
    )
