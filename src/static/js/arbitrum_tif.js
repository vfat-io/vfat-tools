...
const Pools = [
  "0x838ad542B22BA6f3c67f3c3b6BaB18AC51AdD48b",  // TIF
  "0x806B375b2A25A099Ab35a528B5f1E9edd503Db63", // TIF-ETH
  "0x4AB95DbC6b9a5a6CCC12c4411e00Aaa0462efd00", // TIF-USDC
  "0xC703A884e9EaDaF0cF4B640B94273473C18F9360", // ETH
  "0xda378504E84146F6811a8A23eC2ce8A72Fecfd8F", // USDC
  "0x6aeEf8d31A068150852156789bfC2b80505CaBc6", // NYAN
].map(a => ({
      address: a,
      abi: NYAN_ABI,
...
async function main() {
  ...
  let p = await loadMultipleArbitrumTifSynthetixPools(App, tokens, prices, Pools)
...
async function loadMultipleArbitrumTifSynthetixPools(App, tokens, prices, pools) {
  let totalStaked  = 0, totalUserStaked = 0, individualAPRs = [];
  const infos = await Promise.all(pools.map(p =>
      loadArbitrumTifSynthetixPoolInfo(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction)));
  for (const i of infos) {
    let p = await printTifSynthetixPool(App, i, "arbitrum");
...
async function loadArbitrumTifSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
  rewardTokenFunction, stakeTokenFunction) {
    ...
    const calls = [STAKING_MULTI.periodFinish(), STAKING_MULTI.rewardRate(),
      ...
      STAKING_MULTI.depositFee()]
    const [periodFinish, rewardRate, balance, earned_, depositFee_] = await App.ethcallProvider.all(calls);
    ...
    const depositFee = depositFee_ / 10000;
...
async function printTifSynthetixPool(App, info, chain="eth", customURLs) {
