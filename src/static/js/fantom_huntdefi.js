...
async function main() {
   ...
   const HUNT_CHEF_ADDR = "0xEbc74F85ad595Ac9efABfce5Df95e27d7Da887C8";

   const rewardTokenTicker = "HUNT";
   const HUNT_CHEF = new ethers.Contract(HUNT_CHEF_ADDR, HUNT_CHEF_ABI, App.provider);

   let rewardsPerWeek = 0
   const startBlock = await HUNT_CHEF.startBlock();
   ...
   if(currentBlock < startBlock){
    ...
    rewardsPerWeek = await HUNT_CHEF.HuntPerBlock() /1e18 * 604800;
   }

    const tokens = {};
    const prices = await getFantomPrices();

    await loadHuntChefContract(App, tokens, prices, HUNT_CHEF, HUNT_CHEF_ADDR, HUNT_CHEF_ABI, rewardTokenTicker,
      "token", null, rewardsPerWeek, "pendingHunt");

    hideLoading();
  }

async function getHuntPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
async function loadHuntChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getHuntPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
