...
async function getBscCurveToken2(App, curve, address, stakingAddress, minterAddress) {
  const minter = new ethers.Contract(minterAddress, MINTER_BSC_ABI, App.provider)
  const virtualPrice = await minter.virtualPoolValue();
  const coin0 = await minter.allAssets(0);
  const token = await getBscToken(App, coin0, address);
  const decimals = await curve.decimals();
  const staked = await curve.balanceOf(stakingAddress);
  const unstaked = await curve.balanceOf(App.YOUR_ADDRESS)
...
async function getBscStoredToken(App, tokenAddress, stakingAddress, type) {
  switch (type) {
    ...
    case "bscCurve2":
      const crv2 = new ethers.Contract(tokenAddress, BSC_CURVE_ABI, App.provider);
      const minter2 = await crv2.minter();
      return await getBscCurveToken2(App, crv2, tokenAddress, stakingAddress, minter2);
  }
}
...
async function getBscToken(App, tokenAddress, stakingAddress) {
    ...
    try {
      const crv2 = new ethers.Contract(tokenAddress, BSC_CURVE_ABI, App.provider);
      const minter2 = await crv2.minter()
      const res = await getBscCurveToken2(App, crv2, tokenAddress, stakingAddress, minter2);
      window.localStorage.setItem(tokenAddress, "bscCurve2");
      return res;
    }
    catch(err) {
      //console.log(err)
