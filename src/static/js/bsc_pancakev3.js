...
const NFT_FARM_STRATEGY_ADDRESS = "0xfD2D0dE5A902D19b3823D31F1BCe9D8f70D850e7";
const FARM_STRATEGY_ADDRESS = "0x282F8343eEF2aDfE00FE3Fef0755098Fc5983027";
...
async function main() {
  ...
  await loadPancakeV3Contract(App, prices, CAKE_V3_CHEF, CAKE_V3_CHEF_ADDR, CAKE_V3_CHEF_ABI, rewardTokenTicker, "0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82", sickle_account_address, has_sickle_account, owner_sickle_address);
...
function printCakeV3ContractLinks(App, chefAbi, chefAddr, positionInfos, rewardTokenTicker, rewardTokenPrice, has_sickle_account, owner_sickle_address, prices) {
  ...
  for (const [index, info] of positionInfos.entries() || []) {
    ...
    if (has_sickle_account) {
      ...
      index < positionInfos.length -1 ? _print_inline(`|    `) : _print_inline(`     `)
      _print_link(
        `Exit to Underlying (${amount0.toFixed(4)} ${token0Symbol} + ${amount1.toFixed(4)} ${token1Symbol})`,
        () => sickle_exitToUnderlying(info, userNftId)
      )
      index < positionInfos.length -1 ? _print_inline(`|    `) : _print_inline(`     `)
      _print_link(`Exit to 'BNB'`, () => sickle_exitToToken(info, userNftId))
      index < positionInfos.length -1 ? _print_inline(`|    `) : _print_inline(`     `)
      _print_link(
      ...
      index < positionInfos.length -1 ? _print_inline(`|    `) : _print_inline(`     `)
      _print_link(
        `Compound, ${pendingCakes.toFixed(6)} ($${formatMoney(earningsUsd)})`,
        () => sickle_compound(info, userNftId)
      )
    } else {
      index < positionInfos.length -1 ? _print_inline(`|    `) : _print_inline(`     `)
...
function sickle_pancakeV3Contract_unstake(nft_id, App, token0Address, token1Address){
  ...
  const reward_token_address = "0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82";
