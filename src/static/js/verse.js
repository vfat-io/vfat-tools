...
const Pools = [
  ...
  "0xc5af93687088c28da839371f3249df757b219aa8",
  "0x17bdceec80d3506e384db09e5d5696edf70605ef"
].map(a => {
  return {
    address: a,
    abi: VERSE_ABI,
...
async function main() {
  ...
  let p = await loadVerseSynthetixPools(App, tokens, prices, Pools)
...
async function loadVerseSynthetixPools(App, tokens, prices, pools) {
  let totalStaked  = 0, totalUserStaked = 0, individualAPRs = [];
  const infos = await Promise.all(pools.map(p =>
    loadVersePoolInfo(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction)));
...
async function loadVersePoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
  rewardTokenFunction, stakeTokenFunction) {
    ...
    const calls = [STAKING_MULTI.periodFinished(), STAKING_MULTI.rewardRate(),
