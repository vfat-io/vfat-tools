...
async function loadSdaoContract(App, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) => {
    try {
      return await getSdaoPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction, showAll);
    }
    catch (ex) {
  ...
  for (let i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printSdaoPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
async function getSdaoPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction, showAll=false) {
  ...
  const rewardsPerWeek = poolInfo.tokenPerBlock / 1e18 * 604800 / 13.5;
...
function printSdaoPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
