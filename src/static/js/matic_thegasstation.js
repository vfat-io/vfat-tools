...
async function getGasStationMaticPoolInfo(app, poolContract, poolAddress, rewardToken) {
    ...
    const poolToken = await getMaticToken(app, stakeToken, poolAddress);
...
function printGasStationMaticPoolLinks(App, poolAbi, poolAddr, poolAddress, rewardTokenTicker,
...
function printGasStationMaticPool(App, poolAbi, poolAddr, prices, tokens, poolInfo, poolPrices,
                                        rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
                                        fixedDecimals, depositFee=0, withdrawFee=0) {
    fixedDecimals = fixedDecimals ?? 2;
    const sp = (poolInfo.stakedToken == null) ? null : getPoolPrices(tokens, prices, poolInfo.stakedToken, "matic");
    ...
    printGasStationMaticPoolLinks(App, poolAbi, poolAddr, poolInfo.address,
...
async function main() {
    ...
    const lpToken = await getMaticToken(App, "0x2637cE16E98fCc66F2CCdD36087DEFdCf955b68A", "0x0000000000000000000000000000000000000000");
    ...
    for (const pool of pools['polygon']) {
        _print("=========================================================\n");
        ...
        const rewardToken = await getMaticToken(App, rewardTokenAddress, pool.address);
        const rewardsPerWeek = (await poolContract.rewardPerBlock()) / 10 ** rewardToken.decimals * 604800 / 3

        const poolInfo = await getGasStationMaticPoolInfo(App, poolContract, pool.address, rewardToken);

        await Promise.all(poolInfo.poolToken.tokens.map(async (address) => {
            tokens[address] = await getMaticToken(App, address, pool.address);
        }));

        const poolPrice = getPoolPrices(tokens, prices, poolInfo.poolToken, "matic");

        const apr = printGasStationMaticPool(App, POOL_ABI, pool.address, prices, tokens, poolInfo, poolPrice,
