...
// TODO update to arbitrum etherscan when it will launch
function getTokenUrl(addr) {
    return `https://explorer.arbitrum.io/address/${addr}`
}
    ...
    if (myTotalStakeUsd > 0) {
      for (const { tokensPerInterval, tokenPrice, tokenSymbol } of rewards) {
          ...
          _print(
              ...
              + ` Day ${dailyEarnings.toFixed(2)} ($${formatMoney(dailyEarnings * tokenPrice)})`
              + ` Week ${weeklyEarnings.toFixed(2)} ($${formatMoney(weeklyEarnings * tokenPrice)})`
              + ` Year ${yearlyEarnings.toFixed(2)} ($${formatMoney(yearlyEarnings * tokenPrice)})`
          )
      }
    }
    _print('')
...
async function main() {
    ...
    const [
      ...
      glpManager.getAumInUsdg().then(v => v / 1e18)
    ])
    ...
    await loadYieldFarm(App, [
        {tracker: stakedGlpTracker, tokenSymbol: 'GMX', tokenPrice: gmxPrice},
