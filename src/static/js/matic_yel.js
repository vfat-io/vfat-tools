...
async function main() {
    ...
    const YEL_CHEF_ADDR0 = "0x954b15065e4FA1243Cd45a020766511b68Ea9b6E";
    const YEL_CHEF_ADDR1 = "0x52E5EfCdc71DbE15F18Db3b340Ae5738c6c0e939";
    const rewardTokenTicker = "YEL";
    const YEL_CHEF0 = new ethers.Contract(YEL_CHEF_ADDR0, YEL_CHEF_ABI, App.provider);
    const YEL_CHEF1 = new ethers.Contract(YEL_CHEF_ADDR1, YEL_CHEF_ABI, App.provider);

    const rewardsPerWeek0 = await YEL_CHEF0.yelPerSecond() / 1e18 *
        604800;
    const rewardsPerWeek1 = await YEL_CHEF1.yelPerSecond() / 1e18 *
        604800;

    const tokens = {};
    const prices = await getMaticPrices();

    await loadMaticYelChefContract(App, tokens, prices, YEL_CHEF0, YEL_CHEF_ADDR0, YEL_CHEF_ABI, rewardTokenTicker,
        "yel", null, rewardsPerWeek0, "pendingYel", [1]);
    await loadMaticYelChefContract(App, tokens, prices, YEL_CHEF1, YEL_CHEF_ADDR1, YEL_CHEF_ABI, rewardTokenTicker,
        "yel", null, rewardsPerWeek1, "pendingYel");

    hideLoading();
}

async function loadMaticYelChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getMaticYelPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
