$(function () {		
  ...
  async function main() {		
      ...
      const StakingPool = {
        address : "0x32c7bb562e7ecc15bed153ea731bc371dc7ff379",
        abi : _3OM_STAKING_ABI,
        stakeTokenFunction : "share",
        rewardTokenAddresses : "tomb"
      }
      
      const _3SHARES_CHEF_ADDR = "0x1040085d268253e8d4f932399a8019f527e58d04";
      const rewardTokenTicker = "3SHARES";
      const _3SHARES_CHEF = new ethers.Contract(_3SHARES_CHEF_ADDR, _3SHARES_CHEF_ABI, App.provider);

      const rewardsPerWeek = await _3SHARES_CHEF.tSharePerSecond() / 1e18 * 604800;
          
      const tokens = {};
      const prices = await getFantomPrices();
      
      await loadFantom3SharesContract(App, tokens, prices, _3SHARES_CHEF, _3SHARES_CHEF_ADDR, _3SHARES_CHEF_ABI, rewardTokenTicker,		
...
async function loadFantom3omSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadFantom3omSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await print3omSynthetixPool(App, info, "fantom");
}

async function loadFantom3omSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function print3omSynthetixPool(App, info, chain="eth", customURLs) {
...
async function loadFantom3SharesContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
