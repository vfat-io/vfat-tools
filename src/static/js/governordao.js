...
async function main() {
  ...
  await loadXGDAO(App, prices);

  hideLoading();
}

async function loadXGDAO(App, prices){
  const xGDAO_ADDR = "0x306978DA6ebEe060375f35418744F85C1CB6b353";
  const GDAO_ADDR = "0x515d7E9D75E2b76DB60F8a051Cd890eBa23286Bc";
  const xGDAO_CONTRACT = new ethers.Contract(xGDAO_ADDR, xGDAO_TOKEN_ABI, App.provider);
  const GDAO_CONTRACT = new ethers.Contract(GDAO_ADDR, GDAO_TOKEN_ABI, App.provider);
  const totalDepositedGdaos = await GDAO_CONTRACT.balanceOf(xGDAO_ADDR) / 1e18;
  const totalXGdaoTokens = await xGDAO_CONTRACT.totalSupply() / 1e18;
  const virtualPrice = totalDepositedGdaos / totalXGdaoTokens;
  const gdaoPrice = getParameterCaseInsensitive(prices, GDAO_ADDR).usd;
  const xGdaoPrice = gdaoPrice * virtualPrice;
  const rewardTicker = await xGDAO_CONTRACT.symbol();
  const stakeTicker = await GDAO_CONTRACT.symbol();
  const totalOwnedXGdao = await xGDAO_CONTRACT.balanceOf(App.YOUR_ADDRESS) / 1e18;
  const totalOwnedGdao = totalOwnedXGdao * virtualPrice;
  const tvl = totalDepositedGdaos * gdaoPrice
  const usersPercentage = totalOwnedXGdao / totalXGdaoTokens * 100;
  const usersXGdaoUsd = totalOwnedXGdao * gdaoPrice;
  _print(`\n${rewardTicker} Price: $${toFixed(xGdaoPrice, 2)} Total Supply: ${toFixed(totalXGdaoTokens, 4)} TVL: $${formatMoney(tvl)}`);
  _print(`${stakeTicker} Price: $${toFixed(gdaoPrice, 2)}`);
  _print(`There is a total of ${toFixed(totalDepositedGdaos, 2)} GDAO staked in xGDAO`);
  _print(`Your balance is ${totalOwnedXGdao.toFixed(2)} ${rewardTicker} (${totalOwnedGdao.toFixed(2)} ${stakeTicker}), $${formatMoney(usersXGdaoUsd)}, ${usersPercentage.toFixed(2)}% of the pool`);
  const approveAndStake = async function() {
    return contract_stake(xGDAO_TOKEN_ABI, xGDAO_ADDR, App, GDAO_ADDR)
  }
  const unstake = async function() {
    return contract_unstake(xGDAO_TOKEN_ABI, xGDAO_ADDR, totalOwnedXGdao, App)
  }
  _print_link(`Stake ${totalOwnedGdao.toFixed(2)} ${stakeTicker}`, approveAndStake)
  _print_link(`Unstake ${totalOwnedXGdao.toFixed(2)} ${rewardTicker}`, unstake)
}
...
const contract_unstake = async function(abi, contractAddress, stakedAmount, App) {
  const signer = App.provider.getSigner()
  const xGDAO_WRITE_CONTRACT = new ethers.Contract(contractAddress, abi, signer)
  showLoading()
  xGDAO_WRITE_CONTRACT.leave(stakedAmount, {gasLimit: 500000})
      .then(function(t) {
        return App.provider.waitForTransaction(t.hash)
      })
      .catch(function() {
