...
async function main() {
    ...
    const StakingPool = {
      address : "0xE5A972378298BF81110eE9DcBA759fB6038658e5",
      abi : ATLANTIS_STAKING_ABI,
      stakeTokenFunction : "share",
      rewardTokenAddresses : "peg"
    }

    const StakingPool1 = {
      address : "0x8e4d240f343d35c523795335ee7497e3a3c7fc3f",
      abi : ATLANTIS_STAKING_ABI,
      stakeTokenFunction : "share",
      rewardTokenAddresses : "peg"
    }
    ...
    const StakingPool2 = {
      address : "0xf9e2369b2c71604fba7ace493060b16b28f546b2",
      abi : ATLANTIS_STAKING_ABI,
      stakeTokenFunction : "share",
      rewardTokenAddresses : "peg"
    }

   const WAHLE_CHEF_ADDR = "0x73369f8b7ae2665c8ff464a95fa413b7b18e7b2e";
   const rewardTokenTicker = "WAHLE";
   const WAHLE_CHEF = new ethers.Contract(WAHLE_CHEF_ADDR, WAHLE_CHEF_ABI, App.provider);

  const rewardsPerWeek = await WAHLE_CHEF.govPerSecond() / 1e18 * 604800;

    const tokens = {};
    const prices = await getArbitrumPrices();

    await loadWhaleChefContract(App, tokens, prices, WAHLE_CHEF, WAHLE_CHEF_ADDR, WAHLE_CHEF_ABI, rewardTokenTicker,
        "gov", null, rewardsPerWeek, "pendingShare");

        _print(`\nStaked WHALE can be withdrawn 36 hours after deposit.\n`);
        _print(`aFishV2 Earnings.\n`)
        const p0 = await loadAtlantisSynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                        StakingPool.address, 
      ...
      _print(`\nStaked WHALE can be withdrawn 36 hours after deposit.\n`);
      _print(`AETH Earnings.\n`)
      const p1 = await loadAtlantisSynthetixPool(App, tokens, prices, StakingPool1.abi, 
    ...
    if (p1.userStaked > 0) {
      _print(`You are staking a total of $${formatMoney(p1.userStaked)}\n`);
    }

    _print(`\nStaked WHALE can be withdrawn 36 hours after deposit.\n`);
      _print(`AZYB Earnings.\n`)
      const p2 = await loadAtlantisSynthetixPool(App, tokens, prices, StakingPool2.abi, 
    ...
    if (p2.userStaked > 0) {
      _print(`You are staking a total of $${formatMoney(p2.userStaked)}\n`);
    }
    hideLoading();
  }

async function loadAtlantisSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadAtlantisSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printAtlantisSynthetixPool(App, info, "arbitrum");
}

async function loadAtlantisSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printAtlantisSynthetixPool(App, info, chain="eth", customURLs) {
...
async function loadWhaleChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
