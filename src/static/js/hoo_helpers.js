...
async function getHoo20(App, token, address, stakingAddress) {
...
async function getHooVault(App, vault, address, stakingAddress) {
  const decimals = await vault.decimals();
  const token_ = await vault.token();
  const token = await getHooToken(App, token_, address);
  return {
    address,
...
async function getHooSaddleToken(App, saddle, address, stakingAddress, swapAddress) {
  const swap = new ethers.Contract(swapAddress, HOO_SADDLE_SWAP_ABI, App.provider)
  const virtualPrice = await swap.getVirtualPrice();
  const coin0 = await swap.getToken(0);
  const token = await getHooToken(App, coin0, address);
...
async function getHooAdamantVault(App, vault, address, stakingAddress) {
  const token_ = await vault.token();
  const token = await getHooToken(App, token_, address);
  return {
    address,
...
async function getHooWantVault(App, vault, address, stakingAddress) {
  const decimals = await vault.decimals();
...
async function getCHooToken(App, cToken, address, stakingAddress) {
  const calls = [
    cToken.decimals(),
    cToken.underlying(),
    cToken.totalSupply(),
  ...
  const token = await getHooToken(App, underlying, address)
  return {
    address,
    name,
    symbol,
...
async function getHooBhToken(App, vault, address, stakingAddress) {
  const decimals = await vault.decimals();
...
async function getHooStoredToken(App, tokenAddress, stakingAddress, type) {
  switch (type) {
    case "uniswap":
      const pool = new ethers.Contract(tokenAddress, UNI_ABI, App.provider);
      return await getHooUniPool(App, pool, tokenAddress, stakingAddress);
    case "hooVault":
      const vault = new ethers.Contract(tokenAddress, HOO_VAULT_TOKEN_ABI, App.provider);
      return await getHooVault(App, vault, tokenAddress, stakingAddress);
    case "bhToken":
      const bhToken = new ethers.Contract(tokenAddress, HOO_BH_TOKEN_ABI, App.provider);
      return await getHooBhToken(App, bhToken, tokenAddress, stakingAddress);
    case "hooAdamantVault":
      const adamantVault = new ethers.Contract(tokenAddress, HOO_ADAMANT_VAULT_TOKEN_ABI, App.provider);
      return await getHooAdamantVault(App, adamantVault, tokenAddress, stakingAddress);
    case "hooWantVault":
      const wantVault = new ethers.Contract(tokenAddress, HOO_VAULT_WANT_ABI, App.provider);
      return await getHooWantVault(App, wantVault, tokenAddress, stakingAddress);
    case "curve":
      const crv = new ethers.Contract(tokenAddress, HOO_CURVE_ABI, App.provider);
      if (tokenAddress.toLowerCase() == "0x88E11412BB21d137C217fd8b73982Dc0ED3665d7".toLowerCase()) {
        ...
        return await getCurveHooToken(App, crv, tokenAddress, stakingAddress, minter);
      }
      const minter = await crv.minter();
      return await getCurveHooToken(App, crv, tokenAddress, stakingAddress, minter);
    case "hooSaddle":
      const saddle = new ethers.Contract(tokenAddress, HOO_SADDLE_LP_TOKEN_ABI, App.provider);
      const swap = await saddle.swap();
      return await getHooSaddleToken(App, saddle, tokenAddress, stakingAddress, swap);
    case 'cToken':
      const cHooToken = new ethcall.Contract(tokenAddress, CTOKEN_ABI)
      return await getCHooToken(App, cHooToken, tokenAddress, stakingAddress)
    case "erc20":
      const erc20 = new ethers.Contract(tokenAddress, ERC20_ABI, App.provider);
      return await getHoo20(App, erc20, tokenAddress, stakingAddress);
  }
}

async function getHooToken(App, tokenAddress, stakingAddress) {
    if (tokenAddress == "0x0000000000000000000000000000000000000000") {
      return getHoo20(App, null, tokenAddress, "")
    }
    const type = window.localStorage.getItem(tokenAddress);
    if (type) return getHooStoredToken(App, tokenAddress, stakingAddress, type);
    try {
      const pool = new ethers.Contract(tokenAddress, UNI_ABI, App.provider);
      const _token0 = await pool.token0();
      const uniPool = await getHooUniPool(App, pool, tokenAddress, stakingAddress);
    ...
    try {
      ...
      if (tokenAddress.toLowerCase() == "0x0a2ad1e60bcf5f812a2c74ec519822da36f86617".toLowerCase()) {
        ...
        const res = await getCurveHooToken(App, crv, tokenAddress, stakingAddress, minter);
    ...
    try {
      ...
      const vault = await getHooVault(App, VAULT, tokenAddress, stakingAddress);
      window.localStorage.setItem(tokenAddress, "hooVault");
      return vault;
    }
    catch(err) {
    }
    try {
      const BH = new ethers.Contract(tokenAddress, HOO_BH_TOKEN_ABI, App.provider);
      const _token = await BH.LUSD();
      const bhToken = await getHooBhToken(App, BH, tokenAddress, stakingAddress);
    ...
    try {
      ...
      const res = await getCHooToken(App, cToken, tokenAddress, stakingAddress)
      window.localStorage.setItem(tokenAddress, 'cToken')
      return res
    } catch (err) {}
    try {
      const ADAMANT_VAULT = new ethers.Contract(tokenAddress, HOO_ADAMANT_VAULT_TOKEN_ABI, App.provider);
      const _totalShares = await ADAMANT_VAULT.totalShares();
      const vault = await getHooAdamantVault(App, ADAMANT_VAULT, tokenAddress, stakingAddress);
      window.localStorage.setItem(tokenAddress, "hooAdamantVault");
      return vault;
    }
    catch(err) {
    }
    try {
      const WANT_VAULT = new ethers.Contract(tokenAddress, HOO_VAULT_WANT_ABI, App.provider);
      const _want = await await WANT_VAULT.want();
      const wantVault = await getHooWantVault(App, WANT_VAULT, tokenAddress, stakingAddress);
      window.localStorage.setItem(tokenAddress, "hooWantVault");
      return wantVault;
    }
    catch(err) {
    }
    try {
      const saddle = new ethers.Contract(tokenAddress, HOO_SADDLE_LP_TOKEN_ABI, App.provider);
      const swap = await saddle.swap();
      const res = await getHooSaddleToken(App, saddle, tokenAddress, stakingAddress, swap);
      window.localStorage.setItem(tokenAddress, "hooSaddle");
      return res;
    }
    catch(err) {
    }
    try {
      ...
      const erc20tok = await getHoo20(App, erc20, tokenAddress, stakingAddress);
...
async function getCurveHooToken(app, curve, address, stakingAddress, minterAddress) {
...
async function loadHooSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
    rewardTokenFunction, stakeTokenFunction) {
      ...
      var stakeToken = await getHooToken(App, stakeTokenAddress, stakingAddress);
      ...
      for (const address of newTokenAddresses) {
          tokens[address] = await getHooToken(App, address, stakingAddress);
      }
      if (!getParameterCaseInsensitive(tokens, rewardTokenAddress)) {
          tokens[rewardTokenAddress] = await getHooToken(App, rewardTokenAddress, stakingAddress);
      }
      const rewardToken = getParameterCaseInsensitive(tokens, rewardTokenAddress);

      const rewardTokenTicker = rewardToken.symbol;

      const poolPrices = getPoolPrices(tokens, prices, stakeToken, "hoo");
...
async function loadHooSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
    const info = await loadHooSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
    if (!info) return null;
    return await printSynthetixPool(App, info, "hoo");
}

async function loadHooBasisFork(data) {
...
async function getHooPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  ...
  const poolToken = await getHooToken(app, poolInfo.lpToken ?? poolInfo.stakingToken ?? poolInfo.token, chefAddress);
...
async function loadHooChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  _print(`<a href='https://hooscan.com/address/${chefAddress}' target='_blank'>Staking Contract</a>`);
  ...
  const rewardToken = await getHooToken(App, rewardTokenAddress, chefAddress);
  const rewardsPerWeek = rewardsPerWeekFixed ??
    await chefContract.callStatic[rewardsPerBlockFunction]()
    / 10 ** rewardToken.decimals * 604800 / blocksPerSeconds

  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getHooPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));

  var tokenAddresses = [].concat.apply([], poolInfos.filter(x => x.poolToken).map(x => x.poolToken.tokens));

  await Promise.all(tokenAddresses.map(async (address) => {
      tokens[address] = await getHooToken(App, address, chefAddress);
  }));

  if (deathPoolIndices) {   //load prices for the deathpool assets
    deathPoolIndices.map(i => poolInfos[i])
                     .map(poolInfo =>
      poolInfo.poolToken ? getPoolPrices(tokens, prices, poolInfo.poolToken, "hoo") : undefined);
  }

  const poolPrices = poolInfos.map(poolInfo => poolInfo.poolToken ? getPoolPrices(tokens, prices, poolInfo.poolToken, "hoo") : undefined);
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printChefPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
        ...
        pendingRewardsFunction, null, null, "hoo", poolInfos[i].depositFee, poolInfos[i].withdrawFee)
      aprs.push(apr);
    }
  }
...
async function loadMultipleHooSynthetixPools(App, tokens, prices, pools, customURLs) {
  let totalStaked  = 0, totalUserStaked = 0, individualAPRs = [];
  const infos = await Promise.all(pools.map(p =>
      loadHooSynthetixPoolInfo(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction)));
  for (const i of infos.filter(i => i?.poolPrices)) {
    let p = await printSynthetixPool(App, i, "hoo", customURLs);
...
async function loadMultipleHooSynthetixPoolsSequential(App, tokens, prices, pools) {
  let totalStaked  = 0, totalUserStaked = 0, individualAPRs = [];
  for (const p of pools) {
    let res = await loadHooSynthetixPool(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction);
    if (!res) continue;
