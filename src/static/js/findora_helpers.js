...
async function getFRC20(App, token, address, stakingAddress) {
    if (address == "0x0000000000000000000000000000000000001000") {
      return {
        address,
        name : "Findora",
        symbol : "FRA",
...
async function getFindoraVault(App, vault, address, stakingAddress) {
  const decimals = await vault.decimals();
  const token_ = await vault.token();
  const token = await getFindoraToken(App, token_, address);
  return {
    address,
...
async function getFindoraSwapPool(App, _3pool, address, stakingAddress, swapAddress) {
  const swap = new ethers.Contract(swapAddress, FINDORA_SWAP_ABI, App.provider)
  const virtualPrice = await swap.getVirtualPrice();
  const coin0 = await swap.getToken(0);
  const token = await getFindoraToken(App, coin0, address);
...
async function getFindoraCurveToken(App, curve, address, stakingAddress, minterAddress) {
...
async function getFindoraBalancerPool(App, pool, poolAddress, stakingAddress, tokens, smartToken) {
...
async function getFindoraBunicornTokenPool(App, pool, poolAddress, stakingAddress, tokens, smartToken) {
    const poolData = await getFindoraBalancerPool(App, pool, poolAddress, stakingAddress, tokens, smartToken);

    return { ...poolData, ...{ buniPoolTokens: poolData.poolTokens }}
}

async function getFindoraBunicornStablePool(App, pool, poolAddress, stakingAddress, tokens) {
    let decimals = await pool.decimals();
    let symbol = await pool.symbol();
    let name = await pool.name();
    let totalSupply = await pool.totalSupply();
...
async function getFindoraDlpPool(App, dlpPool, tokenAddress, originTokenAddress, stakingAddress){
  const ownerAddress = await dlpPool._OWNER_();
  const originToken = await getFindoraToken(App, originTokenAddress, ownerAddress);
...
async function getFindoraStoredToken(App, tokenAddress, stakingAddress, type) {
  switch (type) {
    case "uniswap":
      const pool = new ethers.Contract(tokenAddress, UNI_ABI, App.provider);
      return await getFindoraUniPool(App, pool, tokenAddress, stakingAddress);
    case "balancer":
      const bal = new ethers.Contract(tokenAddress, BALANCER_POOL_ABI, App.provider);
      const tokens = await bal.getFinalTokens();
      return await getFindoraBalancerPool(App, bal, tokenAddress, stakingAddress, tokens);
    case "bunicorn:stable":
      ...
      return await getFindoraBunicornStablePool(App, poolStableContract, tokenAddress, stakingAddress, poolStables);
    case "swap":
      const _3pool = new ethers.Contract(tokenAddress, FINDORA_3POOL_ABI, App.provider);
      const swap = await _3pool.swap();
      return await getFindoraSwapPool(App, _3pool, tokenAddress, stakingAddress, swap);
    case "value":
...
async function getFindoraToken(App, tokenAddress, stakingAddress) {
    ...
    try {
      const bal = new ethers.Contract(tokenAddress, BALANCER_POOL_ABI, App.provider);
      const tokens = await bal.getFinalTokens();
      const balPool = await getFindoraBalancerPool(App, bal, tokenAddress, stakingAddress, tokens);
    ...
    try {
      const pool = new ethers.Contract(tokenAddress, FINDORA_DLP_ABI, App.provider);
      const originTokenAddress = await pool.originToken();
      const dlpPool = await getFindoraDlpPool(App, pool, tokenAddress, originTokenAddress, stakingAddress);
    ...
    try {
      ...
      const uniPool = await getFindoraUniPool(App, pool, tokenAddress, stakingAddress);
    ...
    try {
      const _3pool = new ethers.Contract(tokenAddress, FINDORA_3POOL_ABI, App.provider);
      const swap = await _3pool.swap();
      const res = await getFindoraSwapPool(App, _3pool, tokenAddress, stakingAddress, swap);
    ...
    try {
      ...
      const vault = await getFindoraVault(App, VAULT, tokenAddress, stakingAddress);
      window.localStorage.setItem(tokenAddress, "findoraVault");
      return vault;
    }
    catch(err) {
    }
    try {
      const crv = new ethers.Contract(tokenAddress, CURVE_ABI, App.provider);
      const minter = await crv.minter()
      const res = await getFindoraCurveToken(App, crv, tokenAddress, stakingAddress, minter);
      window.localStorage.setItem(tokenAddress, "findoraCurve");
      return res;
    }
    catch(err) {
      //console.log(err)
    ...
    try {
        ...
        const stablePool = await getFindoraBunicornStablePool(App, stablePoolContract, tokenAddress, stakingAddress, tokens);
    ...
    try {
      ...
      const erc20tok = await getFRC20(App, erc20, tokenAddress, stakingAddress);
...
async function loadFindoraSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function loadFindoraSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
    const info = await loadFindoraSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
    return await printSynthetixPool(App, info, "findora");
}

async function loadFindoraBasisFork(data) {
...
async function getFindoraPoolInfo(App, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  ...
  const poolToken = await getFindoraToken(App, poolInfo.lpToken ?? poolInfo.token, chefAddress);
  const userInfo = chefAddress === FORLEND_ADDR ? await chefContract.userInfo(App.YOUR_ADDRESS) : await chefContract.userInfo(poolIndex, App.YOUR_ADDRESS);
  const pendingRewardTokens = chefAddress === FORLEND_ADDR ? await chefContract.callStatic[pendingRewardsFunction](App.YOUR_ADDRESS) : await chefContract.callStatic[pendingRewardsFunction](poolIndex, App.YOUR_ADDRESS);
...
async function loadFindoraChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  let poolCount;
  try {
    poolCount = parseInt(await chefContract.poolLength(), 10);
  } catch {
    try {
    poolCount = parseInt(await chefContract.getPoolLength(), 10);
    } catch (err) {
      try {
        poolCount = 1
      } catch {}
  ...
  try {
    totalAllocPoints = await chefContract.totalAllocPoint();
  } catch {
    totalAllocPoints = 1000;
  }

  _print(`<a href='https://evm.findorascan.io/address/${chefAddress}' target='_blank'>Staking Contract</a>`);
  ...
  const rewardToken = await getFindoraToken(App, rewardTokenAddress, chefAddress);
  const rewardsPerWeek = rewardsPerWeekFixed ??
    await chefContract.callStatic[rewardsPerBlockFunction]()
    / 10 ** rewardToken.decimals * 604800 / 3

  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getFindoraPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));

  var tokenAddresses = [].concat.apply([], poolInfos.filter(x => x.poolToken).map(x => x.poolToken.tokens));

  await Promise.all(tokenAddresses.map(async (address) => {
      tokens[address] = await getFindoraToken(App, address, chefAddress);
  }));

  if (deathPoolIndices) {   //load prices for the deathpool assets
    deathPoolIndices.map(i => poolInfos[i])
                     .map(poolInfo =>
      poolInfo.poolToken ? getPoolPrices(tokens, prices, poolInfo.poolToken, "findora") : undefined);
  }

  const poolPrices = poolInfos.map(poolInfo => poolInfo.poolToken ? getPoolPrices(tokens, prices, poolInfo.poolToken, "findora") : undefined);
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printChefPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
        ...
        pendingRewardsFunction, null, null, "findora", poolInfos[i].depositFee, poolInfos[i].withdrawFee)
      aprs.push(apr);
    }
  }
...
async function loadMultipleFindoraSynthetixPools(App, tokens, prices, pools) {
  let totalStaked  = 0, totalUserStaked = 0, individualAPRs = [];
  const infos = await Promise.all(pools.map(p =>
    loadFindoraSynthetixPoolInfo(App, tokens, prices, p.abi, p.address, p.rewardTokenFunction, p.stakeTokenFunction)));
