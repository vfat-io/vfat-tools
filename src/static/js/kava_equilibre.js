...
async function main() {
  ...
  const VARA_VOTER_ADDR = "0x4eB2B9768da9Ea26E3aBe605c9040bC12F236a59"
  const VARA_VOTER_CONTRACT = new ethcall.Contract(VARA_VOTER_ADDR, VARA_VOTER_ABI);
  const VARA_TOKEN_ADDR = "0xe1da44c0da55b075ae8e2e4b6986adc76ac77d73"
  const [_poolLength] = await App.ethcallProvider.all([VARA_VOTER_CONTRACT.length()]);
  const poolLength = _poolLength / 1;
  const calls = [...Array(poolLength).keys()].map(i => VARA_VOTER_CONTRACT.pools(i));
  const lpTokens = await App.ethcallProvider.all(calls);
  const calls2 = lpTokens.map(a => VARA_VOTER_CONTRACT.gauges(a));
  ...
  //vara/wkava to receive the price
  await loadVaraSynthetixPoolInfoPrice(App, tokens, prices, App.YOUR_ADDRESS, "0x7d8100072ba0e4da8dc6bd258859a5dc1a452e05")

  let p = await loadVaraSynthetixPools(App, tokens, prices, gauges, clicked)
  _print_bold(`Total staked: $${formatMoney(p.staked_tvl)}\n`);
  if (p.totalUserStaked > 0) {
    _print(`You are staking a total of $${formatMoney(p.totalUserStaked)} at an APR of ${(p.totalApr * 100).toFixed(2)}%\n`);
    _print(`Estimated VARA earnings:`
  ...
  const claimAll = async function() {
    const rewardTokenAddresses = p.gaugeAddresses.map((g) => [VARA_TOKEN_ADDR]);
    return varaContract_claimAll(p.gaugeAddresses, rewardTokenAddresses, VARA_VOTER_ADDR, App)
  }
  ...
  _print_link(`Claim All ${p.earnings.toFixed(6)} VARA ($${formatMoney(p.earningsUSD)})\n`, claimAll)
  _print("");
...
const varaContract_claimAll = async function(gaugeAddresses, rewardTokenAddresses, voterAddress, App) {
  const signer = App.provider.getSigner()

  const REWARD_POOL = new ethers.Contract(voterAddress, VARA_VOTER_ABI, signer)
...
const reloadFun = async function (App, tokens, prices, pools, clicked){
  if(clicked == false){
    clicked = true;
    let p = await loadVaraSynthetixPools(App, tokens, prices, pools, clicked)
    if (p.totalUserStaked > 0) {
      _print(`You are staking a total of $${formatMoney(p.totalUserStaked)} at an APR of ${(p.totalApr * 100).toFixed(2)}%\n`);
      _print(`Estimated VARA earnings:`
              + ` Day ${p.totalUserDailyRewards.toFixed(2)} ($${formatMoney(p.totalUserDailyRewardsUSD)})`
...
async function loadVaraSynthetixPools(App, tokens, prices, pools, clicked) {
  ...
  const infos = await Promise.all(pools.map(p =>
      loadVaraSynthetixPoolInfo(App, tokens, prices, p.abi, p.address, p.stakeTokenFunction)));
  for (const i of infos) {
    let p = await printVaraSynthetixPool(App, i, "kava", clicked);
...
async function loadVaraSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
  stakeTokenFunction) {
    ...
    const rewardTokenAddress = "0xe1da44c0da55b075ae8e2e4b6986adc76ac77d73";
    ...
    const poolPrices = getPoolPrices(tokens, prices, stakeToken, "kava");
...
async function printVaraSynthetixPool(App, info, chain="eth", clicked, customURLs) {
  ...
  const claim = async function() {
    return varaContract_claim(info.rewardTokenAddresses[0], info.stakingAddress, App)
  }
...
const varaContract_claim = async function(rewardTokenAddress, rewardPoolAddr, App) {
  const signer = App.provider.getSigner()

  const REWARD_POOL = new ethers.Contract(rewardPoolAddr, VARA_GAUGE_ABI, signer)
...
async function loadVaraSynthetixPoolInfoPrice(App, tokens, prices, stakingAddress, stakeTokenAddress) {
  var stakeToken = await getGeneralToken(App, stakeTokenAddress, stakingAddress);
