...
async function main() {
   ...
   const TAPS_CHEF_ADDR = "0xEDdadBd5C67fE8CBD16b058757778381e228bD8D";
   const rewardTokenTicker = "TAPS";
   const TAPS_CHEF = new ethers.Contract(TAPS_CHEF_ADDR, TAPS_CHEF_ABI, App.provider);

   const startBlock = await TAPS_CHEF.startBlock();
   ...
   if(currentBlock < startBlock){
    ...
    rewardsPerWeek = await TAPS_CHEF.tapsPerBlock() /1e18
        * 604800 / 3;
   }
    ...
    await loadTapContract(App, tokens, prices, TAPS_CHEF, TAPS_CHEF_ADDR, TAPS_CHEF_ABI, rewardTokenTicker,
        "taps", null, rewardsPerWeek, "pendingTaps", [1]);

    hideLoading();
  }

async function loadTapContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getTapInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printTapPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
function printTapPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printTapContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fee, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printTapContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
