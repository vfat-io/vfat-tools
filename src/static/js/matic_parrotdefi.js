...
async function main() {
 ...
 const PPEGG_CHEF_ADDR = "0x34E4cd20F3a4FdC5e42FdB295e5A118D4eEB0b79";
 const rewardTokenTicker = "PPEGG";
 const PPEGG_CHEF = new ethers.Contract(PPEGG_CHEF_ADDR, PPEGG_CHEF_ABI, App.provider);

 let rewardsPerWeek = await PPEGG_CHEF.EggPerBlock() /1e18
    * 604800 / 13.5;

  const tokens = {};
  const prices = await getMaticPrices();

  await loadMaticParrotChefContract(App, tokens, prices, PPEGG_CHEF, PPEGG_CHEF_ADDR, PPEGG_CHEF_ABI, rewardTokenTicker,
      "egg", null, rewardsPerWeek, "pendingEgg");

  hideLoading();
}

async function loadMaticParrotChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  //const magicWethLP = await getMaticToken(App, "0x8eDfb54E8c76C512DBdD4FAa252Bf4E163CaAD32", chefAddress);
  //getPoolPrices(tokens, prices, magicWethLP, "matic")
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printChefPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
        ...
        pendingRewardsFunction, null, claimFunction, "matic", poolInfos[i].depositFee, poolInfos[i].withdrawFee)
      aprs.push(apr);
    }
  }
