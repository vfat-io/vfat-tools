...
const FARM_STRATEGY_ADDRESS = '0xd248aa0B8c6C20276f996a284A8791fF774F2fA0'
const REWARD_TOKEN_ADDRESS = '0xdf002282C1474C9592780618Adda7EaA99998Abd'
...
async function main() {
  ...
  const TOPAZ_FACTORY_ADDRESS_V2 = '0x65E6cD0eF5D3467030103cf3d433034E570b5784'
  const TOPAZ_FACTORY_V2 = new ethcall.Contract(TOPAZ_FACTORY_ADDRESS_V2, TOPAZ_V2_FACTORY_ABI)

  const TOPAZ_VOTER_ADDRESS = '0x2F80F810a114223AC69E34E84E735CaD515dAD67'
  const TOPAZ_VOTER = new ethcall.Contract(TOPAZ_VOTER_ADDRESS, TOPAZ_VOTER_ABI)

  const _length = await App.ethcallProvider.all([TOPAZ_VOTER.length()])
  const length = _length / 1

  const pool_calls = [...Array(length).keys()].map(i => TOPAZ_VOTER.pools(i))
  const pools = await App.ethcallProvider.all(pool_calls)

  const is_v2_pool_calls = pools.map(p => TOPAZ_FACTORY_V2.isPool(p))
  ...
  const gauge_v2_calls = pools_v2.map(pv2 => TOPAZ_VOTER.gauges(pv2))
  ...
  if (has_sickle_account) {
    ...
    await loadTopazV2SynthetixPools(App, tokens, prices, gauges, has_sickle_account, sickle_account_address)
...
async function loadTopazV2SynthetixPools(
  App,
  tokens,
  prices,
  pools,
  ...
  for (const i of infos.filter(i => i?.poolPrices)) {
    await printTopazV2Pool(App, i, 'bsc', customURLs)
  }
}

async function loadTopazV2SynthetixPoolInfo(
  App,
  tokens,
  prices,
  stakingAbi,
  ...
  const rewardTokenAddress = '0xdf002282C1474C9592780618Adda7EaA99998Abd'
...
async function printTopazV2Pool(App, info, chain = 'eth', customURLs) {
  ...
  _print(`<a target="_blank" href="https://bscscan.com/address/${info.stakingAddress}#code">Bnb</a>`)
...
async function getBscPrices2() {
  const BnbTokenContracts = bscTokens.map(x => x.contract.toLowerCase())
  const idPrices = await lookUpPrices2(BnbTokenContracts)
  const prices = {}
  for (const bt of bscTokens) if (idPrices[bt.contract]) prices[bt.contract] = { usd: idPrices[bt.contract] }
  return prices
}
...
const lookUpPrices2 = async function (id_array) {
  ...
  let res = await $.ajax({
    url: 'https://api.vfat.io/v1/token?chainId=56&address=' + ids,
    type: 'GET',
  })
