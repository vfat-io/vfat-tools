$(function() {
  ...
  async function main() {
    ...
    const NEBULA_CHEF = new ethers.Contract(NEBULA_CHEF_ADDR, NEBULA_CHEF_ABI, App.provider);
  
    const blockNumber = await App.provider.getBlockNumber();
    const multiplier = await NEBULA_CHEF.getMultiplier(blockNumber, blockNumber+1);
    const rewardPerBlock = await NEBULA_CHEF.REWARD_PER_BLOCK();
    const rewardsPerWeek = rewardPerBlock / 1e18 * multiplier * 201600
  
    const tokens = {};
    const basePrices = await getHarmonyPrices();
  
    const { prices, totalUserStaked, totalStaked, averageApr } = await loadNebulaChefContract(
      App,
      tokens,
      basePrices,
      NEBULA_CHEF,
    ...
    const NEBULA = new ethers.Contract(NEBULA_TOKEN_ADDR, ERC20_ABI, App.provider.getSigner());
    const NEBULA_INTERSTELLAR = new ethers.Contract(NEBULA_INTERSTELLAR_ADDR, NEBULA_INTERSTELLAR_ABI, App.provider.getSigner());
    const nebulaInterstellar = await nebulaInterstellarData(NEBULA, NEBULA_INTERSTELLAR, App, prices);
  
    const totalStakedInclNebulaInterstellar = totalStaked + nebulaInterstellar.totalBalanceUSD
  
    _print_bold(`Total Staked: $${formatMoney(totalStakedInclNebulaInterstellar)}`);
    ...
    if (nebulaInterstellar.userBalanceUSD > 0) {
      _print(`You are staking a total of $${formatMoney(nebulaInterstellar.userBalanceUSD)} in the ${INTERSTELLAR_NAME} single staking pool.`);
    }
  
    hideLoading();
  }
  
  async function loadNebulaChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
    ...
    for (i = 0; i < poolCount; i++) {
      if (poolPrices[i]) {
        const apr = printNebulaChefPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
  ...
  function printNebulaChefPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                              totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
    ...
    printNebulaChefContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fixedDecimals, claimFunction, rewardPrice, chain, depositFee, withdrawFee);
    return apr;
  }
  
  function printNebulaChefContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
                                       rewardTokenTicker, stakeTokenTicker, unstaked, userStaked, pendingRewardTokens, fixedDecimals,
                                       claimFunction, rewardTokenPrice, chain, depositFee, withdrawFee) {
    fixedDecimals = fixedDecimals ?? 2;
    const approveAndStake = async function() {
      return nebulaContract_stake(chefAbi, chefAddr, poolIndex, poolAddress, App)
    }
    const unstake = async function() {
      return nebulaContract_unstake(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction)
    }
    const claim = async function() {
      return nebulaContract_claim(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction, claimFunction)
    }
    if(depositFee > 0){
  ...
  const nebulaContract_stake = async function(chefAbi, chefAddress, poolIndex, stakeTokenAddr, App) {
  ...
  const nebulaContract_unstake = async function(chefAbi, chefAddress, poolIndex, App, pendingRewardsFunction) {
    const signer = App.provider.getSigner()
    const CHEF_CONTRACT = new ethers.Contract(chefAddress, chefAbi, signer)
  ...
  const nebulaContract_claim = async function(chefAbi, chefAddress, poolIndex, App,
  ...
  async function nebulaInterstellarData(NEBULA, NEBULA_INTERSTELLAR, App, prices) {
    const nebulaInterstellarTotalBalance = await NEBULA.balanceOf(NEBULA_INTERSTELLAR_ADDR) / 1e18;
    const nebulaInterstellarTotalSupply = await NEBULA_INTERSTELLAR.totalSupply() / 1e18;
    const xNebulaRatio = nebulaInterstellarTotalBalance / nebulaInterstellarTotalSupply;
  
    const nebulaInterstellarUserBalanceBigNum = await NEBULA_INTERSTELLAR.balanceOf(App.YOUR_ADDRESS);
    const nebulaInterstellarUserBalance = (nebulaInterstellarUserBalanceBigNum / 1e18) * xNebulaRatio;
    const nebulaInterstellarShare = nebulaInterstellarUserBalance / nebulaInterstellarTotalBalance;
    const nebulaBalanceBigNum = await NEBULA.balanceOf(App.YOUR_ADDRESS);
    const nebulaBalance = nebulaBalanceBigNum / 1e18;
  
    const nebulaPrice = prices[NEBULA_TOKEN_ADDR];
  
    let nebulaInterstellarTotalBalanceUSD;
    let nebulaInterstellarUserBalanceUSD;
  
    if (nebulaPrice && nebulaPrice.usd) {
      nebulaInterstellarTotalBalanceUSD = nebulaInterstellarTotalBalance * nebulaPrice.usd;
      nebulaInterstellarUserBalanceUSD = nebulaInterstellarUserBalance * nebulaPrice.usd;
  
      _print(`${INTERSTELLAR_NAME} TVL: $${formatMoney(nebulaInterstellarTotalBalanceUSD)}`);
      _print(`${REWARD_TOKEN_TICKER} Price: $${formatMoney(nebulaPrice.usd)}`);
      _print(`Staked: ${(nebulaInterstellarTotalBalance).toFixed(2)} ${REWARD_TOKEN_TICKER} ($${formatMoney(nebulaInterstellarTotalBalanceUSD)})`);
      _print(`You are staking ${(nebulaInterstellarUserBalance).toFixed(2)} ${REWARD_TOKEN_TICKER} ($${formatMoney(nebulaInterstellarUserBalanceUSD)}), ${(nebulaInterstellarShare * 100).toFixed(2)}% of the pool.`);
  
      const approveAndEnter = async function() {
        return nebulaInterstellarApproveAndEnter(NEBULA, NEBULA_INTERSTELLAR, nebulaBalanceBigNum, App);
      }
  
      const leave = async function() {
        return nebulaInterstellarLeave(NEBULA_INTERSTELLAR, nebulaInterstellarUserBalanceBigNum, App);
      }
  
      _print_link(`Stake ${(nebulaBalance).toFixed(2)} ${REWARD_TOKEN_TICKER}`, approveAndEnter);
  
      if (nebulaInterstellarUserBalance > 0) {
        _print_link(`Unstake ${nebulaInterstellarUserBalance.toFixed(2)} ${REWARD_TOKEN_TICKER}`, leave);
      }
  
      _print(`${INTERSTELLAR_NAME} rewards are visible after unstaking / exiting the pool.`)
      _print('');
    }
    ...
    return {
      totalBalance: nebulaInterstellarTotalBalance,
      totalBalanceUSD: nebulaInterstellarTotalBalanceUSD,
      userBalance: nebulaInterstellarUserBalance,
      userBalanceUSD: nebulaInterstellarUserBalanceUSD
    }
  }
  
  async function nebulaInterstellarApproveAndEnter(NEBULA, NEBULA_INTERSTELLAR, currentTokens, App) {
    const allowedTokens = await NEBULA.allowance(App.YOUR_ADDRESS, NEBULA_INTERSTELLAR_ADDR)
    let allow = Promise.resolve()
  
    if (allowedTokens / 1e18 < currentTokens / 1e18) {
      showLoading()
      allow = NEBULA.approve(NEBULA_INTERSTELLAR_ADDR, ethers.constants.MaxUint256)
      .then(function(t) {
        return App.provider.waitForTransaction(t.hash)
      })
      .catch(function() {
    ...
    if (currentTokens / 1e18 > 0) {
      ...
      allow
      .then(async function() {
        NEBULA_INTERSTELLAR.enter(currentTokens, {gasLimit: 500000})
        .then(function(t) {
          App.provider.waitForTransaction(t.hash).then(function() {
            hideLoading()
          })
   ...
   async function nebulaInterstellarLeave(NEBULA_INTERSTELLAR, currentStakedAmount, App) {
    showLoading()
    NEBULA_INTERSTELLAR.leave(currentStakedAmount, {gasLimit: 500000})
    .then(function(t) {
      App.provider.waitForTransaction(t.hash).then(function() {
        hideLoading()
      })
