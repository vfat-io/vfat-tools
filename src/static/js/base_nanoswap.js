$(function() {
    ...
    async function main() {
       ...
       const NANO_CHEF_ADDR = "0x042e217938Ba9fdEBAEF67A7DBeDb76eD65C1A80";
       const rewardTokenTicker = "NANO";
       const NANO_CHEF = new ethers.Contract(NANO_CHEF_ADDR, NANO_CHEF_ABI, App.provider);
    
       const rewardsPerWeek = await NANO_CHEF.nanoPerSec() / 1e18 * 604800;
    
        const tokens = {};
        const prices = await getBasePrices();
    
        await loadNanoChefContract(App, tokens, prices, NANO_CHEF, NANO_CHEF_ADDR, NANO_CHEF_ABI, rewardTokenTicker,
            "nano", null, rewardsPerWeek, "pendingTokens", [2], "base");
    
        hideLoading();
      }

async function getNanoPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
async function loadNanoChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getNanoPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
