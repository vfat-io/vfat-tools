...
const FARM_STRATEGY_ADDRESS = '0x5A6584F18f5D454b2CEb0226cF77C87603Aec8F8'
const REWARD_TOKEN_ADDRESS = '0x57C0E45cB534413D1C20A4240955d6bB250BB4F1'
...
async function main() {
  ...
  const tokens = {}
  try {
    prices = await getRobinhoodPrices()
  } catch (e) {
    prices = await getRobinhoodPrices2()
  }

  const SICKLE_FACTORY_ADDR = '0x3575Aa02Ae85D8Cd2AaE6DCaA5D8750cFc9622e6'
  ...
  const UP33_FACTORY_ADDRESS_V2 = '0xFA5429AEBa338BEa2BFcc1b9a889862Ee395bc28'
  const UP33_FACTORY_V2 = new ethcall.Contract(UP33_FACTORY_ADDRESS_V2, UP33_V2_FACTORY_ABI)

  const UP33_VOTER_ADDRESS = '0x7F749fDD351C1Ceed82d76d7699CB631Eb8332a7'
  const UP33_VOTER = new ethcall.Contract(UP33_VOTER_ADDRESS, UP33_VOTER_ABI)

  const _length = await App.ethcallProvider.all([UP33_VOTER.length()])
  const length = _length / 1

  const pool_calls = [...Array(length).keys()].map(i => UP33_VOTER.pools(i))
  const pools = await App.ethcallProvider.all(pool_calls)

  const is_v2_pool_calls = pools.map(p => UP33_FACTORY_V2.isPool(p))
  ...
  const gauge_v2_calls = pools_v2.map(pv2 => UP33_VOTER.gauges(pv2))
  ...
  if (has_sickle_account) {
    ...
    await loadUpV2SynthetixPools(App, tokens, prices, gauges, has_sickle_account, sickle_account_address)
...
async function loadUpV2SynthetixPools(
  App,
  tokens,
  prices,
  pools,
  ...
  for (const i of infos.filter(i => i?.poolPrices)) {
    await printUpV2Pool(App, i, 'robinhood', customURLs)
  }
}

async function loadUpV2SynthetixPoolInfo(
  App,
  tokens,
  prices,
  stakingAbi,
  ...
  const rewardTokenAddress = '0x57C0E45cB534413D1C20A4240955d6bB250BB4F1'
  ...
  const poolPrices = getPoolPrices(tokens, prices, stakeToken, 'robinhood')
...
async function printUpV2Pool(App, info, chain = 'eth', customURLs) {
  ...
  const claim = async function () {
    return bnbContract_claim(info.stakingAddress, App)
  }
  const sickle_bnb_unstake = async function () {
    return sickle_bnbContract_unstake(info.stakingAddress, info.stakeTokenAddress, info._userStaked, App)
  }
  const sickle_bnb_claim = async function () {
    return sickle_bnbContract_claim(info.stakingAddress, App)
  }
  _print(`<a target="_blank" href="https://robinhoodchain.blockscout.com/address/${info.stakingAddress}">Robinhood</a>`)
  if (info.has_sickle_account) {
    _print_link(`Unstake ${info.userStaked.toFixed(6)} ${info.stakeTokenTicker}`, sickle_bnb_unstake)
  } else {
...
const sickle_bnbContract_unstake = async function (rewardPoolAddr, lpToken, userStaked, App) {
...
const sickle_bnbContract_claim = async function (rewardPoolAddr, App) {
  const signer = App.provider.getSigner()
...
async function getRobinhoodPrices2() {
  const RobinhoodTokenContracts = RobinhoodTokens.map(x => x.contract.toLowerCase())
  const idPrices = await lookUpPrices2(RobinhoodTokenContracts)
  const prices = {}
  for (const bt of RobinhoodTokens) if (idPrices[bt.contract]) prices[bt.contract] = { usd: idPrices[bt.contract] }
  return prices
}
...
const lookUpPrices2 = async function (id_array) {
  ...
  let res = await $.ajax({
    url: 'https://api.vfat.io/v1/token?chainId=4663&address=' + ids,
    type: 'GET',
  })
