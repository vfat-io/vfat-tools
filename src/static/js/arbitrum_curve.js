...
async function main() {
    ...
    let p = await loadMultipleCurveArbitrumPools(App, tokens, prices, Pools, CURVE_STAKING_ABI)
  ...
  async function loadMultipleCurveArbitrumPools(App, tokens, prices, pools, abi) {
    let totalStaked  = 0, totalUserStaked = 0, individualAPRs = [];
    const infos = await Promise.all(pools.map(p =>
        loadArbitrumCurvePoolInfo(App, tokens, prices, p, abi)));
    for (const i of infos) {
      let p = await printCurvePool(App, i, "arbitrum");
  ...
  async function loadArbitrumCurvePoolInfo(App, tokens, prices, stakingAddress, stakingAbi) {
      ...
      try{
        ...
        let stakeToken = await getArbitrumToken(App, stakeTokenAddress, rewardReceiver);
        ...
        const rewardReceiverProxy = new ethers.Contract(rewardReceiver, REWRAD_RECEIVER_PROXY_ABI, App.provider);

        const rewardReceiver2 = await rewardReceiverProxy.reward_receiver();

        const rewardReceiverContract = new ethers.Contract(rewardReceiver2, REWARD_RECEIVER_ABI, App.provider);

        const stakeTokenAddress = await rewardReceiverContract.lp_token();

        let stakeToken = await getArbitrumToken(App, stakeTokenAddress, rewardReceiver2);
