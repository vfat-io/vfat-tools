$(function () {		
    ...
    async function main() {		
        ...
        const MINE_CHEF_ADDR = "0x565d328630694fb560a427B07c4167Eb88DE2d4D";		
        const rewardTokenTicker = "MINE";		
        const MINE_CHEF = new ethers.Contract(MINE_CHEF_ADDR, MINE_CHEF_ABI, App.provider);

        const rewardsPerWeek = await MINE_CHEF.minePerSecond() / 1e18 * 604800;
        		
        const tokens = {};		
        const prices = await getCronosPrices();		
    		
        await loadCrystalChefContract(App, tokens, prices, MINE_CHEF, MINE_CHEF_ADDR, MINE_CHEF_ABI, rewardTokenTicker,		
            "mine", null, rewardsPerWeek, "pendingShare", []);		
    		
        hideLoading();		
    }		
    	
async function loadCrystalChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
