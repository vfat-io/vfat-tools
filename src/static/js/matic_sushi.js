...
function printSushiContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
  ...
  const claim = async function() {
    return sushiContract_claim(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction, claimFunction)
  }
...
const sushiContract_stake = async function(chefAbi, chefAddress, poolIndex, stakeTokenAddr, App) {
  ...
  if (currentTokens / 1e18 > 0) {
    ...
    allow
      .then(async function() {
          CHEF_CONTRACT.deposit(poolIndex, currentTokens, chefAddress, {gasLimit: 500000})
          .then(function(t) {
...
const sushiContract_unstake = async function(chefAbi, chefAddress, poolIndex, App, pendingRewardsFunction) {
  ...
  if (earnedTokenAmount > 0) {
    showLoading()
    CHEF_CONTRACT.withdrawAndHarvest(poolIndex, currentStakedAmount, chefAddress, {gasLimit: 500000})
      .then(function(t) {
        return App.provider.waitForTransaction(t.hash)
      })
      .catch(function() {
...
const sushiContract_claim = async function(chefAbi, chefAddress, poolIndex, App,
  ...
  if (earnedTokenAmount > 0) {
    showLoading()
    if (claimFunction) {
      claimFunction(poolIndex, chefAddress, {gasLimit: 500000})
        .then(function(t) {
          return App.provider.waitForTransaction(t.hash)
        })
    }
