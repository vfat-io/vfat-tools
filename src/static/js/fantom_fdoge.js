$(function () {		
  ...
  async function main() {		
      ...
      const StakingPool = {
        address : "0xDd1Fa691D2fd01FE9206b15350462b712B4AE371",
        abi : FDOGE_STAKING_ABI,
        stakeTokenFunction : "share",
        rewardTokenAddresses : "tomb"
      }
      
      const SDOGE_CHEF_ADDR = "0x5331bE243A6AA35253b8bAe3E12157C6F5B61aDE";
      const rewardTokenTicker = "SDOGE";
      const SDOGE_CHEF = new ethers.Contract(SDOGE_CHEF_ADDR, SDOGE_CHEF_ABI, App.provider);

      const rewardsPerWeek = await SDOGE_CHEF.tSharePerSecond() / 1e18 * 604800;
          
      const tokens = {};
      const prices = await getFantomPrices();
      
      await loadFantomSDogeContract(App, tokens, prices, SDOGE_CHEF, SDOGE_CHEF_ADDR, SDOGE_CHEF_ABI, rewardTokenTicker,		
          "tshare", null, rewardsPerWeek, "pendingShare");
      _print(`\nStaked SDOGEs can only be withdrawn after 6 epochs since deposit.\n`);
      const p0 = await loadFantomFDOGESynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                            StakingPool.address, 
...
async function loadFantomFDOGESynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadFantomFDOGESynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printFDOGESynthetixPool(App, info, "fantom");
}

async function loadFantomFDOGESynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printFDOGESynthetixPool(App, info, chain="eth", customURLs) {
...
async function loadFantomSDogeContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
