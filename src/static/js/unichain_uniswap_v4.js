...
// Uniswap v4 Unichain deployments (130): https://docs.uniswap.org/contracts/v4/deployments
const nft_manager_address_v4 = '0x4529A01c7A0410167c5740C487A8DE60232617bf'
const pool_manager_address_v4 = '0x1f98400000000000000000000000000000000004'
// Reading current tick/sqrtPrice is done via StateView, not PoolManager.
const state_view_address_v4 = '0x86e8631a016f9068c3f085faf484ee3f5fdee8f2'
const sickle_factory_address = '0x233D9067677dCf1a161954D45B4C965B9d567168'
const sweep_address = '0x4F025Aba4887631a6D601AE3156bf53568f5A5Fd'
...
const withdraw_nfts = async function(App, nft_manager_v4, nft_manager_address_v4, sickleAddress) {
  ...
  for (const p of positions) {
    ...
    const token0Meta = await getErc20MetaCached(App, tokenMetaCache, p.poolKey.currency0, UNICHAIN_CHAIN_ID)
    const token1Meta = await getErc20MetaCached(App, tokenMetaCache, p.poolKey.currency1, UNICHAIN_CHAIN_ID)
...
const fallback_withdraw = async function(App, nft_manager_address_v4, sickleAddress) {
  ...
  _print('')
  _print('You could check your nfts on the explorer link below')
  _print(
    `<a target="_blank" href="https://unichain.blockscout.com/address/${sickleAddress}?tab=tokens_nfts">Nft Position Manager</a>`
  )
  _print('')
