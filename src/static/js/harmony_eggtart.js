...
const EGGT_CHEF_ABI = [
    {
        "inputs": [
            {
                "internalType": "contract EggtartToken",
                "name": "_eggt",
                "type": "address"
            },
            {
...
async function main() {
  ...
  const multiplier = await EGGT_CHEF.getMultiplier(blockNumber, blockNumber+1);
  const rewardPerBlock = await EGGT_CHEF.cakePerBlock();
