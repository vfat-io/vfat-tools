$(function () {		
    ...
    async function main() {		
        ...
        const StakingPool = {
          address : "0x05cab739fdc0a4ce0642604c78f307c6c543cd6d",
          abi : LION_STAKING_ABI,
          stakeTokenFunction : "share",
          rewardTokenAddresses : "lion"
        }
        
        const TIGER_CHEF_ADDR = "0x44b4a1e8f34bb52ed39854ad218ff94d2d5b4800";
        const rewardTokenTicker = "TIGER";
        const TIGER_CHEF = new ethers.Contract(TIGER_CHEF_ADDR, TIGER_CHEF_ABI, App.provider);
  
        const rewardsPerWeek = await TIGER_CHEF.tSharePerSecond() / 1e18 * 604800;
            
        const tokens = {};
        const prices = await getCronosPrices();
        
        await loadCronosTigerContract(App, tokens, prices, TIGER_CHEF, TIGER_CHEF_ADDR, TIGER_CHEF_ABI, rewardTokenTicker,		
            "tshare", null, rewardsPerWeek, "pendingTIGER");
        _print("");
        _print("Staked TIGER can only be withdrawn after 6 epochs (48 hours) since deposit. LION rewards can be harvested every 3 epoches (24 Hours) since last deposit or harvest.")
        _print("Any times tokens are Harvested, Deposited, Or Withdrawn, The Lockup timer gets reset.\n")
        const p0 = await loadCronosLIONSynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                              StakingPool.address, 
  ...
  async function loadCronosLIONSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
    const info = await loadCronosLIONSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
    if (!info) return null;
    return await printLIONSynthetixPool(App, info, "cronos");
  }
  
  async function loadCronosLIONSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
  ...
  async function printLIONSynthetixPool(App, info, chain="eth", customURLs) {
  ...
  async function loadCronosTigerContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
