...
const Pools = [
  ...
  "0x52CACa9a2D52b27b28767d3649565774A3B991f3",
...
async function main() {
  ...
  console.log(prices)
  let p = await loadMultipleRoseSynthetixPools(App, tokens, prices, Pools)
...
async function loadMultipleRoseSynthetixPools(App, tokens, prices, pools) {
  let totalStaked  = 0, totalUserStaked = 0, individualAPRs = [];
  const infos = await Promise.all(pools.map(p =>
      loadRoseSynthetixPoolInfo(App, tokens, prices, p.abi, p.address, p.stakeTokenFunction)));
...
async function loadRoseSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress, stakeTokenFunction) {
    ...
    const calls = [STAKING_MULTI.balanceOf(App.YOUR_ADDRESS), STAKING_MULTI.earned(App.YOUR_ADDRESS, rewardTokenAddress), STAKING_MULTI.rewardData(rewardTokenAddress)]
    const [balance, earned_, rewardData] = await App.ethcallProvider.all(calls);
