$(function () {		
    ...
    async function main() {		
        ...
        const StakingPool = {
          address : "0x530F76e221563a60083D9d38Eabe84c1025476aD",
          abi : AXE_STAKING_ABI,
          stakeTokenFunction : "chop",
          rewardTokenAddresses : "axe"
        }
        
        const CHOP_CHEF_ADDR = "0x8F3376F4c465C1d714D92f9703119fE458eB1673";
        const rewardTokenTicker = "CHOP";
        const CHOP_CHEF = new ethers.Contract(CHOP_CHEF_ADDR, CHOP_CHEF_ABI, App.provider);
  
        const rewardsPerWeek = await CHOP_CHEF.chopPerSecond() / 1e18 * 604800;
            
        const tokens = {};
        const prices = await getPulsePrices();
        
        await loadPulseCHOPContract(App, tokens, prices, CHOP_CHEF, CHOP_CHEF_ADDR, CHOP_CHEF_ABI, rewardTokenTicker,		
            "chop", null, rewardsPerWeek, "pendingCHOP");
            
        _print("");
        _print("Staked CHOP can only be withdrawn after 4 epochs since deposit.\n")
        const p0 = await loadPulseAXESynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                              StakingPool.address, 
  ...
  async function loadPulseAXESynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
    const info = await loadPulseAXESynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
    if (!info) return null;
    return await printAXESynthetixPool(App, info, "pulse");
  }
  
  async function loadPulseAXESynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
  ...
  async function printAXESynthetixPool(App, info, chain="eth", customURLs) {
  ...
  async function loadPulseCHOPContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
    ...
    for (i = 0; i < poolCount; i++) {
      if (poolPrices[i]) {
        const apr = printChopChefPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
function printChopChefPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printChopChefContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fixedDecimals, claimFunction, rewardPrice, chain, depositFee, withdrawFee);
  return apr;
}

function printChopChefContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
    rewardTokenTicker, stakeTokenTicker, unstaked, userStaked, pendingRewardTokens, fixedDecimals,
    claimFunction, rewardTokenPrice, chain, depositFee, withdrawFee) {
  fixedDecimals = fixedDecimals ?? 2;
  const approveAndStake = async function() {
    return chopContract_stake(chefAbi, chefAddr, poolIndex, poolAddress, App)
  }
  const unstake = async function() {
    return chopContract_unstake(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction)
  }
  const claim = async function() {
...
const chopContract_stake = async function(chefAbi, chefAddress, poolIndex, stakeTokenAddr, App) {
...
const chopContract_unstake = async function(chefAbi, chefAddress, poolIndex, App, pendingRewardsFunction) {
  const signer = App.provider.getSigner()
  const CHEF_CONTRACT = new ethers.Contract(chefAddress, chefAbi, signer)
