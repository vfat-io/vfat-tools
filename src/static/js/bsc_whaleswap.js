...
$(function() {
  ...
  async function main() {
     ...
     const POD_MASTER = new ethers.Contract(POD_MASTER_ADDR, POD_MASTER_ABI, App.provider);
      ...
      await loadBscWhaleChefContract(App, tokens, prices, POD_MASTER, POD_MASTER_ADDR, POD_MASTER_ABI, rewardTokenTicker,
          "POD", "pendingPod");
  ...
  async function loadBscWhaleChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
    ...
    const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
      await getBscWhalePoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
  ...
  async function getBscWhalePoolInfo(App, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
