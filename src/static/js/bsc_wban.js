  $(function() {
    ...
    async function main() {
       ...
       const BENIS_ADDR = "0x1E30E12e82956540bf870A40FD1215fC083a3751";
       ...
       if(currentBlock < startBlock){
        ...
        await loadBscBenisContract(App, tokens, prices, BENIS, BENIS_ADDR, BENIS_ABI, rewardTokenTicker,"wban", null, rewardsPerWeek, "pendingWBAN");
    
        hideLoading();
      }
    
      
async function loadBscBenisContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
...
async function getBscPoolInfo(App, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  ...
  if (poolInfo.allocPoint == 0 || poolIndex == 105) {
    return {
      address: poolInfo.stakingToken ?? poolInfo.token,
  ...
  const poolToken = await getBscToken(App, poolInfo.stakingToken ?? poolInfo.token, chefAddress);
