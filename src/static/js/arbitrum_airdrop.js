...
async function main() {
   ...
   const ARB_AIRDROP_ADDR = "0x67a24CE4321aB3aF51c2D0a4801c3E111D88C9d9";
   const rewardTokenTicker = "ARB";
   const ARB_AIRDROP = new ethers.Contract(ARB_AIRDROP_ADDR, ARB_AIRDROP_ABI, App.provider);

   const claimableTokens = await ARB_AIRDROP.claimableTokens(App.YOUR_ADDRESS);

   const claim = async function() {
    return airdropContract_claim(ARB_AIRDROP_ADDR, App, claimableTokens)
  }
...
const airdropContract_claim = async function(rewardPoolAddr, App, claimableTokens) {
  const signer = App.provider.getSigner()

  const REWARD_POOL = new ethers.Contract(rewardPoolAddr, ARB_AIRDROP_ABI, signer)
