...
async function main() {
  ...
  const MAIA_VOTER_CONTRACT = new ethcall.Contract(MAIA_VOTER_ADDR, MAIA_VOTER_ABI);
  const HERMES_TOKEN_ADDR = "0xb27BbeaACA2C00d6258C3118BAB6b5B6975161c8"
  const [_poolLength] = await App.ethcallProvider.all([MAIA_VOTER_CONTRACT.length()]);
  const poolLength = _poolLength / 1;
  const calls = [...Array(poolLength).keys()].map(i => MAIA_VOTER_CONTRACT.pools(i));
  const lpTokens = await App.ethcallProvider.all(calls);
  const calls2 = lpTokens.map(a => MAIA_VOTER_CONTRACT.gauges(a));
