...
$(function() {
    ...
    async function main() {
       ...
       const ARB_CHEF_ADDR = "0x1b1987278d7737aa6bced283643c314c318c3879";
       const rewardTokenTicker = "ARB";
       const ARB_CHEF = new ethers.Contract(ARB_CHEF_ADDR, ARB_CHEF_ABI, App.provider);
    
    
      const blockNumber = await App.provider.getBlockNumber();
      const multiplier = await ARB_CHEF.getMultiplier(blockNumber, blockNumber+1);
      const rewardPerBlock = await ARB_CHEF.REWARD_PER_BLOCK();
      const rewardsPerWeek = rewardPerBlock / 1e18 * multiplier * 604800 / 2
    
        const tokens = {};
        const prices = await getHarmonyPrices();
    
        await loadCosmicChefContract(App, tokens, prices, ARB_CHEF, ARB_CHEF_ADDR, ARB_CHEF_ABI, rewardTokenTicker,
