...
async function main() {
   ...
   const FLARE_CHEF_ADDR = "0x995da7dfB96B4dd1e2bd954bE384A1e66cBB4b8c";
   const rewardTokenTicker = "FLARE";
   const FLARE_CHEF = new ethers.Contract(FLARE_CHEF_ADDR, FLARE_CHEF_ABI, App.provider);
   const rewardsPerWeek = await FLARE_CHEF.flarePerSec() / 1e18 * 604800;

    const tokens = {};
    const prices = await getMoonbeamPrices();

    await loadFlareChefContract(App, tokens, prices, FLARE_CHEF, FLARE_CHEF_ADDR, FLARE_CHEF_ABI, rewardTokenTicker,
      "flare", null, rewardsPerWeek, "pendingTokens");

    hideLoading();
  }

async function loadFlareChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getFlarePoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
...
async function getFlarePoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
