...
async function main() {
  ...
  const MINI_CHEF_ADDR = '0x156e1d61cd02C7F99B5b0Fb5f37163312C7596bF'
  const rewardTokenTicker = '1CRO'
  const rewardTokenAddress = '0x7A1803b4433A83819f9796564829b37f49841672'
  ...
  prices['0x6f61e4c3299ea49a19878821daf9d54aad8d9325'] = {usd: 1} // OB3USD

  const customURLs = {
    add: 'https://mm.finance/add',
    remove: 'https://mm.finance/remove',
    swap: 'https://mm.finance/swap',
    info: 'https://mm.finance/analytics/pool',
  }

  const chef1cronos_stake = async function(chefAbi, chefAddress, poolIndex, stakeTokenAddr, App) {
  ...
  const chef1cronos_unstake = async function(chefAbi, chefAddress, poolIndex, App, pendingRewardsFunction) {
    const signer = App.provider.getSigner()
    const CHEF_CONTRACT = new ethers.Contract(chefAddress, chefAbi, signer)
  ...
  const chef1cronos_claim = async function(
    chefAbi,
    chefAddress,
    poolIndex,
    App,
  ...
  function printChefContractLinks(
    ...
    const approveAndStake = async function() {
      return chef1cronos_stake(chefAbi, chefAddr, poolIndex, poolAddress, App)
    }
    const unstake = async function() {
      return chef1cronos_unstake(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction)
    }
    const claim = async function() {
      return chef1cronos_claim(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction, claimFunction)
    }
    if (depositFee > 0) {
  ...
  async function get1cronosPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  ...
  async function load1cronosChefContract(
    App,
    tokens,
    prices,
    chef,
    ...
    const poolInfos = await Promise.all(
      [...Array(poolCount).keys()].map(
        async x => await get1cronosPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)
      )
    )
    ...
    if (deathPoolIndices) {
      ...
      deathPoolIndices
        .map(i => poolInfos[i])
        .map(poolInfo => (poolInfo.poolToken ? getPoolPrices(tokens, prices, poolInfo.poolToken, 'cronos') : undefined))
    }
