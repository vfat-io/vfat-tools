...
async function getercMeter20(App, token, address, stakingAddress) {
...
async function getMeterVault(App, vault, address, stakingAddress) {
  ...
  const token = await getMeterToken(App, token_, address);
  return {
    address,
    name : name,
    symbol : symbol,
...
async function getMeterWantVault(App, vault, address, stakingAddress) {
...
async function getMeterDlpPool(App, dlpPool, tokenAddress, originTokenAddress, stakingAddress){
  const calls = [
  ...
  const originToken = await getMeterToken(App, originTokenAddress, ownerAddress);
  return {
...
async function getMeterStoredToken(App, tokenAddress, stakingAddress, type) {
  switch (type) {
    ...
    case "doualMeterDlp":
      const doualDlpPool = new ethcall.Contract(tokenAddress, DLP_METER_DUAL_TOKEN_ABI);
      return await getDodoMeterDualPoolToken(App, doualDlpPool, tokenAddress, stakingAddress);
    case "meterVault":
      const vault = new ethcall.Contract(tokenAddress, METER_VAULT_TOKEN_ABI);
      return await getMeterVault(App, vault, tokenAddress, stakingAddress);
    case "meterWantVault":
      const wantVault = new ethcall.Contract(tokenAddress, METER_VAULT_WANT_ABI);
      return await getMeterWantVault(App, wantVault, tokenAddress, stakingAddress);
    case "dlp":
      const dlpPool = new ethcall.Contract(tokenAddress, METER_DLP_ABI);
      const [originTokenAddress] = await App.ethcallProvider.all([dlpPool.originToken()]);
      return await getMeterDlpPool(App, dlpPool, tokenAddress, originTokenAddress, stakingAddress);
    case "erc20":
...
async function getMeterToken(App, tokenAddress, stakingAddress) {
    if (tokenAddress == "0x0000000000000000000000000000000000000000") {
      return getercMeter20(App, null, tokenAddress, stakingAddress)
    }
    ...
    try {
      const pool = new ethcall.Contract(tokenAddress, DLP_METER_DUAL_TOKEN_ABI);
      const _baseToken = await App.ethcallProvider.all([pool._BASE_TOKEN_()]);
      const doualDlpPool = await getDodoMeterDualPoolToken(App, pool, tokenAddress, stakingAddress);
      window.localStorage.setItem(tokenAddress, "doualMeterDlp");
      return doualDlpPool;
    }
    catch(err) {
    }
    try {
      const pool = new ethcall.Contract(tokenAddress, METER_DLP_ABI);
      const [originTokenAddress] = await App.ethcallProvider.all([pool.originToken()]);
      const dlpPool = await getMeterDlpPool(App, pool, tokenAddress, originTokenAddress, stakingAddress);
    ...
    try {
      ...
      const vault = await getMeterVault(App, VAULT, tokenAddress, stakingAddress);
      window.localStorage.setItem(tokenAddress, "meterVault");
      return vault;
    }
    catch(err) {
    }
    try {
      const WANT_VAULT = new ethcall.Contract(tokenAddress, METER_VAULT_WANT_ABI);
      const _want = await App.ethcallProvider.all([WANT_VAULT.want()]);
      const wantVault = await getMeterWantVault(App, WANT_VAULT, tokenAddress, stakingAddress);
      window.localStorage.setItem(tokenAddress, "meterWantVault");
      return wantVault;
    }
    catch(err) {
    }
