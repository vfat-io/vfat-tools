...
async function main() {
  ...
  const STG_CHEF_ADDR = "0xB0D502E938ed5f4df2E681fE6E419ff29631d62b";
  const STG_CHEF = new ethers.Contract(STG_CHEF_ADDR, STG_CHEF_ABI, App.provider);
  const rewardsPerWeek = await STG_CHEF.stargatePerBlock() / 1e18 * 604800 / 13.5;

  await loadStgContract(App, STG_CHEF, STG_CHEF_ADDR, STG_CHEF_ABI,
    "STG", "stargate", null, rewardsPerWeek, "pendingStargate");

  hideLoading();
}

async function getStgPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction, showAll=false) {
...
async function loadStgContract(App, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) => {
    try {
      return await getStgPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction, showAll);
    }
    catch (ex) {
  ...
  prices["0x38EA452219524Bb87e18dE1C24D3bB59510BD783"] = { usd : 1 }
  prices["0xdf0770dF86a8034b3EFEf0A1Bb3c889B8332FF56"] = { usd : 1 }
