...
const GRAND_CHEF_ABI = [
  ...
  },
  {
    inputs: [
      ...
      {internalType: 'address', name: '_frySailing', type: 'address'},
      {internalType: 'address', name: '_fryLanding', type: 'address'},
      {internalType: 'bool', name: '_isLp', type: 'bool'},
    ],
    name: 'add',
    outputs: [],
  ...
  {
    inputs: [{internalType: 'address', name: '_paidToken', type: 'address'}],
    name: 'addPaidToken',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  ...
  },
  {
    inputs: [
      ...
      {internalType: 'address', name: '_paidToken', type: 'address'},
    ],
    name: 'depositToken',
    outputs: [],
    stateMutability: 'nonpayable',
  ...
  },
  {
    ...
    outputs: [
      ...
      {internalType: 'uint256', name: 'accGrandPerShare', type: 'uint256'},
      {internalType: 'address', name: 'strat', type: 'address'},
      {internalType: 'address', name: 'frySailing', type: 'address'},
      {internalType: 'address', name: 'fryLanding', type: 'address'},
...
async function main() {
  ...
  const rewardsPerWeek = (((await GRAND_CHEF.GrandPerBlock()) / 1e18) * 604800) / 3

  const tokens = {}
  const prices = await getBscPrices()

  await loadBscGrandContract(
    App,
    tokens,
    prices,
    GRAND_CHEF,
...
async function loadBscGrandContract(
  App,
  tokens,
  prices,
  chef,
  ...
  const poolInfos = await Promise.all(
    [...Array(poolCount).keys()].map(async x => await getBscGrandPoolInfo(App, chefContract, x, pendingRewardsFunction))
  )
...
function printGrandContractLinks(
  App,
  chefAbi,
  chefAddr,
  poolIndex,
