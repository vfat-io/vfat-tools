...
async function main() {
   ...
   const RNG_CHEF_ADDR = "0x8596612D55ead92A2de35A4dC0f527e012E928e2";
   const rewardTokenTicker = "RNG";
   const RNG_CHEF = new ethers.Contract(RNG_CHEF_ADDR, RNG_CHEF_ABI, App.provider);

   const startBlock = await RNG_CHEF.startBlock();
   ...
   if(currentBlock < startBlock){
    ...
    rewardsPerWeek = await RNG_CHEF.eggPerBlock() /1e18
      * 604800 / 2.1;
   }
    ...
    await loadRangersContract(App, tokens, prices, RNG_CHEF, RNG_CHEF_ADDR, RNG_CHEF_ABI, rewardTokenTicker,
        "egg", null, rewardsPerWeek, "pendingEgg");

    hideLoading();
  }

async function loadRangersContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getRangersPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printRangersPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
async function getRangersPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
function printRangersPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printRangersContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fee, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printRangersContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
