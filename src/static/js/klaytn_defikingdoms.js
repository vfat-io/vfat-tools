...
$(function() {
    ...
    async function main() {
       ...
       const JADE_CHEF_ADDR = "0xad2ea7b7e49be15918E4917736E86ff7feA57a6A";
       const rewardTokenTicker = "JADE";
       const JADE_CHEF = new ethers.Contract(JADE_CHEF_ADDR, JADE_CHEF_ABI, App.provider);
    
    
      const blockNumber = await App.provider.getBlockNumber();
      const multiplier = await JADE_CHEF.getMultiplier(blockNumber, blockNumber+1);
      // const rewardPerBlock = await JADE_CHEF.REWARD_PER_BLOCK();
      // const rewardsPerWeek = rewardPerBlock / 1e18 * multiplier * 604800 / 2
    
        const tokens = {};
        const prices = await getKlaytnPrices();
    
        await loadJadeChefContract(App, tokens, prices, JADE_CHEF, JADE_CHEF_ADDR, JADE_CHEF_ABI, rewardTokenTicker,
            "govToken", null, 0, "pendingReward", [3,4], "klaytn");
    
        hideLoading();
      }
  
    async function loadJadeChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  _print(`<a href='https://scope.klaytn.com/account/${chefAddress}' target='_blank'>Staking Contract</a>`);
  ...
  for(let i = 0; i < poolCount; i++){
    const poolInfo = await getGeneralPoolInfo(App, chefContract, chefAddress, i, pendingRewardsFunction);
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printDefikingdomsChefPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
function printDefikingdomsChefPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printDefikingdomsChefContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fixedDecimals, claimFunction, rewardPrice, chain, depositFee, withdrawFee);
  return apr;
}

function printDefikingdomsChefContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
    rewardTokenTicker, stakeTokenTicker, unstaked, userStaked, pendingRewardTokens, fixedDecimals,
    claimFunction, rewardTokenPrice, chain, depositFee, withdrawFee) {
  fixedDecimals = fixedDecimals ?? 2;
  const approveAndStake = async function() {
    return chefDefikingdomsContract_stake(chefAbi, chefAddr, poolIndex, poolAddress, App)
  }
  const unstake = async function() {
    return chefDefikingdomsContract_unstake(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction)
  }
  const claim = async function() {
    return chefDefikingdomsContract_claim(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction, claimFunction)
  }
  if(depositFee > 0){
...
const chefDefikingdomsContract_stake = async function(chefAbi, chefAddress, poolIndex, stakeTokenAddr, App) {
...
const chefDefikingdomsContract_unstake = async function(chefAbi, chefAddress, poolIndex, App, pendingRewardsFunction) {
  const signer = App.provider.getSigner()
  const CHEF_CONTRACT = new ethers.Contract(chefAddress, chefAbi, signer)
...
const chefDefikingdomsContract_claim = async function(chefAbi, chefAddress, poolIndex, App,
