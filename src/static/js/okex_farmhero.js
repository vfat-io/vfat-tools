...
async function main() {
   ...
   const HEROFARM_CHEF_ADDR = "0xDb457E7fA88C9818f6134afD673941fCE777F92F";
    const rewardTokenTicker = "GLORY";
  ...
  async function loadHEROChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
    ...
    if (deathPoolIndices) {   //load prices for the deathpool assets
      deathPoolIndices.map(i => poolInfos[i])
                       .map(poolInfo =>
        poolInfo.poolToken && poolInfo.erc20 ? getPoolPrices(tokens, prices, poolInfo.poolToken,"okex") : undefined);
    }

    const poolPrices = poolInfos.map(poolInfo => poolInfo.poolToken && poolInfo.erc20 ? getPoolPrices(tokens, prices, poolInfo.poolToken,"okex") : undefined);
...
function printHeroChefPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
                       pendingRewardsFunction, fixedDecimals, claimFunction, chain="okex", depositFee=0, withdrawFee=0) {
  fixedDecimals = fixedDecimals ?? 2;
