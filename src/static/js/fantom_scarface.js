$(function () {		
  ...
  async function main() {		
      ...
      const StakingPool = {
        address : "0xc255aa0a5735541b76ae318b68186247745665d7",
        abi : SCAR_STAKING_ABI,
        stakeTokenFunction : "share",
        rewardTokenAddresses : "scar"
      }
      
      const SSHARE_CHEF_ADDR = "0xD3c723e1A822997D60976e20d1f3F4f69DBeF482";
      const rewardTokenTicker = "SSHARE";
      const SSHARE_CHEF = new ethers.Contract(SSHARE_CHEF_ADDR, SSHARE_CHEF_ABI, App.provider);

      const rewardsPerWeek = await SSHARE_CHEF.sSharePerSecond() / 1e18 * 604800;
          
      const tokens = {};
      const prices = await getFantomPrices();
      
      await loadFantomSSharesContract(App, tokens, prices, SSHARE_CHEF, SSHARE_CHEF_ADDR, SSHARE_CHEF_ABI, rewardTokenTicker,		
          "sshare", null, rewardsPerWeek, "pendingShare");
      _print(`\nStaked SSHAREs can only be withdrawn after 3 epochs since deposit.`);
      _print(`Rewards can be claimed after 1 epoch.`);
      _print(`Any time tokens are harvested, deposited, or withdrawn, the lockup timer gets reset.\n`);
      const p0 = await loadFantomScarSynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                            StakingPool.address, 
...
async function loadFantomScarSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadFantomScarSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printScarSynthetixPool(App, info, "fantom");
}

async function loadFantomScarSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printScarSynthetixPool(App, info, chain="eth", customURLs) {
...
async function loadFantomSSharesContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
