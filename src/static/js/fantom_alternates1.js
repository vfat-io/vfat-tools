$(function () {		
  ...
  async function main() {		
      ...
      const StakingPool = {
        address : "0x7723427970fe2210e61376063ed28356ed7ff146",
        abi : ALT_STAKING_ABI,
        stakeTokenFunction : "share",
        rewardTokenAddresses : "tomb"
      }
      
      const ASHARE_CHEF_ADDR = "0xc5f97e19c454c379aab0f5820507bd9d43cb09b7";
      const rewardTokenTicker = "ASHARE";
      const ASHARE_CHEF = new ethers.Contract(ASHARE_CHEF_ADDR, ASHARE_CHEF_ABI, App.provider);

      const rewardsPerWeek = await ASHARE_CHEF.aSharePerSecond() / 1e18 * 604800;
          
      const tokens = {};
      const prices = await getFantomPrices();
      
      await loadFantomAshareContract(App, tokens, prices, ASHARE_CHEF, ASHARE_CHEF_ADDR, ASHARE_CHEF_ABI, rewardTokenTicker,		
          "ashare", null, rewardsPerWeek, "pendingShare");
      _print("");
      _print("Staked ASHAREs can only be withdrawn after 6 epochs since deposit.\n")
      const p0 = await loadFantomALTSynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                            StakingPool.address, 
...
async function loadFantomALTSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadFantomALTSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printALTSynthetixPool(App, info, "fantom");
}

async function loadFantomALTSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printALTSynthetixPool(App, info, chain="eth", customURLs) {
...
async function loadFantomAshareContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
