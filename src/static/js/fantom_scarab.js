$(function () {		
  ...
  async function main() {		
      ...
      const StakingPool = {
        address : "0xd00f41d49900d6affd707ec6a30d1bf7d4b7de8f",
        abi : SCARAB_STAKING_ABI,
        stakeTokenFunction : "share",
        rewardTokenAddresses : "tomb"
      }

      const SCARAB_CHEF_ADDR = "0xc88690163b10521d5fb86c2ecb293261f7771525";
      const rewardTokenTicker = "GSCARAB";
      const SCARAB_CHEF = new ethers.Contract(SCARAB_CHEF_ADDR, SCARAB_CHEF_ABI, App.provider);

      //const rewardsPerWeek = await SCARAB_CHEF.tSharePerSecond() / 1e18 * 604800;
      const rewardsPerWeek = 0;
          
      const tokens = {};
      const prices = await getFantomPrices();

      await loadFantomScarabContract(App, tokens, prices, SCARAB_CHEF, SCARAB_CHEF_ADDR, SCARAB_CHEF_ABI, rewardTokenTicker,		
        "tshare", null, rewardsPerWeek, "pendingShare");

      _print("");
      
      const p0 = await loadFantomScarabSynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                       StakingPool.address, 
...
async function loadFantomScarabContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
...
async function loadFantomScarabSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadFantomScarabSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printScarabSynthetixPool(App, info, "fantom");
}

async function loadFantomScarabSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printScarabSynthetixPool(App, info, chain="eth", customURLs) {
  ...
  const claim = async function() {
    return scarabContract_claim(info.stakingAddress, App)
  }
...
const scarabContract_claim = async function(rewardPoolAddr, App) {
