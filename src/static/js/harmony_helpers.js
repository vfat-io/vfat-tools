...
async function getHarmonyStableswapToken(App, stable, address, stakingAddress) {
  ...
  const [decimals, staked, unstaked, name, symbol, totalSupply, virtualPrice, coin0] = await App.ethcallProvider.all(
    calls
  )
  const token = await getToken(App, coin0, address)
  return {
...
async function loadHarmonySynthetixPoolInfo(
  App,
  tokens,
  prices,
  stakingAbi,
...
async function loadHarmonySynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadHarmonySynthetixPoolInfo(
    App,
    tokens,
    prices,
    abi,
...
async function loadHarmonyBasisFork(data) {
  ...
  let p1 = await loadHarmonySynthetixPool(
    App,
    tokens,
    prices,
    data.PoolABI,
  ...
  if (data.SharePool2) {
    let p3 = await loadHarmonySynthetixPool(
      App,
      tokens,
      prices,
      data.PoolABI,
  ...
  let p2 = await loadHarmonySynthetixPool(
    App,
    tokens,
    prices,
    data.PoolABI,
...
async function getHarmonyPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  ...
  const poolToken = await getHarmonyToken(app, poolInfo.lpToken ?? poolInfo.token, chefAddress)
...
async function loadHarmonyChefContract(
  App,
  tokens,
  prices,
  chef,
...
async function loadMultipleHarmonySynthetixPoolsSequential(App, tokens, prices, pools) {
  ...
  for (const p of pools) {
    let res = await loadHarmonySynthetixPool(
      App,
      tokens,
      prices,
      p.abi,
