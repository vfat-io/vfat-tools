...
async function main() {
   ...
   const GLINT_CHEF_ADDR = "0xC6ca172FC8BDB803c5e12731109744fb0200587b";
   const rewardTokenTicker = "GLINT";
   const GLINT_CHEF = new ethers.Contract(GLINT_CHEF_ADDR, GLINT_CHEF_ABI, App.provider);
   const rewardsPerWeek = await GLINT_CHEF.beamPerSec() / 1e18 * 604800;

    const tokens = {};
    const prices = await getMoonbeamPrices();

    await loadBeamChefContract(App, tokens, prices, GLINT_CHEF, GLINT_CHEF_ADDR, GLINT_CHEF_ABI, rewardTokenTicker,
      "beam", null, rewardsPerWeek, "pendingTokens");

    hideLoading();
  }

async function loadBeamChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getBeamPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
...
async function getBeamPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
