...
async function main() {
   ...
   const KBN_CHEF_ADDR = "0x32c4e5f5fa80B393973967AE566f4418f3B48651";
   const rewardTokenTicker = "KBN";
   const KBN_CHEF = new ethers.Contract(KBN_CHEF_ADDR, KBN_CHEF_ABI, App.provider);

   const currentBlock = await App.provider.getBlockNumber();

   const multiplier = await KBN_CHEF.getMultiplier(currentBlock,currentBlock+1);
   ...
   const rewardsPerWeek = await KBN_CHEF.kobanPerBlock() /1e18
    ...
    await loadPokeChefContract(App, tokens, prices, KBN_CHEF, KBN_CHEF_ADDR, KBN_CHEF_ABI, rewardTokenTicker,
        "koban", null, rewardsPerWeek, "pendingKoban", [2,4]);

    hideLoading();
  }

async function getPokePoolInfo(App, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  ...
  return {
      ...
      depositFee : (poolInfo.depositBP ?? 0) / 100,
      withdrawFee : (poolInfo.withdrawBP ?? 0) / 100
  };
}

async function loadPokeChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getPokePoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
