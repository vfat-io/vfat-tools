$(function () {		
    ...
    async function main() {		
        ...
        const StakingPool = {
          address : "0x22f979Ebd7d43F6F3a21650c8046abdFa6450061",
          abi : WOOD_STAKING_ABI,
          stakeTokenFunction : "share",
          rewardTokenAddresses : "wood"
        }
        
        const WTRUNK_CHEF_ADDR = "0xC393A3A342B405E2f8f0D85D2baDB7fab70c6569";
        const rewardTokenTicker = "wTRUNK";
        const WTRUNK_CHEF = new ethers.Contract(WTRUNK_CHEF_ADDR, WTRUNK_CHEF_ABI, App.provider);
  
        const rewardsPerWeek = await WTRUNK_CHEF.wTrunkPerSecond() / 1e18 * 604800;
  ...
  async function loadFantomWOODSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
    const info = await loadFantomWOODSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
    if (!info) return null;
    return await printWOODSynthetixPool(App, info, "fantom");
  }
  
  async function loadFantomWOODSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
  ...
  async function printWOODSynthetixPool(App, info, chain="eth", customURLs) {
  ...
  async function loadFantomWtrunkContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
