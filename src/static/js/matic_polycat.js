...
async function main() {
    ...
    const PAW_CHEF_ADDR = "0x4ce9ae2f5983e19aebf5b8bae4460f2b9ece811a";
    const rewardTokenTicker = "PAW";
    const PAW_CHEF = new ethers.Contract(PAW_CHEF_ADDR, PAW_CHEF_ABI, App.provider);

    const startBlock = await PAW_CHEF.startBlock();
    ...
    if (currentBlock < startBlock) {
        ...
        rewardsPerWeek = await PAW_CHEF.pawPerBlock() / 1e18 *
            604800 / 2.1;
    }
    ...
    let pools = await loadMaticPawChefContract(App, tokens, prices, PAW_CHEF, PAW_CHEF_ADDR, PAW_CHEF_ABI, rewardTokenTicker,
      "paw", null, rewardsPerWeek, "pendingPaw");
    
    const vestingAll = async function() {
...
async function loadMaticPawChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      ...
      aprs.push(apr);
      totalPendingTokens += poolInfos[i].pendingRewardTokens;
    }
  }
  ...
  totalUSDPendingRewards = totalPendingTokens * rewardPrice;
  return { prices, totalUserStaked, totalStaked, averageApr, totalPendingTokens, totalUSDPendingRewards }
}
...
const chefPawContract_claimAll = async function(chefAbi, chefAddress, App, pendingRewardTokens) {
  const signer = App.provider.getSigner()
