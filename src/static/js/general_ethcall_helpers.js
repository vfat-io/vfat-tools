...
async function getGeneralEthcallAmmV2Pool(App, pool, poolAddress, stakingAddress) {
  const calls = [
    pool.decimals(), pool.token0(), pool.token1(), pool.symbol(), pool.name(),
    pool.totalSupply(), pool.balanceOf(stakingAddress), pool.balanceOf(App.YOUR_ADDRESS),
    pool.reserve0(), pool.reserve1()];
  const [decimals, token0, token1, symbol, name, totalSupply, staked, unstaked, q0, q1]
    = await App.ethcallProvider.all(calls);
  return {
      symbol,
      name,
...
async function getGeneralEthcallStoredToken(App, tokenAddress, stakingAddress, type) {
  switch (type) {
    ...
    case "ammV2":
      const amm = new ethcall.Contract(tokenAddress, AMM_V2_ABI);
      return await getGeneralEthcallAmmV2Pool(App, amm, tokenAddress, stakingAddress);
    case "hummus":
...
async function getGeneralEthcallToken(App, tokenAddress, stakingAddress) {
    ...
    try {
      const amm = new ethcall.Contract(tokenAddress, AMM_V2_ABI);
      const [observationLength] = await App.ethcallProvider.all([amm.observationLength()]);
      const res = await getGeneralEthcallAmmV2Pool(App, amm, tokenAddress, stakingAddress);
      window.localStorage.setItem(tokenAddress, "ammV2");
      return res;
    }
    catch(err) {
    }
