$(function() {
  ...
  async function main() {  
    ...
    await loadCompliContract(App, COMFI_CHEF, COMFI_CHEF_ADDR, COMFI_CHEF_ABI, rewardTokenTicker,
        "rewardToken", null, rewardsPerWeek, "getPendingReward");
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) => {
    try {
      return await getCompliPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction, showAll);
    }
    catch (ex) {
  ...
  async function getCompliPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction, showAll=false) {  
  ...
  const userInfo = await chefContract.userPoolInfo(poolIndex, app.YOUR_ADDRESS);
