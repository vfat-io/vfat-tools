...
async function main() {
  ...
  const BARD_CHEF_ADDR = "0x88ebd46751a94602f798e20037c5bb5ed17f78db";
  const rewardTokenTicker = "BARD";
  const BARD_CHEF = new ethers.Contract(BARD_CHEF_ADDR, BARD_CHEF_ABI, App.provider);

  const blockNumber = await App.provider.getBlockNumber();

  const multiplier = await BARD_CHEF.getMultiplier(blockNumber, blockNumber+1);
  const rewardPerBlock = await BARD_CHEF.REWARD_PER_BLOCK();
  const rewardsPerWeek = rewardPerBlock / 1e18 * multiplier * 604800 / 2

  const tokens = {};
  const prices = await getHarmonyPrices();

  await loadCosmicChefContract(
    App,
    tokens,
    prices,
    BARD_CHEF,
