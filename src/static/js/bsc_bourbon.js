$(function () {		
  ...
  async function main() {		
      ...
      const StakingPool = {
        address : "0x920ccda0406fb3d3175f319821b692c4a7602be0",
        abi : BOURBON_STAKING_ABI,
        stakeTokenFunction : "share",
        rewardTokenAddresses : "bourboncake"
      }
      
      const BSHARES_CHEF_ADDR = "0x820f55b63123913eaab0126c0963610bd5f58e00";
      const rewardTokenTicker = "BSHARES";
      const BSHARES_CHEF = new ethers.Contract(BSHARES_CHEF_ADDR, BSHARES_CHEF_ABI, App.provider);

      const rewardsPerWeek = await BSHARES_CHEF.tSharePerSecond() / 1e18 * 604800;
          
      const tokens = {};
      const prices = await getBscPrices();
      
      await loadBscBSharesContract(App, tokens, prices, BSHARES_CHEF, BSHARES_CHEF_ADDR, BSHARES_CHEF_ABI, rewardTokenTicker,		
          "bshare", null, rewardsPerWeek, "pendingShare");
      _print(`\nStaked BSHARES can only be withdrawn after 6 epochs since deposit.\n`);
      const p0 = await loadBscBourbonSynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                            StakingPool.address, 
...
async function loadBscBourbonSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadBscBourbonSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printBourbonSynthetixPool(App, info, "bsc");
}

async function loadBscBourbonSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printBourbonSynthetixPool(App, info, chain="eth", customURLs) {
...
async function loadBscBSharesContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
