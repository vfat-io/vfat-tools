$(function () {		
  ...
  async function main() {		
      ...
      const StakingPool = {
        address : "0xa252FfDB3A73Bd0F88Eea39658c7C00a281B3bB6",
        abi : SUB_STAKING_ABI,
        stakeTokenFunction : "share",
        rewardTokenAddresses : "subzero"
      }
      
      const ZSHARE_CHEF_ADDR = "0xDAccfd92e37be54Ca1A8ff37A7922446614b4759";
      ...
      await loadAvaxZSHAREContract(App, tokens, prices, ZSHARE_CHEF, ZSHARE_CHEF_ADDR, ZSHARE_CHEF_ABI, rewardTokenTicker,		
          "zshare", null, rewardsPerWeek, "pendingShare", [1]);
      _print(`\nRegulation will only allow SUB to be printed when TWAP is over 1.01.`);
      _print(`Staked ZSHAREs can only be withdrawn after 6 epochs since deposit.\n`)
      const p0 = await loadAvaxSUBSynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                     StakingPool.address, 
...
async function loadAvaxSUBSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadAvaxSUBSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printSUBSynthetixPool(App, info, "avax");
}

async function loadAvaxSUBSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printSUBSynthetixPool(App, info, chain="eth", customURLs) {
