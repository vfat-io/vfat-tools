...
async function main() {
   ...
   const YEL_CHEF_ADDR = "0x954b15065e4FA1243Cd45a020766511b68Ea9b6E";
   const rewardTokenTicker = "YEL";
   const YEL_CHEF = new ethers.Contract(YEL_CHEF_ADDR, YEL_CHEF_ABI, App.provider);

   const rewardsPerWeek = await YEL_CHEF.yelPerSecond() /1e18
        * 604800;

    const tokens = {};
    const prices = await getBscPrices();

    await loadBscYelChefContract(App, tokens, prices, YEL_CHEF, YEL_CHEF_ADDR, YEL_CHEF_ABI, rewardTokenTicker,
        "yel", null, rewardsPerWeek, "pendingYel", [1]);

    hideLoading();
  }

async function getBscYelPoolInfo(App, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
async function loadBscYelChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getBscYelPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
