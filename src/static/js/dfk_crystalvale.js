...
async function main() {
 ...
 const CRYSTAL_CHEF_ADDR = "0x57dec9cc7f492d6583c773e2e7ad66dcdc6940fb";

 const rewardTokenTicker = "CRYSTAL";
 const CRYSTAL_CHEF = new ethers.Contract(CRYSTAL_CHEF_ADDR, CRYSTAL_CHEF_ABI, App.provider);

  const tokens = {};
  const prices = await getDfkPrices();

  const blockNumber = await App.provider.getBlockNumber();
  const multiplier = await CRYSTAL_CHEF.getMultiplier(blockNumber, blockNumber+1);
  //const rewardPerBlock = await CRYSTAL_CHEF.tokenPerBlock();
  //const rewardsPerWeek = rewardPerBlock / 1e18 * multiplier * 604800 / 2

  await loadDfkDefikindomsContract(App, tokens, prices, CRYSTAL_CHEF, CRYSTAL_CHEF_ADDR, CRYSTAL_CHEF_ABI, rewardTokenTicker,
...
async function loadXCrystal(App, prices){
  const xCRYSTAL_ADDR = "0x6e7185872bcdf3f7a6cbbe81356e50daffb002d2";
  const CRYSTAL_ADDR = "0x04b9dA42306B023f3572e106B11D82aAd9D32EBb";
  const xCRYSTAL_CONTRACT = new ethers.Contract(xCRYSTAL_ADDR, xCRYSTAL_ABI, App.provider);
  const CRYSTAL_CONTRACT = new ethers.Contract(CRYSTAL_ADDR, CRYSTAL_ABI, App.provider);
  const totalDepositedCrystals = await CRYSTAL_CONTRACT.balanceOf(xCRYSTAL_ADDR) / 1e18;
  const totalXCrystalTokens = await xCRYSTAL_CONTRACT.totalSupply() / 1e18;
  const virtualPrice = totalDepositedCrystals / totalXCrystalTokens;
  const crystalPrice = getParameterCaseInsensitive(prices, CRYSTAL_ADDR).usd;
  const xCrystalPrice = crystalPrice * virtualPrice;
  const rewardTicker = await xCRYSTAL_CONTRACT.symbol();
  const stakeTicker = await CRYSTAL_CONTRACT.symbol();
  const totalOwnedXCrystal = await xCRYSTAL_CONTRACT.balanceOf(App.YOUR_ADDRESS) / 1e18;
  const totalOwnedCrystal = totalOwnedXCrystal * virtualPrice;
  const tvl = totalDepositedCrystals * crystalPrice
  const usersPercentage = totalOwnedXCrystal / totalXCrystalTokens * 100;
  const usersXCrystalUsd = totalOwnedXCrystal * crystalPrice;
  _print(`\n${rewardTicker} Price: $${toFixed(xCrystalPrice, 2)} Total Supply: ${toFixed(totalXCrystalTokens, 4)} TVL: $${formatMoney(tvl)}`);
  _print(`${stakeTicker} Price: $${toFixed(crystalPrice, 2)}`);
  _print(`There is a total of ${toFixed(totalDepositedCrystals, 2)} $${toFixed(totalDepositedCrystals * crystalPrice, 2)} CRYSTAL staked in xCRYSTAL`);
  _print(`Your balance is ${totalOwnedXCrystal.toFixed(2)} ${rewardTicker} (${totalOwnedCrystal.toFixed(2)} ${stakeTicker}), $${formatMoney(usersXCrystalUsd)}, ${usersPercentage.toFixed(2)}% of the pool`);
  const approveAndStake = async function() {
    return contract_stake(xCRYSTAL_ABI, xCRYSTAL_ADDR, App, CRYSTAL_ADDR)
  }
  const unstake = async function() {
    return contract_unstake(xCRYSTAL_ABI, xCRYSTAL_ADDR, totalOwnedXCrystal, App)
  }
  _print_link(`Stake ${totalOwnedCrystal.toFixed(2)} ${stakeTicker}`, approveAndStake)
  _print_link(`Unstake ${totalOwnedXCrystal.toFixed(2)} ${rewardTicker}`, unstake)
}
...
const contract_unstake = async function(abi, contractAddress, stakedAmount, App) {
  const signer = App.provider.getSigner()
  const xCRYSTAL_WRITE_CONTRACT = new ethers.Contract(contractAddress, abi, signer)
  showLoading()
  xCRYSTAL_WRITE_CONTRACT.leave(stakedAmount, {gasLimit: 500000})
      .then(function(t) {
        return App.provider.waitForTransaction(t.hash)
      })
      .catch(function() {
...
async function loadDfkDefikindomsContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
