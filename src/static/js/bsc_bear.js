...
$(function() {
  ...
  async function main() {
     ...
     const BEAR_CHEF_ADDR = "0xc3D910c9D2bB024931a44Cf127B6231aC1F04de3";
     ...
     if(currentBlock < startBlock){
      ...
      rewardsPerWeek = await BEAR_CHEF.EarningsPerBlock() /1e18
          * 604800 * multiplier / 3;
     }
      ...
      await loadBscBEARContract(App, tokens, prices, BEAR_CHEF, BEAR_CHEF_ADDR, BEAR_CHEF_ABI, rewardTokenTicker,
          "earningToken", null, rewardsPerWeek, "pendingEarnings");
  
      hideLoading();
    }
  
    async function loadBscBEARContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
      ...
      const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
        await getBscBEARPoolInfo(App, chefContract, x, pendingRewardsFunction)));
    ...
    async function getBscBEARPoolInfo(App, chefContract, poolIndex, pendingRewardsFunction) {
