...
async function main() {
    ...
    let p = await loadMultipleCurveFantomPools(App, tokens, prices, Pools, CURVE_STAKING_ABI)
  ...
  async function loadMultipleCurveFantomPools(App, tokens, prices, pools, abi) {
    let totalStaked  = 0, totalUserStaked = 0, individualAPRs = [];
    const infos = await Promise.all(pools.map(p =>
        loadFantomCurvePoolInfo(App, tokens, prices, p, abi)));
    for (const i of infos) {
      let p = await printCurvePool(App, i, "fantom");
  ...
  async function loadFantomCurvePoolInfo(App, tokens, prices, stakingAddress, stakingAbi) {
