...
const NFT_FARM_STRATEGY_ADDRESS = '0x2083A6aAbdb733211a99A61615e39fd71547A463'
const FARM_STRATEGY_ADDRESS = '0xBd89f4c0726Afa67aFCeF8AAb171108514a9Bfb8'
...
async function main() {
  ...
  const tokens = {}
  const prices = await getSoneiumPrices()
...
async function loadVelodroneV2SynthetixPools(
  ...
  for (const i of infos.filter(i => i?.poolPrices)) {
    await printVelodromeV2Pool(App, i, 'soneium', customURLs)
  }
}
...
async function loadVelodromeV2SynthetixPoolInfo(
  ...
  const poolPrices = getPoolPrices(tokens, prices, stakeToken, 'soneium')
...
async function printVelodromeV2Pool(App, info, chain = 'eth', customURLs) {
  ...
  const claim = async function() {
    return soneiumContract_claim(info.stakingAddress, App)
  }
  const sickle_soneium_unstake = async function() {
    return sickle_soneiumContract_unstake(info.stakingAddress, info.stakeTokenAddress, info._userStaked, App)
  }
  const sickle_soneium_claim = async function() {
    return sickle_soneiumContract_claim(info.stakingAddress, App)
  }
  _print(`<a target="_blank" href="https://soneium.blockscout.com/address/${info.stakingAddress}#code">Soneium Scan</a>`)
  if (info.has_sickle_account) {
    _print_link(`Unstake ${info.userStaked.toFixed(6)} ${info.stakeTokenTicker}`, sickle_soneium_unstake)
  } else {
...
const sickle_soneiumContract_unstake = async function(rewardPoolAddr, lpToken, userStaked, App) {
...
const sickle_soneiumContract_claim = async function(rewardPoolAddr, App) {
  const signer = App.provider.getSigner()
...
async function loadVelodroneSynthetixPools(
  ...
  for (const i of infos) {
    await printVelodromePool(App, i, 'soneium', customURLs)
  }
}
...
async function printVelodromePool(App, info, chain = 'eth', customURLs) {
  ...
  if (info.has_sickle_account && window.Sickle?.withdraw) {
    for (const nftId of info.userStakedNfts) {
      ...
      // Get native token symbol dynamically (Soneium chain ID is 1868)
      const nativeSymbol = window.Sickle?.SickleUtils?.getNativeTokenSymbol?.(1868) || 'ETH'
