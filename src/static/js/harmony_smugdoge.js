...
async function main() {
  ...
  const SMUG_CHEF = new ethers.Contract(SMUG_CHEF_ADDR, SMUG_CHEF_ABI, App.provider);

  const blockNumber = await App.provider.getBlockNumber();
  const multiplier = await SMUG_CHEF.getMultiplier(blockNumber, blockNumber+1);
  const rewardPerBlock = await SMUG_CHEF.REWARD_PER_BLOCK();
  const rewardsPerWeek = rewardPerBlock / 1e18 * multiplier * 604800 / 2

  const tokens = {};
  const basePrices = await getHarmonyPrices();

  const { prices, totalUserStaked, totalStaked, averageApr } = await loadSmugChefContract(
    App,
    tokens,
    basePrices,
    SMUG_CHEF,
  ...
  const SMUG = new ethers.Contract(SMUG_TOKEN_ADDR, ERC20_ABI, App.provider.getSigner());
  const SMUG_PIT = new ethers.Contract(SMUG_PIT_ADDR, SMUG_PIT_ABI, App.provider.getSigner());
  const smugPit = await smugPitData(SMUG, SMUG_PIT, App, prices);

  const totalStakedInclSmugPit = totalStaked + smugPit.totalBalanceUSD

  _print_bold(`Total Staked: $${formatMoney(totalStakedInclSmugPit)}`);
  ...
  if (smugPit.userBalanceUSD > 0) {
    _print(`You are staking a total of $${formatMoney(smugPit.userBalanceUSD)} in the ${PIT_NAME} single staking pool.`);
  }

  hideLoading();
}

async function loadSmugChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
...
function printSmugChefPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                            totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printSmugChefContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
  rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
  poolInfo.userStaked, poolInfo.pendingRewardTokens, fixedDecimals, claimFunction, rewardPrice, chain, depositFee, withdrawFee);
  return apr;
}

function printSmugChefContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
                                     rewardTokenTicker, stakeTokenTicker, unstaked, userStaked, pendingRewardTokens, fixedDecimals,
                                     claimFunction, rewardTokenPrice, chain, depositFee, withdrawFee) {
  fixedDecimals = fixedDecimals ?? 2;
  const approveAndStake = async function() {
    return smugContract_stake(chefAbi, chefAddr, poolIndex, poolAddress, App)
  }
  const unstake = async function() {
    return smugContract_unstake(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction)
  }
  const claim = async function() {
    return smugContract_claim(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction, claimFunction)
  }
  if(depositFee > 0){
...
const smugContract_stake = async function(chefAbi, chefAddress, poolIndex, stakeTokenAddr, App) {
...
const smugContract_unstake = async function(chefAbi, chefAddress, poolIndex, App, pendingRewardsFunction) {
  const signer = App.provider.getSigner()
  const CHEF_CONTRACT = new ethers.Contract(chefAddress, chefAbi, signer)
...
async function smugPitData(SMUG, SMUG_PIT, App, prices) {
  const smugPitTotalBalance = await SMUG.balanceOf(SMUG_PIT_ADDR) / 1e18;
  const smugPitTotalSupply = await SMUG_PIT.totalSupply() / 1e18;
  const xSmugRatio = smugPitTotalBalance / smugPitTotalSupply;

  const smugPitUserBalanceBigNum = await SMUG_PIT.balanceOf(App.YOUR_ADDRESS);
  const smugPitUserBalance = (smugPitUserBalanceBigNum / 1e18) * xSmugRatio;
  const smugPitShare = smugPitUserBalance / smugPitTotalBalance;
  const smugBalanceBigNum = await SMUG.balanceOf(App.YOUR_ADDRESS);
  const smugBalance = smugBalanceBigNum / 1e18;

  const smugPrice = prices[SMUG_TOKEN_ADDR];

  let smugPitTotalBalanceUSD;
  let smugPitUserBalanceUSD;

  if (smugPrice && smugPrice.usd) {
    smugPitTotalBalanceUSD = smugPitTotalBalance * smugPrice.usd;
    smugPitUserBalanceUSD = smugPitUserBalance * smugPrice.usd;

    _print(`${PIT_NAME} TVL: $${formatMoney(smugPitTotalBalanceUSD)}`);
    _print(`${REWARD_TOKEN_TICKER} Price: $${formatMoney(smugPrice.usd)}`);
    _print(`Staked: ${(smugPitTotalBalance).toFixed(2)} ${REWARD_TOKEN_TICKER} ($${formatMoney(smugPitTotalBalanceUSD)})`);
    _print(`You are staking ${(smugPitUserBalance).toFixed(2)} ${REWARD_TOKEN_TICKER} ($${formatMoney(smugPitUserBalanceUSD)}), ${(smugPitShare * 100).toFixed(2)}% of the pool.`);

    const approveAndEnter = async function() {
      return smugPitApproveAndEnter(SMUG, SMUG_PIT, smugBalanceBigNum, App);
    }

    const leave = async function() {
      return smugPitLeave(SMUG_PIT, smugPitUserBalanceBigNum, App);
    }

    _print_link(`Stake ${(smugBalance).toFixed(2)} ${REWARD_TOKEN_TICKER}`, approveAndEnter);

    if (smugPitUserBalance > 0) {
      _print_link(`Unstake ${smugPitUserBalance.toFixed(2)} ${REWARD_TOKEN_TICKER}`, leave);
    }
  ...
  return {
    totalBalance: smugPitTotalBalance,
    totalBalanceUSD: smugPitTotalBalanceUSD,
    userBalance: smugPitUserBalance,
    userBalanceUSD: smugPitUserBalanceUSD
  }
}

async function smugPitApproveAndEnter(SMUG, SMUG_PIT, currentTokens, App) {
  const allowedTokens = await SMUG.allowance(App.YOUR_ADDRESS, SMUG_PIT_ADDR)
  let allow = Promise.resolve()

  if (allowedTokens / 1e18 < currentTokens / 1e18) {
    showLoading()
    allow = SMUG.approve(SMUG_PIT_ADDR, ethers.constants.MaxUint256)
    .then(function(t) {
      return App.provider.waitForTransaction(t.hash)
    })
    .catch(function() {
  ...
  if (currentTokens / 1e18 > 0) {
    ...
    allow
    .then(async function() {
      SMUG_PIT.enter(currentTokens, {gasLimit: 500000})
      .then(function(t) {
        App.provider.waitForTransaction(t.hash).then(function() {
          hideLoading()
        })
...
async function smugPitLeave(SMUG_PIT, currentStakedAmount, App) {
  showLoading()
  SMUG_PIT.leave(currentStakedAmount, {gasLimit: 500000})
  .then(function(t) {
    App.provider.waitForTransaction(t.hash).then(function() {
      hideLoading()
    })
