$(function () {		
  ...
  async function main() {		
      ...
      const StakingPool = {
        address : "0x704115b8200392f2855b400bf0e414f3c8c3a472",
        abi : pFTM_STAKING_ABI,
        stakeTokenFunction : "pae",
        rewardTokenAddresses : "pToken"
      }
      
      const PAE_CHEF_ADDR = "0xa058316af6275137b3450c9c9a4022de6482bac2";
      const rewardTokenTicker = "PAE";
      const PAE_CHEF = new ethers.Contract(PAE_CHEF_ADDR, PAE_CHEF_ABI, App.provider);

      const rewardsPerWeek = await PAE_CHEF.paePerSecond() / 1e18 * 604800;
          
      const tokens = {};
      const prices = await getFantomPrices();
      
      await loadFantomPaeContract(App, tokens, prices, PAE_CHEF, PAE_CHEF_ADDR, PAE_CHEF_ABI, rewardTokenTicker,		
          "pae", null, rewardsPerWeek, "pendingPAE");
      _print(`\nStaked PAE can only be withdrawn after 6 epochs since deposit.\n`);
      const p0 = await loadFantompFTMSynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                            StakingPool.address, 
...
async function loadFantompFTMSynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadFantompFTMSynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printpFTMSynthetixPool(App, info, "fantom");
}

async function loadFantompFTMSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printpFTMSynthetixPool(App, info, chain="eth", customURLs) {
...
async function loadFantomPaeContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
