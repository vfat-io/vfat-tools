...
async function main() {
   ...
   const SUGAR_CHEF_ADDR = "0x414d03e5Ecb2dab33D7c9Ccb70c3abC69127e41F";
   const MINT_CHEF_ADDR = "0xC74ff270b0d9f495e03078e1bF5b3A7aaa35D18A";
   const rewardTokenTicker = "SUGAR";
   const rewardMINTTokenTicker = "MINT";
   const SUGAR_CHEF = new ethers.Contract(SUGAR_CHEF_ADDR, SUGAR_CHEF_ABI, App.provider);
   const MINT_CHEF = new ethers.Contract(MINT_CHEF_ADDR, MINT_CHEF_ABI, App.provider);

   const startBlock = await SUGAR_CHEF.startBlock();
   ...
   if(currentBlock < startBlock){
    ...
    rewardsPerWeek = await SUGAR_CHEF.sugarPerBlock() /1e18
        * 604800 / 3;
   }

   const rewardsMINTPerWeek = await MINT_CHEF.MintPerBlock() /1e18
      * 604800 / 3;

   const tokens = {};
   const prices = await getBscPrices();

   let p0 = await loadTeaswapContract(App, tokens, prices, MINT_CHEF, MINT_CHEF_ADDR, MINT_CHEF_ABI, rewardMINTTokenTicker,
        "mint", null, rewardsMINTPerWeek, "pendingMint");
...
async function loadTeaswapContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
