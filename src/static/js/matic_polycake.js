...
async function main() {
   ...
   if(currentBlock < startBlock){
    ...
    await loadPcakeContract(App, tokens, prices, PCAKE_CHEF, PCAKE_CHEF_ADDR, PCAKE_CHEF_ABI, rewardTokenTicker,
        "Pcake", null, rewardsPerWeek, "pendingPcake");

    hideLoading();
  }

async function loadPcakeContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getPcakePoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
  ...
  for (i = 0; i < poolCount; i++) {
    if (poolPrices[i]) {
      const apr = printPcakePool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
...
async function getPcakePoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
function printPcakePool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printPcakeContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fee, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printPcakeContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
