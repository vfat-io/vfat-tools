$(function () {
  ...
  async function main() {
    ...
    const DESIRE_CHEF_ADDR = '0xa6f5d8eD74eaab164cddAe9eabE1Fd2900132B8a'
    const rewardTokenTicker = 'DESIRE'
    const DESIRE_CHEF = new ethers.Contract(DESIRE_CHEF_ADDR, DESIRE_CHEF_ABI, App.provider)
  
    const startBlock = await DESIRE_CHEF.startBlock()
    const currentBlock = await App.provider.getBlockNumber()
  
    const multiplier = await DESIRE_CHEF.getMultiplier(currentBlock, currentBlock + 1)
    ...
    if (currentBlock < startBlock) {
      _print(
        ...
        rewardsPerWeek = (((await DESIRE_CHEF.desirePerBlock()) / 1e18) * 604800 * multiplier) / 3
    }
