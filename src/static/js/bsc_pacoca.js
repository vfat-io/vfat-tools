...
$(function() {
    ...
    async function main() {
       ...
       const PACOCA_CHEF_ADDR = "0x55410D946DFab292196462ca9BE9f3E4E4F337Dd";
       const rewardTokenTicker = "PACOCA";
       const PACOCA_CHEF = new ethers.Contract(PACOCA_CHEF_ADDR, PACOCA_CHEF_ABI, App.provider);

       const rewardsPerWeek = await PACOCA_CHEF.PACOCAPerBlock() /1e18
            * 604800 / 3;

        const tokens = {};
        const prices = await getBscPrices();

        await loadPacocaChefContract(App, tokens, prices, PACOCA_CHEF, PACOCA_CHEF_ADDR, PACOCA_CHEF_ABI, rewardTokenTicker,
            "PACOCA", null, rewardsPerWeek, "pendingPACOCA", [2]);

        hideLoading();
    }

async function loadPacocaChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
    ...
    for(let i = 0; i < poolCount; i++){
        const poolInfo = await getPacocaPoolInfo(App, chefContract, i, pendingRewardsFunction);
    ...
    for (i = 0; i < poolCount; i++) {
      if (poolPrices[i]) {
        const apr = printPacocaPool(App, chefAbi, chefAddress, prices, tokens, poolInfos[i], i, poolPrices[i],
  ...
  async function getPacocaPoolInfo(App, chefContract, poolIndex, pendingRewardsFunction) {
    ...
    if(poolIndex == 8){
        // they have confused and they added the staked token into the strat variable and the strat address into the lp token variable
        poolToken = await getBscToken(App, poolInfo.strat, poolInfo.want);
        strat = new ethers.Contract(poolInfo.want, STRAT_ABI, App.provider);
        address = poolInfo.strat;
        pendingRewardTokens = 0;
    }else{
        poolToken = await getBscToken(App, poolInfo.want, poolInfo.strat);
        strat = new ethers.Contract(poolInfo.strat, STRAT_ABI, App.provider);
        address = poolInfo.want;
        pendingRewardTokens = await chefContract.callStatic[pendingRewardsFunction](poolIndex, App.YOUR_ADDRESS);
    }
    try{
        poolToken.staked = await strat.wantLockedTotal() / 1e18;
    }catch(e){
        console.log(e);
        console.log("have an error here pool: ", poolIndex);
        poolToken.staked = 0;
    }
...
function printPacocaPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
                       totalAllocPoints, rewardsPerWeek, rewardTokenTicker, rewardTokenAddress,
  ...
  printPacocaContractLinks(App, chefAbi, chefAddr, poolIndex, poolInfo.address, pendingRewardsFunction,
    rewardTokenTicker, poolPrices.stakeTokenTicker, poolInfo.poolToken.unstaked,
    poolInfo.userStaked, poolInfo.pendingRewardTokens, fixedDecimals, claimFunction, rewardPrice, chain);
  return apr;
}

function printPacocaContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress, pendingRewardsFunction,
  ...
  const unstake = async function() {
    return chefPacocaContract_unstake(chefAbi, chefAddr, poolIndex, App, pendingRewardsFunction)
  }
...
const chefPacocaContract_unstake = async function(chefAbi, chefAddress, poolIndex, App, pendingRewardsFunction) {
