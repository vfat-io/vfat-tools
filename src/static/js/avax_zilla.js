$(function () {		
  ...
  async function main() {		
      ...
      const StakingPool = {
        address : "0x9e9e6b64c7028667828f27641526f47a8bf693c5",
        abi : ZILLA_STAKING_ABI,
        stakeTokenFunction : "zshare",
        rewardTokenAddresses : "zilla"
      }
      
      const ZSHARE_CHEF_ADDR = "0x978a7c975b1e1e8f7c303a114a73aadeb58493d4";
      const rewardTokenTicker = "ZSHARE";
      const ZSHARE_CHEF = new ethers.Contract(ZSHARE_CHEF_ADDR, ZSHARE_CHEF_ABI, App.provider);

      const rewardsPerWeek = await ZSHARE_CHEF.tSharePerSecond() / 1e18 * 604800;
          
      const tokens = {};
      const prices = await getAvaxPrices();
      
      await loadAvaxZshareContract(App, tokens, prices, ZSHARE_CHEF, ZSHARE_CHEF_ADDR, ZSHARE_CHEF_ABI, rewardTokenTicker,		
          "zshare", null, rewardsPerWeek, "pendingShare");
      _print(`\nStaked ZSHARE can only be withdrawn after 3 epochs (18 hours) since deposit.`);
      _print(`Any time tokens are harvested, deposited, or withdrawn, the lockup timer gets reset.\n`);
      const p0 = await loadAvaxZILLASynthetixPool(App, tokens, prices, StakingPool.abi, 
                                                                            StakingPool.address, 
...
async function loadAvaxZILLASynthetixPool(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction) {
  const info = await loadAvaxZILLASynthetixPoolInfo(App, tokens, prices, abi, address, rewardTokenFunction, stakeTokenFunction);
  if (!info) return null;
  return await printZILLASynthetixPool(App, info, "avax");
}

async function loadAvaxZILLASynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
...
async function printZILLASynthetixPool(App, info, chain="eth", customURLs) {
...
async function loadAvaxZshareContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
