...
const contracts = {
    comptroller: '0x97b491744587d05ca33e84bB18B61Df9B3986DcE',
    oracle: '0x50BAe7d486075e46AcdA0813921A71a3e91a2d25',
    gas: {
        p_address: '0xe970c37243F3d0B2AeB041b855Ef6466CB140BcA',
        symbol: 'ETH',
        decimals: 18,
    },
};
...
async function main() {
    ...
    const COMPTROLLER = new ethers.Contract(contracts.comptroller, BOBA_ABI.comptroller, App.provider);
...
async function loadData(App, token) {
    ...
    const PTOKEN_CONTRACT = new ethers.Contract(token, BOBA_ABI.pToken, App.provider);
    ...
    if (token.toLowerCase() === contracts.gas.p_address.toLowerCase()) {
        ...
        const UNDERLYING_CONTRACT = new ethers.Contract(underlying, BOBA_ABI.erc20, App.provider);
        underlyingSymbol = await UNDERLYING_CONTRACT.symbol();
        underlyingDecimals = await UNDERLYING_CONTRACT.decimals();
    }

    const ORACLE_CONTRACT = new ethers.Contract(contracts.oracle, BOBA_ABI.oracle, App.provider);
    ...
    const RATE_CONTRACT = new ethers.Contract(interestRateModel, BOBA_ABI.rate, App.provider);
