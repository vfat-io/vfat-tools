...
$(function() {
  ...
  async function main() {
     ...
     const JONES_CHEF_ADDR = "0x4EAF0b76a8740e01C054c076E15A3eb758FFE535";
     const JONES_CHEF_ADDR2 = "0xD534bC696D708Bd8E5f035e119D260D79c69F398";
     const JONES_CHEF_ADDR3 = "0x0FFcd436B18FcE15CFCdE62a6490ADa36f973836";

     const rewardTokenTicker = "JONES";
     const rewardTokenTicker3 = "DJONES";
     const JONES_CHEF = new ethers.Contract(JONES_CHEF_ADDR, JONES_CHEF_ABI, App.provider);
     const JONES_CHEF2 = new ethers.Contract(JONES_CHEF_ADDR2, JONES_CHEF_ABI2, App.provider);
     const JONES_CHEF3 = new ethers.Contract(JONES_CHEF_ADDR3, JONES_CHEF_ABI3, App.provider);

     let currentEpoch;
     const currentTime = Date.now() / 1000
     const epochEndTimes = await JONES_CHEF2.epochEndTimes(0);
     if(currentTime < epochEndTimes){
      currentEpoch = 0;
     }else{
      currentEpoch = 1;
     }

     const rewardsPerWeek = await JONES_CHEF.tombPerSecond() /1e18 * 604800;
     const rewardsPerWeek2 = await JONES_CHEF2.epochTombPerSecond(currentEpoch) /1e18 * 604800;
     const rewardsPerWeek3 = await JONES_CHEF3.tSharePerSecond() /1e18 * 604800;

      const tokens = {};
      const prices = await getFantomPrices();

      await loadJonesFtmLptSynthetixPoolInfo(App, tokens, prices, JONES_CHEF_ADDR, "0x21612a24ecbbc2dd5f36e3211bf4c3bd28d79af1")

      let p2 = await loadDavyChefContract2(App, tokens, prices, JONES_CHEF3, JONES_CHEF_ADDR3, JONES_CHEF_ABI3, rewardTokenTicker3,
        "tshare", null, rewardsPerWeek3, "pendingShare");

      _print("");

      let p0 = await loadDavyChefContract2(App, tokens, prices, JONES_CHEF2, JONES_CHEF_ADDR2, JONES_CHEF_ABI2, rewardTokenTicker,
        "tomb", null, rewardsPerWeek2, "pendingTOMB");

      _print("");

      let p1 = await loadDavyChefContract(App, tokens, prices, JONES_CHEF, JONES_CHEF_ADDR, JONES_CHEF_ABI, rewardTokenTicker,
        "tomb", null, rewardsPerWeek, "pendingTOMB", [1]);
...
async function loadJonesFtmLptSynthetixPoolInfo(App, tokens, prices, stakingAddress, stakeTokenAddress) {
...
async function loadDavyChefContract2(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
...
async function loadDavyChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  rewardTokenFunction, rewardsPerBlockFunction, rewardsPerWeekFixed, pendingRewardsFunction,
  deathPoolIndices, claimFunction) {
  const chefContract = chef ?? new ethers.Contract(chefAddress, chefAbi, App.provider);
