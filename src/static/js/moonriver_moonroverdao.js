...
$(function() {
    ...
    async function main() {
       ...
       const startBlock = await ROVR_CHEF.startBlock();
        ...
        await loadMoonroverChefContract(App, tokens, prices, ROVR_CHEF1, ROVR_CHEF_ADDR1, ROVR_CHEF_ABI1, rewardTokenTicker,
            "rovr", null, rewardsPerWeek1, "pendingRovr");
    
        hideLoading();
      }
    
async function getMoonroverPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
...
async function loadMoonroverChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getMoonroverPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)));
